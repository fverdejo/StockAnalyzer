...
HTML;
    ...
    private static function renderResult(array $result, int $pageNum, string $paginationBase, float $fundamentalWeightPercent): string
    {
        ...
        return $tickerErrorsHtml
            ...
            . self::columnHeader('Alpha vs sin señal', 'Retorno medio de las compras menos el retorno medio de los días SIN señal de Comprar (Mantener/Vender/Venta fuerte) -- los dos grupos no comparten ninguna FECHA DE SEÑAL, una separación limpia por construcción. Eso no garantiza independencia estadística completa: la ventana de retorno de una compra y la de una venta cercana en el tiempo pueden llegar a compartir algún día de precio. Esta es la que responde "¿comprar en la señal distingue algo real del resto del tiempo?", y la que sustenta la columna "t de la alpha".', true)
            . self::columnHeader('t de la alpha', 'Alpha vs sin señal dividida entre su error estándar (fórmula de Welch). No es un contraste con independencia garantizada: además de la dependencia ya conocida entre muestras del mismo lado (ventanas de horizonte solapadas, ver "Muestras" y "muestras efectivas independientes" en el detalle de cada fila), las ventanas de compra y no-compra también pueden solapar algún día entre sí. |t| mayor o igual que 1,96 es la referencia habitual, no una frontera exacta entre señal y azar. Con un universo de N tickers, esta columna equivale a N pruebas simultáneas: unas pocas superarán el umbral por puro azar aunque no haya ninguna ventaja real (aproximadamente el 5% de N).', true)
            . '</tr></thead><tbody>'
            . implode('', $rows)
            . '</tbody></table></div>'
            . Layout::renderPagination($pageNum, $totalPages, $paginationBase)
            . '<p class="muted panel-note">t de la alpha: alpha vs sin señal dividida entre su error estándar (fórmula de Welch). Compras y no-compras no comparten ninguna fecha de señal, pero eso no garantiza independencia completa: sus ventanas de retorno pueden solapar algún día entre sí, además de la dependencia ya conocida entre muestras del mismo lado (ventanas de horizonte solapadas). |t| &ge; 1,96 es la referencia habitual, no una frontera exacta entre señal y azar -- y con un universo de N tickers, esperar que unos pocos la superen por puro azar (~5% de N) es lo normal, no un hallazgo por sí solo.</p>'
