...
- **Herramienta lista para cuando pase el tiempo que hace falta, `versions.md` septima entrada del `2026-09-05`**: tanto E2 como E3 dependen de la MISMA pregunta sin contestar (¿el calendario/consenso que hoy parece historico es un vintage congelado, o EODHD lo reescribe?), y esa pregunta no se puede acelerar trabajando mas, necesita que pase tiempo real entre dos capturas. `EodhdRawFundamentalVersionsRepository::allPayloadsFor()` (nuevo) mas `bin/compare-eodhd-calendar-versions.php` (nuevo, probado de extremo a extremo con un caso sintetico) ya estan listos: **dentro de unas semanas**, volver a ejecutar `bin/archive-eodhd-calendar-earnings.php` (para desbloquear E2) y `bin/archive-eodhd-calendar-trends.php` (para E3) y despues `bin/compare-eodhd-calendar-versions.php --section=earnings`/`--section=trends` -- la respuesta sale sola, sin escribir nada de codigo nuevo entonces.
