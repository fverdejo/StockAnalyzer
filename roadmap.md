...
- **`PLAN_VALIDACION_MOTOR_ASTRA_2026-09-10.md` (Astra, plan de validacion del motor sobre el commit `68a4b82`): Entrega 1 (prioridad inmediata) cerrada el mismo dia** (`versions.md`, segunda entrada). `AlertService::checkStopLossBreach()` abandonaba sin comparar nada en cuanto faltaban `RiskLevels` nuevos, incluso cuando ya habia un stop adoptado guardado -- quien consumia el estado releia por separado el ULTIMO estado persistido, que podia estar desactualizado en cualquier direccion (un stop realmente perdido seguia pareciendo "por encima", o un precio ya recuperado seguia pareciendo "perdido"). Corregido: comparar contra un stop YA ADOPTADO ya no necesita niveles nuevos (solo hacen falta para adoptar uno por primera vez); el metodo devuelve un `DTO\StopLossCheck` explicito (`DENTRO`/`CRUZADO`/`SIN_EVALUAR`) en vez de `void`, y el antiguo `isBelowActiveStop()` (la relectura separada que causaba la desactualizacion) se retira. `PositionDecisionAdvisor::decide()` ya no puede caer en `MANTENER` ("dentro de su stop-loss adoptado") cuando el estado es `SIN_EVALUAR`: nueva accion `REVISAR_STOP`, con la misma prioridad que `SALIR`. Cuatro casos de la reproduccion de Astra cubiertos con tests nuevos.
  ...
  - **Pendiente para una sesion futura, todo genuinamente bloqueado o delegado, no por falta de tiempo**: la decision de Francisco entre las tres opciones para la medicion completa de 636 tickers/10 años (`.wslconfig`, lotes mas pequeños, o reducir alcance -- bloqueada por el limite real de infraestructura de WSL2, tres fallos de la misma causa raiz); una fuente de FX historica fechada para cerrar del todo B3 (sub-punto declarado bloqueado, ver arriba); ampliar el piloto del caso 6 al universo completo (probablemente choca con el mismo limite de infraestructura); construir la variante (B) del caso 3 de la auditoria anterior (observacion periodica); A7/extremos del bootstrap (consulta futura con `auditor-estadistico`, decision de metodologia delegada en agentes). El calendario de referencia compartido (caso 2/A6) sigue DEGRADADO de prioridad (0,31% sobre el universo completo, medido dos veces) -- Astra matiza que esa cifra mide SIMBOLOS con hueco, no episodios afectados ni impacto economico, y pide medir ventanas afectadas antes de excluir `LEG`/`FISV` del universo. Los 225 tickers de Japon/Italia/Singapur/Israel/Nueva Zelanda quedan sin ningun dato de EODHD de forma permanente bajo el plan actual. La medicion del `2026-09-14` sigue invalidada.
