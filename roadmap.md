...
**Despues de (4), en orden de prioridad del equipo**: `P3.4` (momentum-only con el lookback corregido, la señal con mas respaldo academico del proyecto, prioridad de `trader-tendencia`) y `P3.3` (conectar `RelativeFundamentalScorer` al backtest transversal con los 628 tickers — primera vez que la muestra cumple `MIN_PEERS=8`, prioridad de `inversor-fundamental`, requiere trabajo de `desarrollador-php` primero).
