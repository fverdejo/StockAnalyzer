...
- **`PLAN_VALIDACION_MOTOR_ASTRA_2026-09-10.md` (Astra, plan de validacion del motor sobre el commit `68a4b82`): Entrega 1 (prioridad inmediata) cerrada el mismo dia** (`versions.md`, segunda entrada). `AlertService::checkStopLossBreach()` abandonaba sin comparar nada en cuanto faltaban `RiskLevels` nuevos, incluso cuando ya habia un stop adoptado guardado -- quien consumia el estado releia por separado el ULTIMO estado persistido, que podia estar desactualizado en cualquier direccion (un stop realmente perdido seguia pareciendo "por encima", o un precio ya recuperado seguia pareciendo "perdido"). Corregido: comparar contra un stop YA ADOPTADO ya no necesita niveles nuevos (solo hacen falta para adoptar uno por primera vez); el metodo devuelve un `DTO\StopLossCheck` explicito (`DENTRO`/`CRUZADO`/`SIN_EVALUAR`) en vez de `void`, y el antiguo `isBelowActiveStop()` (la relectura separada que causaba la desactualizacion) se retira. `PositionDecisionAdvisor::decide()` ya no puede caer en `MANTENER` ("dentro de su stop-loss adoptado") cuando el estado es `SIN_EVALUAR`: nueva accion `REVISAR_STOP`, con la misma prioridad que `SALIR`. Cuatro casos de la reproduccion de Astra cubiertos con tests nuevos.
  ...
  - **Pendiente para una sesion futura**: B3 (contrato de moneda/unidad -- fixture de Astra: la misma cotizacion en peniques vs libras da PER 500 vs 5, sin ese contexto no se pueden conectar los internacionales de forma fiable); B4 (conectar el archivo v1.1 a `PointInTimeFundamentalsBuilder` para los 159 simbolos exclusivamente-v1.1, que hoy tienen archivo pero CERO snapshots -- depende de B3 para los ratios monetarios); B7 (precargar por ticker y resolver en memoria -- el fixture de Astra cuenta 1.464 llamadas por ticker en el runner del replay); el "archivo EODHD recuperable" (exportar archivo+observaciones con hashes, verificar una restauracion aislada sin red, antes del vencimiento del `2026-10-01`); escalar el lote de validacion del replay a mas tickers/tiempo para intentar SI acercarse al limite de memoria con telemetria real (RSS, eventos de contenedor) antes de decidir entre `.wslconfig`, lotes mas pequeños, o reducir alcance; ampliar el piloto del caso 6 al universo completo; construir la variante (B) del caso 3 de la auditoria anterior; A7 (extremos del bootstrap -- consulta futura con `auditor-estadistico`). El calendario de referencia compartido (caso 2/A6) sigue DEGRADADO de prioridad (0,31% sobre el universo completo, medido dos veces) -- Astra matiza que esa cifra mide SIMBOLOS con hueco, no episodios afectados ni impacto economico, y pide medir ventanas afectadas antes de excluir `LEG`/`FISV` del universo (no solo por la conveniencia de una cifra mas bonita). Los 225 tickers de Japon/Italia/Singapur/Israel/Nueva Zelanda quedan sin ningun dato de EODHD de forma permanente bajo el plan actual. La medicion del `2026-09-14` sigue invalidada.
