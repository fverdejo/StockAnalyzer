...
- ~~**Grafico de precio (velas, `2026-09-02`): pan vertical y aprovechar mejor el ancho.**~~ HECHO el `2026-09-04` (tercera entrada de `versions.md` de ese dia): causa del hueco confirmada leyendo el codigo fuente real de `chartjs-chart-financial`/`chart.js` (el `offset: true` heredado de `BarController` para su eje `category` original seguia aplicando tras cambiar el eje a `'linear'`) -- arreglado con `scales.x.offset: false`. Pan vertical: `pan.mode` de `'x'` a `'xy'`, mas `limits.y: { min: 'original', max: 'original' }` (zoom con rueda/pellizco se queda en `'x'`). Verificado visualmente por Claude con Chrome headless contra WFC: las velas llegan ahora al borde del area de grafico, alineadas con el grafico de Volumen de abajo.
