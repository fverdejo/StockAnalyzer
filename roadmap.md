...
- **`SEGUIMIENTO_BACKTESTING_ASTRA_2026-09-09.md` (Astra, continuacion de la auditoria del `2026-09-08`): casos 1 y 2 (los mas graves, P1) corregidos el mismo dia** (`versions.md`, misma fecha) -- `sampleOnCalendar()` ancla la fecha de señal de CADA ticker al mismo calendario compartido en vez de al indice local de cada uno, cerrando el hallazgo de que dos tickers sin huecos internos pero con primera fecha distinta muestreaban fechas de calendario diferentes para "la misma" señal. `hasCalendarGap()` (guarda de todo-o-nada por ticker, `2026-09-08`) se retira, sustituida por un descarte por MUESTRA dentro de `sampleOnCalendar()`, que solo invalida la ventana de lookback/entrada/horizonte que de verdad tiene un hueco -- un hueco posterior a que una operacion ya se resolviera nunca la invalida (caso 2). `collectSamplesWithHistory()`/`runDeteriorationRiskAnalysis()` (E1, ya cerrado con veredicto nulo muy lejos de cualquier umbral) se dejan deliberadamente SIN migrar, documentado como limitacion conocida en su propio docblock -- migrarlos exigiria la misma reestructuracion a dos pasadas, sin justificarse hoy para un resultado ya tan lejos de significar nada. **Pendiente, explicitamente aplazado a una sesion futura** (los tres son "P2, investigacion" segun la propia Astra, de menor prioridad que los casos 1/2 ya cerrados):
  - Caso 3: `runForPeerGroup()` puede mostrar "todo el grupo sectorial" cuando en realidad solo cubrio una parte (limite de 5 calculos nuevos por peticion) -- falta exponer cobertura/completitud en el resultado.
  - Caso 4: en modo `momentum`, los empates de capitalizacion se reparten entre terciles segun el orden de entrada de la lista, no por un criterio estable -- reproducido con un conjunto sintetico donde invertir el orden de 20 tickers con la misma capitalizacion cambia el signo de la alpha (de +10pp a -10pp). Necesita un desempate secundario estable antes de construir terciles.
  - Caso 5: el minimo de 20 valores por sector se comprueba ANTES de descartar candidatos sin validez fundamental point-in-time -- la documentacion de la constante no aclara si "20" es bruto o elegible tras filtros, y la garantia actual ("al menos 20 supervivientes") es falsa. Aclarar el contrato, exponer brutos y elegibles por separado.
  - Precision estadistica (parte del mismo seguimiento, sin caso numerado propio): etiquetar como aproximaciones -- no como independencia/significancia garantizada -- el intervalo normal con pocas fechas (ya documentado como limitacion el `2026-09-08`, Astra pide ademas retirar la deduccion automatica de "superar 1,96 separa señal de azar" de la interfaz mientras no se valide el contraste aplicable) y el uso de Welch (muestras independientes) para `buy_alpha_t_stat`, publicado junto a metricas que SI son pareadas por fecha -- Welch y el contraste pareado tienen denominadores distintos y no deben leerse como la misma pregunta estadistica.
