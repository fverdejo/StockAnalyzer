...
- **Rediseño de "Mi cartera": tres bloques aprobados por el usuario el 2026-08-10 y aplazados a una sesion con tiempo.** Salen de la auditoria de `diseno-usabilidad` (medida en navegador, no estimada) de la que `v2.85` ya implemento solo los 6 bugs. En orden de impacto/esfuerzo:
    1. **Reordenar los paneles**: tarjetas -> **posiciones abiertas** -> grafico de evolucion -> concentracion -> historial (`PortfolioPage::render()`, lineas 50-67 y las asignaciones de 43-48). Hoy las posiciones —el motivo de entrar en la pagina— empiezan en y=1.253 en escritorio (fuera del primer pantallazo) y en **y=2.857 en movil**, siete pantallas de scroll. Riesgo casi nulo: mover interpolaciones de un heredoc, con el cuidado de arrastrar el `<script>` de Chart.js que se emite dentro de `renderValueHistoryChart()`.
    2. **Tabla de posiciones con las cifras a la derecha** y `font-variant-numeric: tabular-nums` en Acciones/Precio medio/Precio actual/Invertido/Beneficio, con la equivalencia en euros bajada a una segunda linea `muted` de 11px en vez de inline entre parentesis (`renderHoldings()` 243-263 y `eurEquivalent()` 466-476; CSS junto a `.table-compact`). Verificado en prototipo: `scrollWidth` baja a 1.200 = ancho visible, o sea la tabla deja de desbordar en escritorio, y las cifras pasan a ser comparables entre filas, que es lo unico que se hace en esa tabla. Aplicar la misma clase a la tabla del historial (linea 350) y a la de la watchlist (`WatchlistPage.php:96`), que hoy pinta la misma fila conceptual con otra densidad y otra alineacion vertical.
    3. **Panel de concentracion reescrito** (`renderConcentration()` 113-160 y `weightList()` 172-192): listas `.list-row` -> barras horizontales reutilizando `.score-bars` (`Layout.php` 1063-1094, ya en la hoja, cero JS); "Por divisa" -> aviso condicional solo si se supera el umbral, con el patron que ya usa `DashboardPage::renderSectorNote()` (140-168); el HHI crudo al `data-tooltip` de un `.info-icon` sobre "Posiciones efectivas" en vez de a 24px en negrita compitiendo con su propia traduccion; y los subtitulos en `<h3 class="panel-subtitle">` en vez de `.metric`, para quitar el efecto de tarjeta dentro de tarjeta (`.panel` y `.metric` comparten estilo exacto, y es la causa principal de que la pagina se vea plana). Medido: el panel baja de 528px a ~300px en escritorio y de **1.711px a ~450px en movil**. Es el bloque que mas HTML cambia; verificar los cuatro casos limite tras implementar: 1 posicion, 1 sector, "Sin sector", cartera solo en euros.
- **Barras y no tarta para los sectores (decidido).** El usuario pregunto por un diagrama de sectores; se descarto la tarta con motivos concretos: el sistema de color no tiene paleta categorica (un accent + `--good`/`--warn`/`--bad`, que significan *veredicto*: un sector en rojo se leeria como "sector malo"), con 8-11 sectores GICS la tarta son astillas indistinguibles y con un solo sector es un anillo que no informa. Si algun dia se quiere la tarta como acompañante (solo >920px), antes hay que definir 6-8 tokens de color categoricos con contraste entre porciones adyacentes y sin pisar `--focus`; para eso, cargar la skill `dataviz`.
- Ampliar la cobertura de tests a `Repository/` y al resto de rutas de `Application.php` (la suite esta en 194 tests desde `v2.85`; `v2.79` cubrio `resolveTickerRequest()`)
