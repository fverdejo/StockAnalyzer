...
- **`PLAN_VALIDACION_MOTOR_ASTRA_2026-09-10.md` (Astra, plan de validacion del motor sobre el commit `68a4b82`): Entrega 1 (prioridad inmediata) cerrada el mismo dia** (`versions.md`, segunda entrada). `AlertService::checkStopLossBreach()` abandonaba sin comparar nada en cuanto faltaban `RiskLevels` nuevos, incluso cuando ya habia un stop adoptado guardado -- quien consumia el estado releia por separado el ULTIMO estado persistido, que podia estar desactualizado en cualquier direccion (un stop realmente perdido seguia pareciendo "por encima", o un precio ya recuperado seguia pareciendo "perdido"). Corregido: comparar contra un stop YA ADOPTADO ya no necesita niveles nuevos (solo hacen falta para adoptar uno por primera vez); el metodo devuelve un `DTO\StopLossCheck` explicito (`DENTRO`/`CRUZADO`/`SIN_EVALUAR`) en vez de `void`, y el antiguo `isBelowActiveStop()` (la relectura separada que causaba la desactualizacion) se retira. `PositionDecisionAdvisor::decide()` ya no puede caer en `MANTENER` ("dentro de su stop-loss adoptado") cuando el estado es `SIN_EVALUAR`: nueva accion `REVISAR_STOP`, con la misma prioridad que `SALIR`. Cuatro casos de la reproduccion de Astra cubiertos con tests nuevos.
  ...
  - **Casos 3 y 4, pendientes de consulta de metodologia, sin tocar codigo todavia**:
    - Caso 3 (contrato del stop): el replay fija el stop en la fecha de la SEÑAL; `AlertService::checkStopLossBreach()` en produccion lo adopta en la PRIMERA CONSULTA con una posicion sin ese estado (que puede llegar mucho mas tarde si el usuario no abre la app justo al comprar) -- Astra demuestra con un fixture que esto puede cambiar SALIR por MANTENER segun cuando se simula que el usuario mira. Decision de que se esta midiendo (orden stop permanentemente activa vs decisiones solo al observar el precio), pendiente de consulta a `analista-mercado`/`gestor-riesgo`.
    - Caso 4 (independencia de los bloques): el diseño de ventanas de calendario del `2026-09-14` sigue sin garantizar independencia real -- Astra construye un contraejemplo (12 operaciones con comparadores expuestos a la misma sesion futura) que da `t=-110,97`, una cifra absurda; tambien señala que ponderar por VENTANA en vez de por OPERACION cambia la magnitud estimada, no solo su incertidumbre. Pendiente de una tercera consulta a `auditor-estadistico` antes de tocar `PolicyReplayStatistics`.
  - **Casos 5 (archivar el detalle completo del estudio) y 6 (protocolo de valoracion comun a una fecha de corte, ya predeclarado en su forma general en la entrada de arriba)**: se abordan despues de cerrar los casos 3 y 4, siguiendo el orden que recomienda la propia Astra.
