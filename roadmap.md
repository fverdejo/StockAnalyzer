...
- **`PLAN_VALIDACION_MOTOR_ASTRA_2026-09-10.md` (Astra, plan de validacion del motor sobre el commit `68a4b82`): Entrega 1 (prioridad inmediata) cerrada el mismo dia** (`versions.md`, segunda entrada). `AlertService::checkStopLossBreach()` abandonaba sin comparar nada en cuanto faltaban `RiskLevels` nuevos, incluso cuando ya habia un stop adoptado guardado -- quien consumia el estado releia por separado el ULTIMO estado persistido, que podia estar desactualizado en cualquier direccion (un stop realmente perdido seguia pareciendo "por encima", o un precio ya recuperado seguia pareciendo "perdido"). Corregido: comparar contra un stop YA ADOPTADO ya no necesita niveles nuevos (solo hacen falta para adoptar uno por primera vez); el metodo devuelve un `DTO\StopLossCheck` explicito (`DENTRO`/`CRUZADO`/`SIN_EVALUAR`) en vez de `void`, y el antiguo `isBelowActiveStop()` (la relectura separada que causaba la desactualizacion) se retira. `PositionDecisionAdvisor::decide()` ya no puede caer en `MANTENER` ("dentro de su stop-loss adoptado") cuando el estado es `SIN_EVALUAR`: nueva accion `REVISAR_STOP`, con la misma prioridad que `SALIR`. Cuatro casos de la reproduccion de Astra cubiertos con tests nuevos.
  ...
  - **A5, A6 y A7, reconocidos sin cambio de codigo**: A5 (procedencia/frescura contable conservada hasta el diagnostico, UI de la ficha comparando tecnico y fundamental) es una pieza de producto mayor, sin empezar. A6 -- Astra demuestra que la estimacion de maduracion (`entrada + 28 dias`, corregida el `2026-09-16`) puede seguir clasificando mal un episodio cerca de un festivo, Y que mi propia medicion de 150 tickers del mismo dia tiene un punto ciego real: el umbral del 90% puede descartar fechas antiguas del calendario de referencia por tickers incorporados tarde al universo, aunque los tickers viejos si tengan dato ahi. **La cifra "1/150" pasa de "medicion limpia" a "cota con sesgo conocido hacia abajo"** -- no se repite la medicion hasta corregir su denominador (instruccion explicita de Astra), y sigue sin resolverse el caso 2 subyacente (calendario de referencia compartido). A7 agrupa lo ya pendiente (extremos del bootstrap, reanudacion del archivado) mas precarga por ticker con consultas as-of, sin cambios en esta entrada.
  - **Pendiente para una sesion futura**: resolver el caso 2/A6 (calendario de referencia compartido) y el limite de infraestructura del caso 5; completar la medicion de 636 tickers/10 años con todos los arreglos aplicados (ahora con mas cobertura de fundamentales disponible tras A1); ampliar el piloto del caso 6 al universo completo; construir la variante (B) del caso 3 de la auditoria anterior (observacion periodica, distinta del caso 3 de ESA auditoria); A5 (producto: procedencia/frescura fundamental en la ficha); revisar `EodhdFiscalPeriodProvider::totalDebt()` (mismo patron de "ausencia se convierte en 0.0" que A4, pero con una ambiguedad distinta -- EODHD normalmente si reporta explicitamente un componente en cero) y la procedencia de un resultado valido vacio en `earnings_events` (A2, exige cambio de esquema). Los 225 tickers de Japon/Italia/Singapur/Israel/Nueva Zelanda quedan sin ningun dato de EODHD de forma permanente bajo el plan actual -- no reintentar sin contratar cobertura adicional. La medicion del `2026-09-14` sigue invalidada.
