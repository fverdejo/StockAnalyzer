...
## Prioridad cero-ter: corregir la especificacion del fundamental antes de repetirlo (pendiente, `2026-09-03`)
...
- **Buscador del Home: buscar por nombre de empresa falla si el ticker no esta en un universo configurado (`2026-09-04`).** Reportado por el usuario: buscar "Nokia" no devuelve resultados, pero buscar directamente por su ticker (`0K8D.L` segun Yahoo Finance) si funciona. Sin implementar, solo anotado para comprobar. Causa probable (no confirmada con debugging real, solo lectura de codigo): `CompanyDirectory` (`src/Config/CompanyDirectory.php`) es un diccionario LOCAL ticker->nombre, cerrado a los tickers que ya aparecen en `config/universes.php` (su propio docblock lo dice: "no es un buscador de mercado completo... solo permite encontrar por nombre las empresas que ya estan en alguna lista configurada") -- Nokia/`0K8D.L` no aparece en ningun universo configurado ni en `CompanyDirectory::NAMES`, confirmado por grep sobre todo el repo. La busqueda directa por ticker en cambio no pasa por este directorio local, va directa a Yahoo Finance (de ahi que si funcione). Pendiente de decidir: ¿merece la pena un proveedor de busqueda de simbolos real (mismo tipo de brecha que "Proveedor oficial de noticias/datos" mas arriba), o basta con ampliar `CompanyDirectory` a mano segun vayan apareciendo casos como este?
