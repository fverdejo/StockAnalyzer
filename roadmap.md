...
## Plan de aprovechamiento de EODHD (Codex, `2026-09-04`): Bloque A y B1-B3-B7 cerrados
...
Pendiente del plan, sin empezar: Bloque B4 (dividendos, baja prioridad segun la propia verificacion -- solo da fechas, los importes ya estan en el archivo legacy), B5 (MID/SML/OEX, confirmado sin historico, no insistir), B6 (listas de simbolos activos/deslistados), Bloque C (modelo de datos normalizado: `fiscal_periods`/`earnings_events`/`estimate_trends`/`corporate_actions`), y Bloques D/E (nuevo marco de "estado/cambio fundamental" y experimentos nuevos) -- estos dos ultimos requieren consulta con el equipo (`auditor-estadistico`/`analista-mercado`/`inversor-fundamental`) antes de implementar nada, mismo criterio que el resto de hallazgos de Codex este mes.
