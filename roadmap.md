...
- **`PLAN_VALIDACION_MOTOR_ASTRA_2026-09-10.md` (Astra, plan de validacion del motor sobre el commit `68a4b82`): Entrega 1 (prioridad inmediata) cerrada el mismo dia** (`versions.md`, segunda entrada). `AlertService::checkStopLossBreach()` abandonaba sin comparar nada en cuanto faltaban `RiskLevels` nuevos, incluso cuando ya habia un stop adoptado guardado -- quien consumia el estado releia por separado el ULTIMO estado persistido, que podia estar desactualizado en cualquier direccion (un stop realmente perdido seguia pareciendo "por encima", o un precio ya recuperado seguia pareciendo "perdido"). Corregido: comparar contra un stop YA ADOPTADO ya no necesita niveles nuevos (solo hacen falta para adoptar uno por primera vez); el metodo devuelve un `DTO\StopLossCheck` explicito (`DENTRO`/`CRUZADO`/`SIN_EVALUAR`) en vez de `void`, y el antiguo `isBelowActiveStop()` (la relectura separada que causaba la desactualizacion) se retira. `PositionDecisionAdvisor::decide()` ya no puede caer en `MANTENER` ("dentro de su stop-loss adoptado") cuando el estado es `SIN_EVALUAR`: nueva accion `REVISAR_STOP`, con la misma prioridad que `SALIR`. Cuatro casos de la reproduccion de Astra cubiertos con tests nuevos.
  ...
  - **Resultado numerico obtenido, pendiente de interpretacion por agentes antes de declarar ninguna conclusion** (`versions.md`, misma fecha): diferencia pareada -5,94pp (base)/-5,93pp (estres), mismo signo y magnitud similar a la medicion invalidada del `2026-09-14` (-5,49pp) -- consistente con que el hallazgo cualitativo original no era puramente un artefacto de los bugs ya corregidos. Pero `bootstrap_has_enough_resolution: false` (la propia salvaguarda predeclarada de este proyecto): con solo 84 ventanas de calendario observadas frente a un ancho de bloque de 982 dias, la resolucion no alcanza para tratar el IC95%/pseudo-t como concluyente. **Pendiente: consulta con `auditor-estadistico`** sobre si este resultado puede reportarse de alguna forma (y como) o queda como "medido, sin resolucion suficiente" -- decision de metodologia, no de mercado, delegada en agentes por mandato explicito de Francisco.
  - **Pendiente para una sesion futura**: la consulta con `auditor-estadistico` de arriba (interpretacion del resultado y su propio ofrecimiento pendiente de revisar el tratamiento de extremos del bootstrap); una fuente de FX historica fechada para cerrar del todo B3 (sub-punto declarado bloqueado); ampliar el piloto del caso 6 al universo completo (ahora con mucha mas confianza en que la infraestructura lo soportara); construir la variante (B) del caso 3 de la auditoria anterior (observacion periodica, decision de metodologia tambien delegada en agentes); la pregunta de seguimiento ya identificada el `2026-09-14` ("comparar retorno TOTAL a una fecha de corte comun, cerradas + pendientes marcadas a mercado", que evitaria el sesgo hacia perdedoras de la comparacion actual). El calendario de referencia compartido (caso 2/A6) sigue DEGRADADO de prioridad (0,31% sobre el universo completo, medido dos veces). Los 225 tickers de Japon/Italia/Singapur/Israel/Nueva Zelanda quedan sin ningun dato de EODHD de forma permanente bajo el plan actual. La medicion del `2026-09-14` sigue invalidada (bugs de codigo reales, distinta a esta).
