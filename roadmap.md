...
- **`PLAN_VALIDACION_MOTOR_ASTRA_2026-09-10.md` (Astra, plan de validacion del motor sobre el commit `68a4b82`): Entrega 1 (prioridad inmediata) cerrada el mismo dia** (`versions.md`, segunda entrada). `AlertService::checkStopLossBreach()` abandonaba sin comparar nada en cuanto faltaban `RiskLevels` nuevos, incluso cuando ya habia un stop adoptado guardado -- quien consumia el estado releia por separado el ULTIMO estado persistido, que podia estar desactualizado en cualquier direccion (un stop realmente perdido seguia pareciendo "por encima", o un precio ya recuperado seguia pareciendo "perdido"). Corregido: comparar contra un stop YA ADOPTADO ya no necesita niveles nuevos (solo hacen falta para adoptar uno por primera vez); el metodo devuelve un `DTO\StopLossCheck` explicito (`DENTRO`/`CRUZADO`/`SIN_EVALUAR`) en vez de `void`, y el antiguo `isBelowActiveStop()` (la relectura separada que causaba la desactualizacion) se retira. `PositionDecisionAdvisor::decide()` ya no puede caer en `MANTENER` ("dentro de su stop-loss adoptado") cuando el estado es `SIN_EVALUAR`: nueva accion `REVISAR_STOP`, con la misma prioridad que `SALIR`. Cuatro casos de la reproduccion de Astra cubiertos con tests nuevos.
  ...
  - **A1 (conservacion y cobertura de EODHD): los dos bugs de guion corregidos, la campana de descarga real SIN ejecutar**. `bin/backfill-eodhd-fundamental-versions.php` comprobaba idempotencia con `hasVersion()` (solo "existe alguna captura"), ahora con `hasVersionWithHash()` (el hash exacto). `bin/archive-eodhd-fundamentals-v11.php` aplicaba `--max-tickers` antes de descartar los ya archivados, asi que repetir el mismo limite no avanzaba -- ahora el recorte opera sobre pendientes. Completar v1.1 para los 1.246 tickers que faltan cuesta ~12.460 unidades de cuota de EODHD (10 unidades/peticion): gasto real contra un servicio de pago, pendiente de que Francisco lo confirme antes de lanzarlo.
  ...
  - **Pendiente para una sesion futura**: resolver el caso 2/A6 (calendario de referencia compartido) y el limite de infraestructura del caso 5; completar la medicion de 636 tickers/10 años con todos los arreglos aplicados; ampliar el piloto del caso 6 al universo completo; construir la variante (B) del caso 3 de la auditoria anterior (observacion periodica, distinta del caso 3 de ESA auditoria); decidir con Francisco si lanzar la campana de descarga de A1 (~12.460 unidades de cuota); A5 (producto: procedencia/frescura fundamental en la ficha); revisar `EodhdFiscalPeriodProvider::totalDebt()` (mismo patron de "ausencia se convierte en 0.0" que A4, pero con una ambiguedad distinta -- EODHD normalmente si reporta explicitamente un componente en cero) y la procedencia de un resultado valido vacio en `earnings_events` (A2, exige cambio de esquema). La medicion del `2026-09-14` sigue invalidada.
