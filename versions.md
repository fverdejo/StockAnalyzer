...
- **Proximidad al maximo de 252 sesiones (~52 semanas) como señal de MOMENTUM (analista-mercado, 2026-08-23).** `TechnicalAnalyzer::analyze()` ya calcula `high52w`/`low52w` (`src/Analyzer/TechnicalAnalyzer.php:58-59`) pero solo como dato mostrado en la ficha (`StockDetailPage.php:148-149`, "Maximo/Minimo (periodo)"), nunca puntua. La literatura (George & Hwang 2004, *The 52-Week High and Momentum Investing*, JF) documenta que la distancia al maximo de 52 semanas predice el retorno futuro mejor que el momentum convencional y, a diferencia de este, sin reversion posterior — señal potencialmente complementaria, no redundante, al `momentum12m1` ya puntuado en `TechnicalScoreAnalyzer::momentum()`. Ojo con dos matices antes de tocar nada: (1) `high52w`/`low52w` hoy se calculan sobre el array de quotes completo que llegue (2 años en la web, distinto en `bin/backtest.php`), no sobre exactamente 252 sesiones — para esta señal concreta haria falta un calculo nuevo acotado a `array_slice($highs, -252)`, no reutilizar el campo existente tal cual. (2) Antes de sumarla a `ScoreWeights`/`MOMENTUM`, medir con `bin/backtest.php`/`runCrossSectional()` tanto el poder predictivo por decil como la correlacion con `momentum12m1` ya calculado — la investigacion del squeeze de Bollinger (`2026-08-22`) ya enseño que una señal con `t` fuerte puede descartarse igualmente por venir duplicada (`r=0,78` con `volatility20` en ese caso); aqui el riesgo de duplicidad es menor por construccion (una mide nivel vs maximo, la otra retorno acumulado) pero no esta medido.

- **Aviso de proximidad a resultados trimestrales junto a la recomendacion, no solo como dato aislado (analista-mercado, 2026-08-23).** `Application::renderDetail()` ya pide `$corporateEvents` (cacheado 24h, cero llamadas nuevas a Yahoo) y `StockDetailPage::renderCompanyOverview()` ya muestra "Proximos resultados" (`StockDetailPage.php:287-292`), pero como un dato mas de la cabecera, desconectado del badge BUY/SELL/HOLD y del texto de `RecommendationExplainer::explain()`. Las señales tecnicas de `TECHNICAL`/`MOMENTUM` (RSI, MACD, Bollinger, cruce de medias) asumen implicitamente continuidad de precio; un resultado trimestral es un catalizador binario que puede saltarse el stop-loss calculado (mismo riesgo de gap que `gestor-riesgo` señalo el `2026-08-22` para el presupuesto de riesgo del 1,5%, pero aqui el objetivo es informativo en el momento de la recomendacion, no recalcular el stop). Propuesta: cuando `corporateEvents?->getNextEarningsDate()` caiga dentro de una ventana corta (a definir, p.ej. 5 sesiones) Y la recomendacion sea BUY, `RecommendationExplainer::explain()` (nueva firma con `?CorporateEvents $corporateEvents = null`, o el propio `Application::renderDetail()` añadiendo una frase a la `Explanation` ya construida) añade una frase de cautela, sin tocar ninguna puntuacion. Verificar antes: (a) que cobertura real tiene `next_earnings_date` en los universos curados (algunos tickers pueden venir sin `calendarEvents` en Yahoo); (b) coordinar con `fiabilidad-datos-mercado` si el proveedor actual expone alguna vez fechas de resultados *pasadas* (no solo la proxima) — sin eso, la ventana de "5 sesiones" es una eleccion razonada por microestructura de mercado (PEAD/gap risk, hecho bien documentado), no algo que se pueda validar con el propio backtest de la app.
...
La de mayor prioridad recomendada para medir a continuacion:

- **Sustituir el cruce SMA20/50 por SMA50/SMA200 ("Golden Cross" clasico) — propuesta de `trader-tendencia`, sin medir todavia, prior alto.** Diagnostico de por que falla el actual: SMA20/50 (~1 y ~2,4 meses) cae en la ventana de **reversion a corto plazo** de Jegadeesh (1990), signo contrario al que premia el flag — coincide exactamente con la inversion documentada desde `v2.78`. SMA50/200 cae en la ventana de **continuacion** de 3-12 meses de Jegadeesh & Titman (1993), la misma zona donde `momentum12m1` (que SI funciona en el motor) ya tiene exito — el propio codigo demuestra internamente que la ventana temporal decide el signo, no el concepto de "seguir tendencia". Respaldo adicional: Brock/Lakonishok/LeBaron (1992) sobre 90 años del Dow. Cambio minimo si se valida: añadir `sma200` a `TechnicalAnalyzer`/`TechnicalSnapshot` (mismo patron que `sma20`/`sma50`) y cambiar la condicion en `TechnicalScoreAnalyzer.php:104-117` de `sma20>sma50` a `sma50>sma200`, mismos 4 puntos, mismo fallback — sin tocar `ScoreWeights`. No repite el error de las tres variantes fallidas del propio cruce 20/50 porque es una ventana temporal distinta, no una transformacion de la misma señal. Pendiente: medir con el protocolo de siempre (parche temporal, `runCrossSectional()`, 4-6 universos, varios horizontes, revision de `auditor-estadistico`) antes de tocar `config/weights.php`.

Resto de propuestas, sin medir:

- **Piotroski F-Score "lite" (tendencia de calidad a 5 años) — `inversor-fundamental`.** `FmpFiscalPeriodProvider` ya trae 5 ejercicios completos por ticker, pero `PointInTimeFundamentalsBuilder` solo usa el actual y el anterior. Contar cuantas de ~5-9 señales de mejora interanual (ΔROA, CFO>NetIncome, ΔApalancamiento, ΔLiquidez, sin dilucion, ΔMargen) se cumplen en la ventana de 5 años, dentro de `FundamentalAnalyzer::quality()`. A diferencia de los percentiles sectoriales (cerrados por `MIN_PEERS`), es intra-empresa: no sufre el limite de muestra por sector, medible ya con los 47 tickers confirmados, sin gastar cuota FMP nueva.
- **Ratio de acumulaciones contables (accrual ratio, Sloan 1996) — `inversor-fundamental`.** Requiere añadir `operatingCashFlow`/`totalAssets` a `FiscalPeriod` (ya vienen en las respuestas de FMP que se descargan, cero llamadas extra). Factor de calidad de beneficio distinto de ROE/margen/FCF-yield (ya probados y planos); si tampoco separa retornos, es confirmacion mas honesta que las tres pruebas anteriores, no razon para inventar una cuarta señal sin medir.
- **Distancia al maximo de 252 sesiones (momentum) y aviso de proximidad a resultados trimestrales — `analista-mercado`.** Ya detalladas en la seccion "Ideas adicionales sugeridas" de mas arriba, con archivo/metodo exacto y que verificar antes de medir.
- **Aviso de "riesgo de gap" en el badge de stop-loss, con cifra propia — `gestor-riesgo`.** Medido hoy con rigor (10 años, 6 universos): el 15,77% de las salidas por stop-loss abren ya por debajo del nivel calculado, con una perdida real un 22% peor que la teorica cuando ocurre — transversal a los 6 sectores, no concentrado en `healthcare` (de hecho `tech40` es el peor en drawdown gestionado, correccion a la lectura preliminar del `2026-08-22`). Un tooltip fijo en `RiskLevelsBadge`/`SuggestedPositionCalculator`, no un recalculo.
- **"Calor de cartera" (portfolio heat, Van Tharp): capar el riesgo abierto agregado, no solo por posicion — `gestor-riesgo`, territorio nuevo.** Hoy el 1,5% se calibra posicion a posicion; nada suma el riesgo total si varios stops saltan a la vez en una caida de mercado amplia (justo cuando los gaps son mas frecuentes, segun el hallazgo de arriba). Propuesta: `PortfolioHeatCalculator` nuevo (mismo patron que `PortfolioConcentrationCalculator`), umbral candidato 6% de la cartera (Van Tharp), a validar.
- **Registrar en vez de silenciar las excepciones de conexion de `CachedMarketDataProvider` — `fiabilidad-datos-mercado`.** Hoy `catch (\Throwable)` trata igual un cache-miss normal que una base de datos inalcanzable. Un aviso minimo (`fwrite(STDERR, ...)`) solo en la rama de excepcion real, sin cambiar el flujo. El proyecto no tiene infraestructura de logging (`grep` de `Psr\Log`/`Monolog`/`error_log` en `src/` no devuelve nada) — decidir el mecanismo antes de tocar los 8 puntos casi identicos de la clase.
- **Backoff/reintento ante 429 de Yahoo — `fiabilidad-datos-mercado`.** No existe ninguna logica de reintento en `HttpClient`; un 429 se propaga tal cual. Propuesta: reintento acotado (2-3 intentos, respetando `Retry-After` si Yahoo lo manda) en `HttpClient::get()`.
- **TTL corto para el intradia — `fiabilidad-datos-mercado`.** Hoy `getIntradayQuotes()` nunca cachea; un TTL de 60-120s reduciria exposicion a rate-limit en sesiones de investigacion con muchas visitas repetidas a la misma ficha, sin perder la semantica "casi en vivo".
- **Verificacion automatizada semanal de `config/universes.php` contra Yahoo — `fiabilidad-datos-mercado`.** Hoy la deteccion de tickers muertos/fusionados (como el caso `FDX` de arriba) depende de que un agente lo note a mano. Una tarea cron ligera en la Pi reutilizando `bin/analyze.php --universe=X` dejaria constancia de 404s reales.
- **Quitar el `font-weight: 800` al Score en las tablas de ranking — `diseno-usabilidad`.** La columna "Score" pesa visualmente igual que el ticker, compitiendo con la pildora de recomendacion (BUY/HOLD/SELL) que es la que de verdad resume la decision. Candidato: `.score-muted` en `Layout.php`.
- **Extraer el patron "+"/"-" a un helper unico, ya hecho arriba con `formatSignedMoney()` — `diseno-usabilidad`.** Implementado en esta misma entrada.
- **Verificar visualmente el formulario de compra/venta entre 641-920px — `diseno-usabilidad`.** `.trade-form` pasa a grid de 2 columnas en ese rango, pudiendo dejar "Precio" pegado solo al boton "Comprar". Deterministicamente cierto por CSS, pero el efecto visual real necesita confirmarse con la skill `run` antes de tocar nada.
