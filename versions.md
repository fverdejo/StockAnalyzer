...
### Universo dinamico alternativo (`undervalued_large_caps`): investigado, descartado

Antes de decidir `largecap60` se evaluo si un screener dinamico distinto de `general` podia servir como universo por defecto en vivo, en vez de uno curado a mano. Se verificaron uno a uno los screeners predefinidos de Yahoo (`most_actives`, `day_gainers`, `day_losers`, `undervalued_growth_stocks`, `growth_technology_stocks`, `undervalued_large_caps`, `aggressive_small_caps`, `small_cap_gainers`; los de fondos devuelven participaciones, inservibles aqui). `undervalued_large_caps` fue el unico cuya poblacion encajaba con lo que el motor mide: 1,10 de 12 ratios ausentes (frente a 3,25 de `general`), `RISK` medio 4,27/10, `QUALITY` 8,53/10, mediana de score 65,6 y 5 BUY de 20 (`most_actives`, la otra alternativa neutral en direccion, se quedaba en 2,50 ausentes y 0 BUY).

**Descartado por un motivo estructural, no de calidad de datos**: el screener filtra por PER y PEG bajos, que es parte de lo que ya puntua `FundamentalAnalyzer::valuation()` — usarlo como universo por defecto haria que el score re-premiara su propio filtro de entrada, midiendo en buena parte su propia seleccion. Ademas, el dia que se verifico, 4 de sus 5 primeras posiciones eran de petroleo y gas, justo el caso que `RankingSectorConcentrationCalculator` (`v2.75`) avisa. Ningun screener dinamico de Yahoo sustituye a un universo curado como defecto; `general` se conserva como universo seleccionable ("Movimientos de hoy"), no como pantalla de entrada.
...
## 2026-08-21 (segunda entrada) - Dos ideas obvias de TECHNICAL, medidas y descartadas

El usuario pide a `analista-mercado` pensar nuevas funcionalidades. Antes de proponer, se miden con datos reales (10 años, `largecap60`/`financials`/`healthcare`, horizonte 20, `--step=20` para fechas independientes, mismas clases de produccion) dos ideas obvias que nunca se habian probado:

- **Breakout de maximo de 52 semanas, como señal alcista clasica.** `TechnicalSnapshot::getHigh52w()`/`getLow52w()` ya existen y se muestran en la ficha, pero nunca puntuan. Medido: cerca del maximo (>=95% del rango de 52 semanas) rinde **peor** que la zona media en los 3 universos (t=-1,89 / -3,41 / -2,51); cerca del minimo rinde mejor pero con muestra pequeña (n=53-59) y t inconsistente (2,06/1,45/1,03). Mismo patron de reversion a corto plazo que ya documentaron `v2.76`/`v2.78` para el momentum de 30 dias. **No se añade**: repetiria el mismo error que el cruce de medias, no lo corregiria.
- **Filtrar el cruce SMA20/SMA50 por "fuerza de tendencia"** (spread SMA20-SMA50 como % del precio), como posible arreglo a la inversion ya conocida del cruce (`t` hasta -4,93, candidato a proxima prioridad en `roadmap.md`). Medido segmentando por magnitud del spread: el **death cross "ancho"** (spread < -2%) es el bucket con MEJOR retorno futuro de los cuatro en los 3 universos (2,3-2,7% frente a 0,5-1,1% del golden cross) — mas mean-reversion, no menos. **Descarta esta via de arreglo**; el hallazgo de `t=-4,93` sigue abierto y sin solucion conocida.

Ninguna de las dos entra en "Ideas adicionales sugeridas" porque no son propuestas, son evidencia en contra de dos ideas razonables que alguien podria proponer mas adelante — se documentan aqui para no volver a gastar tiempo midiendolas.
...
## 2026-08-21 (septima entrada) - BUY por umbral vs top-N por puesto: no se distinguen, y la anecdota de `v2.99` no generaliza

El usuario pide investigar ya la idea que quedo anotada en `v2.99`: ¿es mejor, para un usuario que sigue los badges de la app, seguir solo las señales BUY (umbral fijo >=75%, tamaño variable cada dia) en vez de las 10 primeras del ranking por puesto (lo que mide hoy `config/measured_edge.php`)?

Ventaja de partida: el score de produccion es solo-tecnico desde el `2026-08-15`, asi que esta medicion **no esta limitada a los ~47 tickers de la whitelist gratuita de FMP** — se pudo usar universos completos.

Metodologia: mismo muestreo walk-forward que `runCrossSectional()` (indice >=80, paso = horizonte), reutilizando `BacktestingService::collectSamples()` para no reimplementar la formula del score. Para cada fecha, dos selecciones sobre los mismos tickers/retornos: top-10 por puesto (la de siempre) y BUY por umbral (todo lo que puntue >=75% ese dia, sea cual sea el numero). Test pareado **solo en fechas con al menos 1 BUY** (en las fechas sin BUY no hay nada que comparar). 6 universos (`largecap60`, `ibex35`, `healthcare`, `industrials`, `energy`, `consumer`) x 5 horizontes (5/10/20/40/60 dias) = 30 combinaciones, 10 años de historico.

**Resultado: ninguna de las 30 combinaciones llega a |t|>=1,96**, salvo `energy` a 10 dias (t=2,07) — un unico caso entre 30 pruebas, exactamente lo esperable por azar al probar tantas combinaciones sin corregir. Diferencia media agregada (ponderada por fechas): **+0,04 pp**, dentro del ruido. **No hay evidencia de que seguir BUY por umbral sea mejor ni peor que el top-10 por puesto ya medido: las dos selecciones son estadisticamente indistinguibles entre si.**

**El hallazgo mas util no es ese, es este**: la premisa que motivo la pregunta ("solo 2 de las 10 primeras llevaban BUY") **no generaliza**. Medido de verdad, el tamaño medio del conjunto BUY cuando dispara es **15,3-15,9 en `largecap60`** (mayor que el top-10, no menor) y **6,5-8,8 en los universos sectoriales** (~40-50 tickers cada uno). La cobertura (fechas con al menos 1 BUY) es alta en general — 87-96% en la mayoria de universos, solo `energy` baja a 62-71% con un conjunto BUY tambien mas pequeño (3,6-4,4 de media). En la practica, BUY dispara casi todos los dias y con un tamaño de conjunto igual o mayor que el top-10: **es casi un superconjunto de el en `largecap60`**, por eso sus alphas no se distinguen — no son estrategias tan distintas como sugeria la vista puntual que motivo la pregunta. La captura de pantalla que motivo esto (2 BUY de 10) fue simplemente un dia con pocas señales, no el patron habitual.

**No se toca codigo.** La idea anotada en `v2.99` se retira de "Ideas adicionales sugeridas": queda cerrada con resultado nulo, no abierta a la espera de medicion.

Script de investigacion de un solo uso (no forma parte del repositorio, vive en el scratchpad de la sesion), sin tocar `src/`.
...
## 2026-08-22 (tercera entrada) - El equipo de cinco intenta mejorar el motor: tres investigaciones, cero cambios en `src/`

El usuario pide que, ahora que el equipo de mercado tiene cinco voces, intenten dar una solucion real al motor de analisis, y pregunta explicitamente si los fundamentales pueden reintroducirse ya que la cobertura crecio de 34 a 47 tickers. Se lanzan tres investigaciones en paralelo (una por especialista nuevo/existente), y despues `auditor-estadistico` audita el conjunto agregado — incluyendo re-ejecutar el codigo de medicion de las tres, no solo leer sus resumenes — antes de decidir. Ninguna de las tres sostiene un cambio de codigo.

**1. `gestor-riesgo` — ancho de Bandas de Bollinger (squeeze) como señal de RISK.** Hipotesis: un ancho de banda minimo ("squeeze") precede a mayor volatilidad realizada futura. Resultado: **refutada, y con el signo contrario** — volatilidad estrecha predice volatilidad futura tambien estrecha (persistencia/clustering, el hecho estilizado de Mandelbrot/ARCH bien documentado en la literatura), no una ruptura inminente. `t` entre 4,40 y 16,20 en las 8 combinaciones universo×horizonte, con **direccion monotonica identica en las 8** (Q1<Q2<Q3<Q4 sin excepcion) — la señal mas fuerte medida en todo el proyecto hasta ahora. Aun asi, **no se implementa**: correlaciona a r=0,78 (n=17.773) con `volatility20`, que ya pondera dentro de `RISK` — añadirla seria duplicar una señal que el score ya tiene, no una señal nueva. `auditor-estadistico` señala que el t-test agrupa observaciones de distintos tickers en la misma fecha sin el diseño pareado por fecha que usa el resto del proyecto (fechas compartidas no son independientes entre tickers del mismo universo), lo que puede inflar la magnitud del t-stat en regimenes de mercado amplio — pero no cambia la conclusion cualitativa (hay clustering real) ni el motivo de no implementar (redundancia, calculada aparte y no sujeta al mismo sesgo).

**2. `inversor-fundamental` — re-test de fundamentales con 47 tickers (respuesta directa a la pregunta del usuario).** Con un 38% mas de cobertura que la ultima medicion (34→47), ni la variante de pesos fijos (Graham) ni la de percentiles relativos (`RelativeFundamentalScorer`) mejoran: `t` maximo 0,63 en 10 combinaciones (5 horizontes × 2 variantes), evidencia **mas plana** que el hallazgo anterior con 34 tickers (`t` maximo 1,64). Recomienda mantener el score solo-tecnico. `auditor-estadistico` solo pudo re-ejecutar de forma independiente la variante de pesos fijos (el script de la variante relativa no quedo guardado en el scratchpad); no tiene motivo para dudar del resultado, pero el rastro de auditoria de esa mitad queda incompleto.

**3. `trader-tendencia` — señal continua de spread SMA20/SMA50, en vez del flag binario actual (la idea que quedaba abierta en "Ideas adicionales sugeridas" desde el `2026-08-21`).** Sustituir el flag 0/4 de `TechnicalScoreAnalyzer::technical()` por una rampa lineal sobre `(sma20-sma50)/precio` (parche de prueba en `TechnicalScoreAnalyzer.php:104-117`, revertido, nunca comprometido). Resultado: **0 de 20 combinaciones** (4 universos × 5 horizontes) con `|t pareado|>=1,96` frente al score actual; `t` maximo 1,48. Ademas, en **6 de 20** combinaciones la señal continua empeora la direccion respecto al score actual (cifra corregida por `auditor-estadistico` tras re-ejecutar `compare.php`; el resumen original decia 7), concentrado en `healthcare` (4 de 5 horizontes, incluido el ya significativo `t=-2,07` del binario que pasa a `t=-2,87`) e `ibex35` (5 y 20 dias). El hallazgo puntual del `2026-08-21` ("death cross ancho, mejor bucket en 3 universos") no se replica con la muestra mas amplia (10 años, step=horizonte). No queda formula lista para implementar; la idea se retira de "Ideas adicionales sugeridas" con resultado nulo.

**Auditoria agregada (`auditor-estadistico`, 38 comparaciones nominales entre las tres):** el patron es exactamente el esperable si dos hipotesis son ruido y una es señal real pero redundante — sin inconsistencia que deba alarmar. Ninguna de las tres cambio de metrica despues de ver el resultado (sin p-hacking de metrica). Punto aparte, no una investigacion nueva: `healthcare` sigue arrastrando la anomalia del cruce SMA20/50 sin resolver desde `v2.78` (`t` hasta -4,93), confirmada ahora **por tercera vez** con tres formas distintas de medir la misma variable subyacente (flag, invertido el `2026-08-21`, rampa continua hoy) — no es evidencia nueva que se acumule (es la misma señal transformada tres veces), pero si confirma que la anomalia es estable y sigue siendo, con diferencia, el hallazgo mas solido y menos atendido del proyecto.

**Veredicto: no se toca `src/` hoy.** Las tres conclusiones de "no implementar" aguantan el escrutinio, cada una por un motivo distinto (fundamentales: sin señal; SMA continua: sin señal y con daño asimetrico; Bollinger: con señal real pero redundante — el primer caso del proyecto donde "no implementar" no significa "no hay señal"). La pregunta del usuario sobre reintroducir fundamentales queda respondida: no, la cobertura ampliada no cambia el veredicto de `v2.15`/`2026-08-15`.

Scripts de las tres investigaciones (no comprometidos, viven en scratchpad de sesion): `bollinger_squeeze.php`, `bollinger_redundancy.php`, `run_combos.php`, `compare.php`, `investigate_47.php` (solo variante fija).

---

## 2026-08-22 (cuarta entrada) - ¿Analizar por sector? El equipo de cinco investiga, veredicto: no todavia

El usuario pregunta si tiene sentido analizar acciones de forma distinta segun el sector (una financiera no se rige por los mismos factores que una de salud), motivado por la anomalia SMA20/50 en `healthcare` documentada horas antes, y pide explicitamente que "lo tenga en cuenta el equipo completo". Se lanzan los cinco agentes en paralelo, cada uno desde su terreno, y la sintesis como juez es: **infraestructura para hacerlo existe, pero ningun hallazgo de hoy sostiene tocar `src/` todavia.**

**El hallazgo mas importante lo aporta `auditor-estadistico`, y obliga a corregir la narrativa de la entrada anterior.** El "t hasta -4,93" que se venia citando para `healthcare` viene de un diseño de medicion **sin parear por fecha** (`v2.78` y su replica del `2026-08-21`) — el mismo tipo de diseño que el propio equipo señalo como sospechoso de inflar el t-stat al investigar el squeeze de Bollinger horas antes (observaciones de distintos tickers en la misma fecha no son independientes entre si). El unico diseño metodologicamente correcto (pareado por fecha, `runCrossSectional()`) da **t=-2,07 (binario) y t=-2,87 (continuo)** — mas modesto, y sobre todo: no sobrevive la correccion por comparaciones multiples una vez se cuentan las ~38 pruebas ya gastadas en este mismo hilo desde el `2026-08-21` (umbral Bonferroni ≈3,2). Ademas, `healthcare` no se eligio al azar para investigarse mas — se eligio por ser el sector con el t mas extremo observado, lo que añade sesgo de seleccion post-hoc (winner's curse): el umbral para confiar en el deberia ser mas exigente que para un sector elegido de antemano, no igual.

**Piloto concreto de `trader-tendencia` (tratar el cruce SMA20/50 distinto solo en `healthcare`, dejando el resto del universo igual):** dos variantes probadas, parche temporal revertido, `runCrossSectional()` a 5 horizontes. **Excluir el cruce solo en `healthcare`**: sin direccion consistente (t maximo 0,42 en magnitud, menos del 40% de fechas mejoran en 4/5 horizontes) — descartada. **Invertirlo solo en `healthcare`**: mejora en direccion en 5 de 5 horizontes (mas limpio que el 15/18 del test global del `2026-08-21`), pero `t` maximo 0,75 y la mediana de la diferencia es 0,000 en 4 de 5 horizontes — la media positiva la arrastra un puñado de fechas extremas, no un efecto amplio. **No se adopta ninguna variante.**

**`inversor-fundamental`:** confirma que la critica de diseño es real (aplicar el mismo umbral de deuda/patrimonio a un banco que a una biotech es un error documentado en la propia literatura de Piotroski/Fama-French, no una intuicion sin respaldo), y que el prototipo `RelativeFundamentalScorer` ya "relativiza" pero mezclando todos los sectores en un unico grupo de comparacion — exactamente la dilucion que sospecha el usuario. Pero con ~4 tickers de media por sector en los 47 disponibles (por debajo del propio `MIN_PEERS=8` que el codigo exige), no hay muestra para medirlo ahora. Ajuste de diseño anotado para cuando haya mas cobertura (excluir deuda/patrimonio en `financials`, percentil dentro del sector en vez de contra el universo mezclado), sin tocar codigo hoy.

**`gestor-riesgo`:** no propone recalibrar el stop-loss por sector — el ATR14 ya se autoajusta por ticker (una biotech volatil ya tiene un ATR mayor en terminos absolutos), asi que un multiplicador distinto por sector seria redundante con lo que el modelo ya hace. Encuentra un problema real distinto: el presupuesto de riesgo del 1,5% asume que el stop se ejecuta limpio, y eso falla con gaps de apertura por catalizador binario (aprobacion FDA, resultado de ensayo clinico) — `simulateManagedExit()` ya modela ese hueco pero `RiskLevelsCalculator` no avisa de el. Lectura preliminar (2 años, muestras solapadas, cortada por rate-limit de Yahoo antes de completar 10 años): `healthcare` tiene el peor drawdown medio gestionado de 6 universos, pero `energy` esta casi empatado — sugiere que el fenomeno es "concentracion de eventos binarios", no "healthcare especificamente". Propone un aviso en la interfaz (no un recalculo) y medir si las perdidas catastroficas estan dominadas por el tope de riesgo (1,5%) o por el tope de peso maximo (20%) — sin completar por el rate-limit, queda para otra sesion.

**`analista-mercado`:** confirma que el sector SI esta disponible como dato real al puntuar (`Company::getSector()`, desde Yahoo, no solo agrupacion manual), pero `TechnicalScoreAnalyzer::technical()` lo descarta antes de llegar al calculo del cruce. Aun asi, recomienda **no** bifurcar codigo por sector ahora: el cruce SMA20/50 sale invertido en 6/6 universos sectoriales (no es un efecto especifico de `healthcare`), ya hubo dos intentos previos de sectorizar el lado fundamental (`v2.47`, `v2.51`) con efecto marginal, y esta es ya la cuarta transformacion probada hoy sobre la misma variable subyacente. Si se sigue esta via, la version barata con precedente arquitectonico es particionar `healthcare` en subgrupos (biotecnologia de catalizador binario, farma/dispositivos tradicionales, aseguradoras de salud, herramientas de ciencias de la vida) en `config/universes.php`, igual que ya se hizo con `financials`/`consumer` — cero codigo nuevo.

**Hallazgo colateral (`trader-tendencia`, territorio de `fiabilidad-datos-mercado`):** `CachedMarketDataProvider::getHistoricalQuotes()` traga en silencio la excepcion de conexion PDO cuando el driver `pdo_mysql` no esta disponible (ocurre con `php` de linea de comandos fuera de `ddev`) y cae directo al proveedor en vivo sin avisar — cada llamada de hoy fue una peticion real a Yahoo sin cache, lo que agoto el rate-limit (429) a mitad de la investigacion de `gestor-riesgo`. No es un bug de produccion conocido (la Pi corre con el driver disponible), pero silenciar una excepcion de conexion sin log es un patron a revisar. No investigado mas a fondo hoy.

**Veredicto final (Claude como juez):** no se toca `src/`. La pregunta del usuario queda respondida: analizar por sector es tecnicamente posible (el dato ya existe) pero ningun hallazgo de hoy lo justifica todavia — la anomalia de `healthcare` es mas debil de lo que parecia una vez corregida por el diseño de medicion y las comparaciones ya hechas, y las dos variantes concretas probadas no la resuelven. La prioridad, si se retoma este hilo, es la via barata (particionar `healthcare` en `config/universes.php`) con proteccion explicita contra sobreajuste (division calibracion/validacion, correccion por comparaciones multiples), no bifurcar `TechnicalScoreAnalyzer`.
...
Quedaba una sola idea abierta de las 11 originales, la unica genuinamente bloqueada por falta de datos (no por falta de decision), y `analista-mercado` añade una segunda el mismo `2026-08-21` tras revisar el motor a peticion del usuario:
...
Ambas ideas de arriba (squeeze de Bollinger en RISK, señal continua de SMA20/50) se midieron el `2026-08-22` (ver entrada "tercera") y se retiran de esta lista: la primera tiene señal real pero redundante con `volatility20` (no se implementa por duplicidad, no por falta de evidencia); la segunda no llega a significancia y ademas empeora `healthcare`/`ibex35`. Ninguna vuelve a esta lista sin repetir la medicion desde cero.

- **Homogeneizar `healthcare` en subgrupos antes de seguir tocando el cruce SMA20/50 (analista-mercado, 2026-08-22).** El cruce SMA20>SMA50 sale invertido y significativo en 6/6 universos sectoriales (no es un efecto especifico de `healthcare`), pero `healthcare` es el unico donde las tres transformaciones probadas hasta hoy (flag original, invertido el `2026-08-21`, rampa continua hoy) empeoran o se mantienen significativas — mismo patron que ya obligo a partir `financials` en `financials_banking`/`financials_insurance`/`financials_payments_asset_mgmt` y `consumer` en `consumer_discretionary`/`consumer_staples` (ver `config/universes.php`). `healthcare` (40 tickers) nunca se partio: mezcla biotecnologia de catalizador binario (MRNA, BIIB, VRTX, REGN, INCY, IDXX), farmaceuticas/dispositivos de tendencia mas tradicional (JNJ, ABT, MDT, SYK, BSX, ZBH), aseguradoras/gestoras de salud (UNH, ELV, CI, CNC, HUM, MOH, de comportamiento mas parecido a `financials_insurance` que al resto del sector) y herramientas de ciencias de la vida (TMO, DHR, A, MTD, WAT, IQV). Antes de considerar CUALQUIER rama de codigo por sector, medir con `bin/backtest.php` si el `t` del cruce SMA20/50 se concentra en el subgrupo de catalizador binario/biotecnia (donde un patron de tendencia tiene menos sentido de entrada, por diseño del negocio) o si se mantiene parejo en los cuatro subgrupos — igual que ya se hizo con `CurrentRatio` en `financials` (`v2.51`), donde partir el universo SI explico el sesgo sin tocar `FundamentalAnalyzer`. Aviso explicito de `auditor-estadistico` necesario antes de actuar: partir un universo ya pequeño (40 tickers) en 3-4 subgrupos deja muestras muy por debajo del umbral que el propio proyecto usa en otros sitios para fiarse de una medida relativa (`RelativeFundamentalScorer::MIN_PEERS = 8`), asi que cualquier hallazgo por subgrupo debe tratarse como exploratorio, no confirmatorio, hasta acumular mas historico.

Todas las ideas de mas arriba fuera de "tendencia del score" y "verificacion automatizada de universos" se cerraron o implementaron el `2026-08-25` (ver `v2.103`): 52 semanas y las dos señales fundamentales (Piotroski/Sloan) con resultado nulo; SMA50/SMA200, riesgo de gap, calor de cartera y aviso de resultados trimestrales, medidos o implementados segun el caso. "Verificacion automatizada de universos" tambien cerrada: cron instalado y probado en la Pi el `2026-08-25` (305/305 tickers OK).

Tres candidatas nuevas del `2026-08-27` (ver `v2.104`), ninguna con codigo tocado todavia — las tres necesitan medirse dentro del score real, no como factor aislado, antes de decidir:

- ~~**Bucket de spread SMA20/50 aislado** (`trader-tendencia`)~~ **Retirada sin medir el `2026-09-05` (octava entrada de ese dia)**: es la quinta transformacion de la misma variable subyacente (`sma20-sma50`) tras cuatro rondas ya nulas (72 comparaciones acumuladas, solo 1 nominal y en sentido adverso); su p≈0,0014 viene de cambiar de metrica (signos, no t pareado) despues de que la metrica estandar fallara cuatro veces, sobre 30 combinaciones que ademas comparan fechas/tickers muy correlacionados entre si (independencia dudosa, mismo problema ya corregido para el squeeze de Bollinger). Via cerrada por completo, ver esa entrada.
- ~~**Consistencia de tendencia "Frog in the Pan"** (`trader-tendencia`, Da/Gurun/Warachka 2014)~~ **Medida y cerrada el `2026-09-06` (quinta entrada de ese dia)**: con la metrica estandar del proyecto (t pareado por fecha), 0/12 combinaciones cruza Bonferroni (maximo |t|=1,86). 11/12 con signo positivo (reproduce la consistencia de signo original), pero la magnitud no es distinguible del ruido -- no autoriza medir dentro del score real.
- ~~**RSI(2) de Connors** (`analista-mercado`)~~ **Medida y cerrada el `2026-09-06`** (`versions.md`, entradas "segunda" y "cuarta" de ese dia): 1/24 combinaciones aisladas cruzo Bonferroni (`consumer`/horizonte 5/oversold), autorizado un test adicional dentro del score real que tambien salio nulo (t=0,16). 25 pruebas acumuladas sobre la idea, 0 dentro del score real. No se implementa.
...
## 2026-08-23 - Relleno de fundamentales: dos tandas, sin ninguno nuevo, 47 confirmados se mantiene

Continuando la estrategia de reintento vigente desde el `2026-08-22` (ya no hay candidatos "nunca probados": cualquier tanda reintenta algo bloqueado antes, apostando a que el subconjunto accesible del plan gratuito de FMP cambie de un dia a otro). Dos tandas: `--all-universes --skip-existing --max-tickers=80` (144 llamadas) seguida de una tanda explicita de los siguientes 30 candidatos en el orden de `config/universes.php` (81 llamadas) — **~225 de 250 llamadas diarias**, parado ahi para no arriesgar el cupo. **0 tickers nuevos**: los 48+27=75 candidatos con error fueron todos HTTP 402 (no cubiertos por el plan), incluyendo el bloque completo de tickers `.MC` de IBEX probados hoy (19 de 19 con 402, sin excepcion). Se mantiene en **47 tickers confirmados**.

**Aclaracion sobre el conteo**: al comprobar el estado real de `fundamentals_history` para preparar el lote de hoy, una consulta directa (`SELECT DISTINCT ticker`) devolvia 72 tickers, no 47 — parecia una discrepancia con lo documentado. Investigado: 25 de esos 72 tienen **1-2 filas de historico**, muy por debajo del umbral que el propio `bin/backfill-fundamentals.php` usa para considerar un ticker relleno (`FundamentalsHistoryRepository::countSnapshots($ticker) > 5`, linea 149 del script) — son restos de intentos parciales de sesiones anteriores (`APA`, `BIIB`, `MRK`, `ORCL`, `TRV`, los `.MC` de `AMS`/`BBVA`/`ELE`/`ITX`/`MTS`/`PUIG`/`REP`, entre otros), no relleno confirmado. Con el mismo criterio que usa el script (`>5` snapshots), el conteo real es exactamente **47**, igual que el `2026-08-22`: no hay drift, la discrepancia era del metodo de conteo ad-hoc, no del dato. Documentado aqui para no repetir la confusion en otra sesion: para verificar el conteo real de confirmados, usar `countSnapshots() > 5`, no `SELECT DISTINCT ticker`.

Scripts de un solo uso usados hoy para calcular el lote (no comprometidos, `tmp_list_candidates.php`/`tmp_next_batch.php`/`tmp_count_confirmed.php`, creados y borrados en el mismo turno). Sin datos nuevos que sincronizar con la Pi.

---

## v2.101 - El equipo completo cierra las ideas pendientes y propone mejoras; dos arreglos reales de accesibilidad

El usuario pide finalizar la lista de "Ideas adicionales sugeridas" y que los distintos expertos valoren mejoras para la aplicacion. Se lanzan los siete agentes con permiso de proponer (los cinco de mercado, `fiabilidad-datos-mercado`, `diseno-usabilidad`, `qa-tests`) en paralelo: dos cierran ideas pendientes, todos aportan propuestas nuevas, y dos encontraron problemas reales que se arreglan hoy mismo.

### Ideas pendientes cerradas

- **Tendencia del score (re-rating): sigue bloqueada, con una sorpresa.** `score_history` tiene HOY menos cobertura que el `2026-08-21` (60 filas/40 tickers/8 dias, frente a 431/69/7 de entonces) pese a que la app se ha usado mucho estos dos dias. Investigado: `AUTO_INCREMENT` de la tabla esta en 98, muy por debajo de lo que 431 filas historicas implicarian — la base de datos local de `ddev` se reinicio/recreo en algun momento entre el `2026-08-21` y hoy (no es un bug de codigo: la clave unica `(ticker, snapshot_date)` es correcta y `ON DUPLICATE KEY UPDATE` no explica una perdida de filas). No afecta a datos transaccionales (cartera/transacciones, en tablas distintas), solo al historico organico de `score_history`, que vuelve a acumular desde cero. Sigue bloqueada por el mismo criterio de siempre (cobertura por ticker en un universo curado), ahora mas lejos que nunca.
- **Subgrupos de `healthcare` para el cruce SMA20/50: cerrado, resultado nulo — y la hipotesis de partida iba al reves.** Medido con el diseño validado (`runCrossSectional()`, parche temporal revertido, 4 subgrupos × 3 horizontes = 12 comparaciones del efecto de invertir): **0 de 12 llega a |t|>=1,96** (maximo 1,51). El subgrupo que se sospechaba mas afectado (biotecnologia de catalizador binario) es el UNICO con signo positivo y creciente con el horizonte; el patron negativo mas consistente aparece en aseguradoras de salud, el subgrupo que menos se esperaba investigar. Con n=6 tickers por subgrupo (por debajo de `RelativeFundamentalScorer::MIN_PEERS=8`) y ~74 comparaciones acumuladas en este hilo desde el `2026-08-21`, ningun resultado de hoy sobrevive correccion. Se cierra la particion sectorial fina de este cruce: van cuatro transformaciones distintas de la misma variable (flag, inversion, rampa continua, particion sectorial), las cuatro sin resultado.

### Arreglos reales, implementados hoy

- **Version desactualizada en la cabecera de cada pagina** (`DashboardPage::APP_VERSION`): decia `v2.94` cuando el proyecto va por esta misma entrada. Corregido a `v2.101`.
- **El signo "+" no se mostraba en ganancias, solo el "-" en perdidas (accesibilidad, WCAG 1.4.1).** `Layout::formatMoney()` usa `number_format()`, que nunca antepone `+` a un valor positivo — en la columna "Beneficio" de `PortfolioPage` y la caja "Beneficio latente" de `StockDetailPage`, una ganancia y una perdida se distinguian en la practica solo por el color de `.profit-positive`/`.profit-negative`, ilegible para quien tiene deficiencia de percepcion rojo-verde. Nuevo helper `Layout::formatSignedMoney()` (antepone `+` cuando `$value > 0`, deja intacto el `-` que `number_format()` ya pone), usado en `PortfolioPage::nullableProfitMoney()` y `StockDetailPage::renderPositionPanel()`. No se toco `PortfolioCsvExporter` (el CSV no tiene color, no hay problema de accesibilidad ahi, solo asimetria estetica menor — fuera de alcance de este arreglo).
- **Cobertura de tests reforzada** (`qa-tests`): `PortfolioService::buildEurAccounting()` (el calculo de beneficio realizado en euros, con tipo de cambio historico) replicaba la misma logica de coste medio que fallo en `v2.97`, sin ningun test que la ejercitara de verdad — nuevo `tests/Services/PortfolioServiceRealizedProfitEurTest.php`. El cableado de conversion de divisa en `Application::resolveTradeQuantity()` (`v2.96`) tampoco tenia test — nuevo `tests/Services/ApplicationTradeQuantityTest.php`. Mas `tests/Web/LayoutFormatSignedMoneyTest.php` para el arreglo de arriba.

Verificado: `ddev exec vendor/bin/phpunit`: **335 tests, 984 assertions, OK (1 skipped a proposito)**. `ddev exec vendor/bin/phpstan analyse`: **No errors** (se corrigio de paso un error de tipos en el test nuevo de `qa-tests`: el proveedor de mercado de prueba devolvia `Quote` en vez de `HistoricalQuote` en `getHistoricalQuotes()`).

### Nota documental sin riesgo (`fiabilidad-datos-mercado`)

Comentario añadido en `config/universes.php` sobre el spin-off de FedEx Freight (`FDXF`, 2026-06-01, 1 accion nueva por cada 2 de `FDX`, sin contrasplit) para que un salto de precio en la serie de `FDX` alrededor de esa fecha no se confunda con un bug de datos. No cambia ningun ticker.

### Propuestas nuevas para "Ideas adicionales sugeridas"

Todas se cerraron o implementaron el `2026-08-25` (ver `v2.103`): SMA50/SMA200 con resultado nulo/inconcluso; Piotroski F-Score y ratio de acumulaciones de Sloan, nulos; riesgo de gap y calor de cartera, implementados; logging/backoff/TTL, implementados en `v2.102`; el score de tablas y el formulario de compra/venta en tablet, implementados en `v2.103`. Solo queda pendiente de instalar en la Pi (sin acceso SSH desde los agentes) `bin/verify-universes.php` (`v2.102`).

Sin cambios en `config/weights.php` ni en ninguna señal de puntuacion existente. Sincronizado con la Pi.

---

## v2.102 - Tres ideas de fiabilidad de datos implementadas, cuarta preparada sin instalar

Estado: implementado y verificado en ddev (ideas 1-3); idea 4 con script listo, sin instalar en la Pi (fuera del alcance de este agente, sin acceso SSH).

Objetivo:

Cerrar tres de las cuatro ideas de bajo riesgo que `fiabilidad-datos-mercado` anoto en `v2.101` ("Ideas adicionales sugeridas"): registrar las excepciones reales de `CachedMarketDataProvider` en vez de silenciarlas, reintento acotado ante 429 en `HttpClient`, y TTL corto para el intradia. Ninguna toca señales de puntuacion ni `config/weights.php`. La cuarta (verificacion semanal de `config/universes.php` contra Yahoo) implica una tarea programada en la Pi: se deja el script listo y la unidad systemd documentada, sin instalarla.

Decisiones de arquitectura:

- **`CachedMarketDataProvider`: los `catch (\Throwable)` existentes ya envuelven solo al repositorio de cache (PDO/JSON), nunca al proveedor externo.** Un cache-miss normal (fila ausente o caducada) ya se resuelve devolviendo `null`/`[]` sin lanzar nada (`MarketDataCacheRepository::isFresh()`), asi que llegar a cualquiera de esos `catch` ya significa un fallo real (PDO caido, fila corrupta, error de serializacion JSON al guardar) — no hacia falta "distinguir" el cache-miss con una condicion nueva, ya estaba distinguido por construccion. Se añade un helper privado `logCacheFailure()` que hace `fwrite(STDERR, ...)`, llamado desde los 6 puntos de la clase (no 8: se conto de nuevo al editar). Confirmado antes de elegir el mecanismo que el proyecto no tiene infraestructura de logging propia: `monolog/monolog` esta en `composer.json` como dependencia transitiva sin ningun uso directo (`grep` de `Monolog`/`Logger`/`Psr\Log` en `src/` no devuelve nada), y el patron ya establecido en `bin/*.php` para avisos es `fwrite(STDERR, ...)`. El flujo de control no cambia: se sigue cayendo al dato en vivo exactamente igual que antes.
- **`HttpClient::get()`: reintento acotado a 3 intentos totales (1 inicial + 2 reintentos), solo ante 429.** Respeta el header `Retry-After` si Yahoo lo manda; si no, backoff exponencial corto (1s, 2s). Cualquier otro codigo (402, 404...) o error de red sigue propagandose tal cual en el primer intento, sin reintento — son "no cubierto"/"no existe", no un problema temporal. El constructor gana `?HandlerStack $handlerStack = null` y `?callable $sleeper = null`, los dos solo para poder sustituir la conexion real (`MockHandler` de Guzzle) y la espera real (un sleeper que solo registra) en tests: ninguna llamada de produccion los pasa, `new HttpClient()` sigue funcionando igual en los 5 proveedores que lo usan como valor por defecto.
- **`CachedMarketDataProvider::getIntradayQuotes()` deja de pedir siempre en vivo.** Reutiliza el mismo mecanismo que el historico diario (`MarketDataCacheRepository::findHistory()`/`saveHistory()`), con el intervalo como parte de la clave de rango (`"intraday_{interval}"`, p.ej. `intraday_5m`) para no colisionar ni con las claves de historico diario (`'6mo'`, `'1y'`, `'2y'`...) ni entre intervalos intradia distintos. TTL propio de 90s por defecto (dentro del rango 60-120s pedido) via un parametro nuevo del constructor (`$intradayTtl`), con un getter `getIntradayTtl()` analogo al `getHistoryTtl()` que ya existia, expuesto por el mismo motivo: poder verificarlo en un test sin espiar la peticion. El comentario de `Application::renderIntraday()` que decia "no pasa por CachedMarketDataProvider (los intervalos intradia no se cachean)" estaba a punto de quedar desactualizado por este cambio — se corrige de paso.
- **`bin/verify-universes.php` (idea 4, sin instalar) pide solo `getHistoricalQuotes()`, no el pipeline completo de `bin/analyze.php`.** Evita el endpoint de fundamentales (`quoteSummary`), que ya es "mas fragil" por diseño (`YahooFinanceProvider::fetchFundamentalsAndProfileSafely()`) y cuyo fallo no dice nada sobre si el ticker sigue existiendo. Deliberadamente no usa `CachedMarketDataProvider` ni toca la base de datos (ni lee `market_history_cache` ni escribe en `daily_rankings`): es una comprobacion de solo lectura contra Yahoo en vivo, y una cache de 1 dia podria esconder un 404 de hoy hasta mañana. Cada ticker unico de `config/universes.php` se comprueba una sola vez aunque aparezca en varios universos (305 unicos frente a 540 entradas repetidas, medido hoy), agrupando los universos a los que pertenece cada ticker para el reporte de fallos, en vez de pedir lo mismo varias veces a Yahoo. Clasifica cada fallo por el texto del mensaje en `NOT-FOUND` (contiene `404`/`No data found`/`delisted` — el patron real observado para `DFS`/`HES`/`MRO` y reproducido hoy con un ticker inventado), `RATE-LIMITED` (contiene `429`/`Too Many Requests`, no cuenta como ticker roto: `HttpClient` ya reintento antes de que este script vea el fallo) y `OTHER`. Exit code 1 solo si hay `NOT-FOUND` u `OTHER` (los 429 en solitario no hacen fallar la ejecucion). Pausa configurable entre tickers (`--sleep-ms`, 300ms por defecto) para no lanzar 305 peticiones seguidas sin margen.

Incluye:

- `src/Providers/CachedMarketDataProvider.php`: `logCacheFailure()` nuevo (privado, estatico); los 6 `catch (\Throwable)` existentes lo llaman; parametro de constructor `$intradayTtl` (`PT90S` por defecto) + getter `getIntradayTtl()`; `getIntradayQuotes()` reescrito para cachear con TTL corto.
- `src/Infrastructure/Http/HttpClient.php`: reintento/backoff en `get()` (constantes `MAX_ATTEMPTS`/`RETRY_STATUS_CODE`/`BASE_BACKOFF_SECONDS`, metodo privado `retryDelaySeconds()`); constructor con `$handlerStack`/`$sleeper` opcionales, ambos solo para tests.
- `src/Services/Application.php`: comentario de `renderIntraday()` actualizado para reflejar que ahora si pasa por `CachedMarketDataProvider` (con TTL corto propio).
- `bin/verify-universes.php` (nuevo, **no instalado**): ver decisiones de arquitectura arriba.
- `tests/Providers/CachedMarketDataProviderIntradayTest.php` (nuevo): TTL de intradia por defecto (90s), un hit de cache no llama al proveedor externo, un miss llama al proveedor y guarda con la clave `intraday_{interval}` correcta, un TTL explicito manda sobre el valor por defecto.
- `tests/Infrastructure/Http/HttpClientRetryTest.php` (nuevo): un 429 seguido de un 200 se resuelve con reintento, se respeta `Retry-After` en vez del backoff exponencial, sin ese header se usa el backoff exponencial corto (1s, 2s), un 429 persistente agota los 3 intentos y propaga la excepcion, un 404 no reintenta y se propaga en el primer intento.

Verificado en ddev con...:

`php -l` sin errores en los 5 ficheros de `src/`/`bin/` tocados o creados. `ddev exec vendor/bin/phpunit`: **344 tests, 1005 assertions, OK** (1 skipped a proposito, el mismo de siempre — 9 tests/21 assertions nuevos entre los dos ficheros de test). `ddev exec vendor/bin/phpstan analyse` (nivel 5, incluye `bin/`): **No errors**. Prueba manual contra Yahoo real (sin mock, red saliente disponible en esta sesion) con `AAPL`/`MSFT`/`GOOGL` (500 velas cada uno, `OK`) y un ticker inventado (`FAKETICKERXYZ`): el error real devuelto por Yahoo es `` `404 Not Found` `` con cuerpo `{"chart":{"result":null,"error":{"code":"Not Found","description":"No data found, symbol may be delisted"}}}`, confirmando que la clasificacion de `bin/verify-universes.php` (busca `404`/`No data found`) detecta exactamente el caso real que viene a cubrir. No se ejecuto `bin/verify-universes.php` contra el universo completo (305 tickers unicos) en esta sesion, para no gastar cuota de Yahoo sin necesidad real dentro de una verificacion de ideas 1-3; la logica de clasificacion se valido de forma equivalente con ese subconjunto pequeño usando el mismo metodo (`getHistoricalQuotes()`) que usa el script.

Idea 4 — script listo, instalacion pendiente de aprobacion del usuario (fuera del alcance de este agente, sin acceso SSH a la Pi):

Unidades systemd propuestas, mismo patron que `stockanalyzer-analyze.timer`/`stockanalyzer-backup.timer` (usuario `admin`, `/var/www/StockAnalyzer`, PHP 8.4, `Persistent=true` porque la Pi no esta encendida 24h) pero sin `mariadb.service` en `After=` (el script no toca la base de datos):

`/etc/systemd/system/stockanalyzer-verify-universes.service`:
```
[Unit]
Description=StockAnalyzer - verificacion semanal de tickers en config/universes.php
...
Type=oneshot
User=admin
WorkingDirectory=/var/www/StockAnalyzer
ExecStart=/usr/bin/php8.4 bin/verify-universes.php
```

`/etc/systemd/system/stockanalyzer-verify-universes.timer`:
```
[Unit]
Description=Lanza stockanalyzer-verify-universes.service una vez por semana
...
Instalacion (a ejecutar en la Pi, no hecha en esta sesion):
```
sudo systemctl daemon-reload
sudo systemctl enable --now stockanalyzer-verify-universes.timer
systemctl list-timers | grep stockanalyzer
```

Revision del resultado (semanal, a mano via SSH): `journalctl -u stockanalyzer-verify-universes.service -n 200 --no-pager`. Un `code=exited, status=1` en `systemctl status stockanalyzer-verify-universes.service` marca que hubo al menos un `NOT-FOUND`/`OTHER` que revisar a mano (posible ticker delistado/fusionado, mismo criterio que `DFS`->`COF`/`HES`->`CVX`/`MRO`->`COP`); un `status=0` con `RATE-LIMITED` en el log es ruido esperado de Yahoo, no una accion pendiente.
...
Misma estrategia de reintento de las ultimas sesiones: `--all-universes --skip-existing --max-tickers=80` (144 llamadas) mas una tanda explicita de los siguientes 30 candidatos en el orden de `config/universes.php` (81 llamadas) — ~225 de 250 llamadas diarias. **0 tickers nuevos**, los 75 candidatos con error fueron todos HTTP 402, incluyendo de nuevo el bloque completo de IBEX (`.MC`). Se mantiene en **47 tickers confirmados** (verificado con `countSnapshots() > 5`, el criterio correcto aclarado el `2026-08-23`). Van ya tres dias seguidos sin ningun ticker nuevo — el subconjunto accesible del plan gratuito de FMP parece haberse estabilizado, aunque no hay forma de saber si es permanente sin seguir probando.

---

## 2026-08-27 - Relleno de fundamentales: dos tandas, sin ninguno nuevo, 47 confirmados se mantiene

Con la Raspberry Pi y `ddev` activos, misma estrategia de reintento: `--all-universes --skip-existing --max-tickers=80` (144 llamadas) seguida de una tanda explicita de los siguientes 30 candidatos en el orden de `config/universes.php` (90 llamadas) — 234 de 250 llamadas diarias. **0 tickers nuevos**: los 48+30=78 candidatos con error fueron todos HTTP 402, incluyendo de nuevo el bloque completo de IBEX (`.MC`). Se mantiene en **47 tickers confirmados** (verificado con `countSnapshots() > 5`). Cuarta sesion seguida sin ningun ticker nuevo (`2026-08-22`, `2026-08-23`, `2026-08-25`, hoy). Sin datos nuevos que sincronizar con la Pi.

Scripts de un solo uso usados hoy para calcular el lote (`tmp_count_confirmed.php`, `tmp_list_candidates.php`), creados y borrados en el mismo turno.

---

## v2.103 - El equipo cierra cuatro mediciones mas y se implementan cinco mejoras reales

El usuario pide "realizar" las ideas pendientes en "Ideas adicionales sugeridas". Se lanzan cinco agentes en paralelo: cuatro miden ideas que afectarian al motor de puntuacion (con el mismo rigor de siempre, sin saltarselo pese al "realizadlo ya" — la prioridad de fondo del proyecto sigue siendo que el motor prediga bien, verificado con datos) y uno (`fiabilidad-datos-mercado`, ya con permiso de escritura) implementa directamente sus cuatro ideas de bajo riesgo (ver `v2.102`, mismo turno). Las cuatro mediciones de señal salen nulas o inconcluyentes; las tres ideas informativas (no tocan ninguna puntuacion) se implementan.

### Cuatro mediciones, cuatro cierres

- **SMA50/SMA200 ("Golden Cross" clasico) en vez de SMA20/50 — la propuesta de mayor prioridad, cerrada con resultado nulo/inconcluso.** 24 combinaciones (6 universos × 4 horizontes), parche temporal revertido y verificado byte a byte. Solo 2/24 nominalmente significativas (esperable por azar), 0/24 tras correccion Bonferroni. Direccion positiva debil en 17/24 (71%, sign test p=0,064, no significativo) y la anomalia de `healthcare` queda plana en vez de invertida (t entre -0,44 y +0,42) sin demostrar revertirla con fuerza. Dato adicional que pesa en contra: SMA50/200 genera **3,8x menos señales** que SMA20/50 (1,56 vs 5,95 cruces/ticker/año), aportando poco poder discriminante dia a dia en un ranking transversal aunque el t hubiera sido bueno. No se implementa; van ya cuatro transformaciones distintas del mismo cruce (flag, inversion, rampa continua, particion sectorial, y ahora ventana) sin resultado.
- **Piotroski F-Score completo (no solo "lite") — cerrado con resultado nulo.** `inversor-fundamental` diseño la medicion mas favorable posible a su propia tesis (horizonte hasta 2 años, retorno en exceso sobre SPY para no confundir con el rally de mercado, señales intra-empresa que evitan el problema de `MIN_PEERS` por sector) sobre 188 transiciones interanuales de los 47 tickers confirmados. Resultado: `|t|` maximo 1,16 en 5 horizontes, signo que se **invierte** a partir de 1 año — justo donde la tesis value/quality deberia ser mas fuerte. No se implementa.
- **Ratio de acumulaciones contables (Sloan 1996) — cerrado con resultado nulo.** Mismo diseño y misma muestra que el F-Score. `|t|` maximo 0,76, sin direccion consistente entre horizontes (coincide con Sloan a 20 y 252 dias, lo contradice a 60 y 120). No se implementa.
- **Distancia al maximo de 252 sesiones como señal de MOMENTUM — cerrado con resultado nulo.** `analista-mercado` midio la ventana exacta de 252 sesiones (no el campo `high52w` existente, que usa el rango de quotes que llegue) en 28 combinaciones (7 universos × 4 horizontes). 0/28 con el signo que predice la hipotesis (George & Hwang 2004); el unico caso significativo (`healthcare` a 20 dias, t=-2,10) va al reves y es la misma anomalia de reversion ya conocida de ese universo, no un hallazgo nuevo. Correlacion con `momentum12m1` moderada (r≈0,39-0,47) pero irrelevante al no haber efecto que proteger de duplicidad. No se implementa.

### Tres ideas informativas, implementadas (ninguna toca `ScoreWeights` ni ninguna recomendacion)

- **`EarningsProximityNotice`** (`src/Web/EarningsProximityNotice.php`, nuevo): aviso en la ficha de detalle cuando una recomendacion BUY cae a 7 dias naturales o menos de resultados trimestrales (misma ventana que `AlertService::checkUpcomingEarnings()`, para no mantener dos magic numbers distintos para el mismo aviso). `analista-mercado` verifico cobertura real de `next_earnings_date` contra Yahoo en vivo: **100% en `largecap60` (60/60)**, muy por encima del umbral minimo que se habia pedido (60-70%). Conectado en `StockDetailPage::render()`, junto al aviso de ventaja medida — cero llamadas nuevas, `$corporateEvents` ya se pedia.
- **Aviso de riesgo de gap en el stop-loss** (`src/Web/RiskLevelsBadge.php`, `src/Web/StockDetailPage.php`): tooltip fijo con la cifra ya medida por `gestor-riesgo` el `2026-08-23` (10 años, 6 sectores): el 15,77% de las salidas por stop-loss abren el mercado ya por debajo del nivel calculado, con una perdida real un 22% peor que la teorica cuando ocurre. Texto fijo sin variacion por sector (el riesgo es transversal, no concentrado en uno). Sin recalculo de ningun nivel.
- **`PortfolioHeatCalculator`** (`src/Services/PortfolioHeatCalculator.php`, `src/DTO/PortfolioHeat.php`, nuevo panel "Calor de cartera" en `PortfolioPage`): suma el riesgo abierto de TODAS las posiciones a la vez (cuanto se perderia si todos los stop-loss saltaran juntos), algo que `RiskLevelsCalculator` (posicion a posicion) y `PortfolioConcentrationCalculator` (composicion) no responden ninguno de los dos por separado. Umbral de aviso **15%**, no el 6% de referencia de Van Tharp que se habia propuesto: `gestor-riesgo` midio con datos de mercado reales de hoy que una cartera `largecap60` normal de 10-15 posiciones ya ronda 6,5%-7,6% de calor igual-ponderado (hasta 12,92% en sectores volatiles como semiconductores), asi que 6% habria disparado el aviso casi siempre. 15% queda por encima de toda cartera diversificada medida y por debajo de lo que sale de concentrar el riesgo siguiendo al pie de la letra el propio dimensionador de la app (~22%). Una posicion sin `RiskLevels`/precio/tipo de cambio se excluye de la suma en vez de invalidar el calculo entero (a diferencia de `PortfolioConcentration`): un calor parcial sigue siendo una cota inferior util, se indica en el panel cuando aplica.

### Dos arreglos de diseño, implementados directamente (sin agente, ideas ya cerradas de `diseno-usabilidad` el `2026-08-23`)

- **El Score de las tablas de ranking ya no compite en peso visual con el ticker.** `.score` en `Layout.php` pasa de `font-weight: 800` (igual que `.ticker`) a `font-weight: 400; color: var(--muted)`, para que la pildora de recomendacion (BUY/HOLD/SELL) sea lo que destaca en un vistazo rapido, no dos columnas en negro-negrita compitiendo por la atencion.
- **El formulario de compra/venta ya no arriesga dejar "Precio" pegado al boton "Comprar" en tablets (641-920px).** `.trade-form` pasaba a un grid de 2 columnas en ese rango; con 5 hijos directos (Cantidad, Importe, Precio, boton Comprar, boton Vender) el auto-placement dejaba Precio en la misma fila que Comprar. Colapsado a 1 columna en ese breakpoint, mismo patron que ya se usaba por debajo de 640px. No se pudo verificar visualmente (no hay `chromium-cli`/Playwright/Puppeteer disponibles en este entorno), pero el analisis CSS es deterministico y el arreglo es de bajo riesgo (reutiliza un patron ya probado).

Verificado: `ddev exec vendor/bin/phpunit`: **362 tests, 1042 assertions, OK (1 skipped a proposito)**. `ddev exec vendor/bin/phpstan analyse`: **No errors**. Tests nuevos: `tests/Web/EarningsProximityNoticeTest.php`, `tests/Web/RiskLevelsBadgeTest.php`, `tests/Services/PortfolioHeatCalculatorTest.php`, mas casos nuevos en `tests/Web/PortfolioPageTest.php` para el panel de calor.

Sin cambios en `config/weights.php` ni en ninguna señal de puntuacion existente. Sincronizado con la Pi.

---

## 2026-08-25 (segunda entrada) - Primer modelo multivariante del proyecto: tambien nulo

Tras mas de 20 investigaciones señal-por-señal con resultado mayormente nulo, el usuario pregunta que haria falta para que el motor mejore de verdad y propone la via que nunca se habia probado: combinar varias señales en un modelo multivariante real (regresion, no una suma de umbrales fijos), reintroduciendo tambien los datos fundamentales como variables candidatas.

**Paso 1, `auditor-estadistico` diseña el protocolo antes de ajustar nada** (primera vez que el proyecto intenta esto, riesgo de sobreajuste mucho mayor que en un test de una sola señal): verifico contra codigo y datos reales que los fundamentales **no son viables todavia** para este uso — no por la cobertura de 47 tickers ya conocida, sino porque `FmpFiscalPeriodProvider::PERIOD='annual'` (plan gratuito de FMP) da solo **~5 valores distintos por ticker en 5 años** por variable fundamental, insuficiente para ningun split entrenamiento/validacion/test serio por muy bien regularizado que este el modelo. Para la parte tecnica: 121 fechas independientes (`largecap60`, 10 años, horizon=20), suficientes para 6-8 variables con ridge regression (no lasso, por la alta colinealidad entre señales tecnicas) y un split cronologico 60/20/20, evaluando el bloque de test una sola vez.

**Paso 2, `analista-mercado` ejecuta el protocolo exacto**: 8 variables tecnicas reducidas (fuera las ya contaminadas o redundantes: cruce SMA20/50 binario, momentum30, distancia a 52 semanas, ATR14%), ridge con solucion cerrada por eliminacion de Gauss-Jordan escrita en PHP, **verificada contra dos casos de solucion analitica conocida antes de tocar datos reales** (coincide hasta el error de redondeo de punto flotante). El requisito de `momentum12m1` (~250 sesiones de historia previa) reduce las fechas usables de 121 a 112 — desviacion real y explicada, no un error silencioso. Split cronologico 67/22/23 fechas.

**Resultado en el bloque de test (23 fechas, evaluado una sola vez, sin repetir con otro λ):** el modelo del ranking multivariante saca +0,447pp de alpha media mas que el score de produccion en esas mismas fechas, pero con **t pareado = 0,641** — muy lejos de cualquier umbral de significancia, indistinguible de ruido. El unico coeficiente fiable (las demas señales de tendencia estan correlacionadas 0,45-0,85 entre si, ridge reparte el peso entre ellas de forma no interpretable individualmente) es `momentum12m1`, con signo positivo — la misma señal que el motor ya sabia que funciona, sin aportar nada nuevo en combinacion.

**Veredicto: la combinacion lineal de las señales ya conocidas no encuentra ninguna interaccion explotable que las pruebas univariantes no hubieran visto ya.** No es un fallo del metodo (el protocolo es el mas riguroso aplicado hasta ahora en el proyecto: verificacion de la implementacion, split temporal correcto, test evaluado una sola vez) — es la confirmacion de que, con estas señales y este universo (mega-caps de EEUU, las acciones mas eficientemente valoradas del mercado), no hay mas alpha que extraer combinandolas de forma distinta. El cuello de botella real, tras esta investigacion, apunta con mas fuerza al universo (demasiado eficiente, demasiado seguido) y a la profundidad de datos (fundamentales a resolucion anual) que al metodo de combinacion de señales.

No se toca `src/` ni `config/weights.php`. Scripts de la investigacion (no comprometidos, scratchpad de sesion): `extract_features.php`, `ridge_lib.php`, `verify_ridge.php`, `run_model.php`, `corr.php`.

---

## 2026-08-27 (segunda entrada) - Dos huecos de cobertura reales cerrados: `RecommendationExplainer` y `NewsRepository`

El usuario pide al equipo proponer mejoras; `qa-tests` cruza `src/` contra `tests/` (ninguna herramienta de cobertura con Xdebug/PCOV disponible en este entorno, asi que la comparacion es manual, clase a clase) para elegir huecos de riesgo genuino, no cobertura por cobertura.

**`RecommendationExplainer` no tenia ni un test**, pese a ser la clase que en `v2.17` arreglo un bug real de produccion: el resumen tomaba "las 3 primeras" señales de un array combinado y, como el analisis tecnico genera mas `Signal` por accion que el fundamental, casi siempre dejaba fuera el motivo fundamental aunque si contara para la puntuacion. `tests/Services/RecommendationExplainerTest.php` (7 tests) cubre exactamente esa regresion (BUY con 5 señales tecnicas y 1 fundamental cita ambos bloques, no solo el tecnico), el limite de 2 señales por bloque de `pickRandom()`, la intro distinta por recomendacion (BUY/HOLD/SELL/STRONG SELL), que HOLD no destaque ninguna señal, y el aviso "Aun asi, conviene tener en cuenta" cuando una BUY tiene negativas. Para fijar el porcentaje del `Score` sin depender de `config/weights.php` (que hoy pone FUNDAMENTAL/VALUATION/QUALITY/DIVIDEND a 0 en la rama solo-tecnico), los tests construyen un `ScoreWeights` propio con toda la ponderacion en `TECHNICAL`.

**`NewsRepository` tampoco tenia ninguno.** Su `sentimentForTicker()` usa SQL especifico de MySQL (`DATE_SUB(NOW(), INTERVAL :days DAY)` con el parametro ligado como `PDO::PARAM_INT`, mas `AVG()`/`COUNT()`), asi que solo se puede probar de verdad contra el motor, no con SQLite ni un doble en memoria. `tests/Integration/NewsRepositoryTest.php` (8 tests, misma infraestructura `IntegrationTestCase` que ya tenia el proyecto desde `v2.90`) cubre: sin noticias devuelve `null`, media y contador correctos sobre varias noticias, que noticias de otro ticker no contaminan el calculo, normalizacion de mayusculas al guardar y al consultar, que una noticia fuera de la ventana de dias no cuenta (y que ampliar la ventana si la incluye), que el titular/fuente devueltos son los de la noticia mas reciente, y que dos noticias del mismo ticker en el mismo instante cuentan las dos (sin restriccion de unicidad).

Verificado: `ddev exec vendor/bin/phpunit`: **384 tests, 1096 assertions, OK (1 skipped, el mismo de siempre)** — sube desde 362/1042. `ddev exec vendor/bin/phpstan analyse` sobre los ficheros nuevos: sin errores. Ningun cambio en `src/` de produccion.

---

## v2.104 - Dos arreglos reales de fiabilidad, tres de diseño, y cinco investigaciones de mercado que no sobreviven a la auditoria

Estado: implementado y verificado.

Mismo encargo que la entrada anterior ("pregunta al equipo si hay alguna mejora"): con la Raspberry Pi y `ddev` activos, se lanzan en paralelo los siete agentes con permiso de proponer (los cinco de mercado, `fiabilidad-datos-mercado`, `diseno-usabilidad`; `qa-tests` ya cerro su parte arriba). Dos hallazgos de mercado pedian auditoria antes de darles credito — `auditor-estadistico` los revisa despues, con el mismo criterio que ya aplico al squeeze de Bollinger el `2026-08-22`.

### Dos arreglos reales de fiabilidad, implementados directamente (`fiabilidad-datos-mercado`, permiso de escritura)

- **`YahooCorporateProfileProvider::fetchCached()` no capturaba fallos de cache.** A diferencia de `CachedMarketDataProvider` (arreglado en `v2.102`), esta clase no envuelve en `try/catch` sus llamadas a `CorporateProfileCacheRepository::find()`/`save()` (PDO con `ERRMODE_EXCEPTION`). Un PDO caido en la ficha de detalle (`Application.php:410`, sin otro `try/catch` exterior) tumbaria la pagina entera — justo lo que el comentario de `Application.php:404-406` decia que no podia pasar ("un fallo aqui nunca debe tumbar la ficha"), una garantia que `fetch()` si cumplia pero `fetchCached()` no. Arreglado con el mismo patron que `v2.102`: fallo de lectura cae a cache-miss (dato en vivo), fallo de escritura se ignora sin propagar, ambos logueados via `logCacheFailure()` nuevo (mismo mecanismo `fwrite(STDERR, ...)`).
- **`CachedMarketMoversProvider::getCached()` silenciaba fallos de cache sin loguearlos.** Ya tenia `try/catch`, pero sin registro — el mismo problema que `v2.102` corrigio en `CachedMarketDataProvider` pero no se replico aqui. Ahora llama al mismo `logCacheFailure()`.

Tests nuevos: `tests/Providers/YahooCorporateProfileProviderCacheFailureTest.php` (3 casos), `tests/Providers/CachedMarketMoversProviderCacheFailureTest.php` (4 casos, la clase no tenia ninguno). Ningun cambio en `ScoreWeights`, `src/Analyzer` ni `config/weights.php`.

### Tres arreglos de diseño, propuestos por `diseno-usabilidad` e implementados directamente

- **`STRONG SELL` compartia badge visual con `SELL`.** `Layout::recommendationClass()` mandaba cualquier recomendacion que no fuera BUY/HOLD al mismo `default => 'sell'`, asi que la señal mas fuerte del motor (la cola izquierda de la distribucion, ya sin tramo `STRONG BUY` simetrico desde `v2.77`) era indistinguible de un SELL moderado en las cuatro pantallas que usan el badge (ranking del Home, Watchlist, Mi cartera, cabecera de la ficha de detalle). Ya señalado como problema real (no bug) en la auditoria de `diseno-usabilidad` del `2026-08-23` sin llegar a arreglarse. Nueva clase `.strong-sell` (fondo solido `var(--bad)`, texto `var(--surface)`, contraste ≈6,57:1, AA con margen) — el unico badge "solido" del sistema, reservado a la unica categoria que lo amerita; cero tokens de color nuevos. Test nuevo: `tests/Web/LayoutRecommendationClassTest.php`.
- **La tabla de Backtesting era la unica de la app con columnas numericas sin alinear a la derecha.** Con 12 columnas numericas (la mas densa de toda la aplicacion) y `text-align: left` por defecto, los numeros quedaban con anchos de digito variables — el mismo problema que `v2.87` ya diagnostico y arreglo para las tablas de cartera. Ranking, Watchlist y Cartera ya usaban `class="num"`; Backtesting no, sin ningun comentario que indicara que fuera deliberado. Añadido `class="num"` a las 11 celdas numericas y a su cabecera (`BacktestPage::columnHeader()`), reutilizando el CSS ya existente.
- **Version de cabecera desactualizada** (`DashboardPage::APP_VERSION`): decia `v2.101` con el proyecto ya en `v2.103`. Segunda vez que se corrige el mismo desfase (la primera, `v2.94→v2.101`, en `v2.101`) — el propio comentario del codigo ya avisaba que volveria a pasar por quedar facil de olvidar al cerrar una version. Corregido a `v2.104`.

### Cinco investigaciones de mercado, ninguna sobrevive

- **`analista-mercado`**: calibracion de RSI(14) (bucket "sweet-spot" 50-70 vs extremos, pareado por fecha, 4 universos × 3 horizontes) y de Volumen (bucket alto ≥1,5x vs bajo &lt;0,6x media 20 sesiones) — nunca auditados con este rigor pese a tener umbrales fijos en `TechnicalScoreAnalyzer`. **0/12 y 0/12** llegan a |t|≥1,96 en ninguno de los dos. Cerradas con resultado nulo. Propone sin medir **RSI(2) de Connors** (reversion de 1-2 sesiones, distinto del RSI14 ya existente) como candidato para otra sesion — encaja mejor que RSI14 con el regimen de reversion a corto plazo que el proyecto lleva documentando desde `v2.78`.
- **`trader-tendencia`**: dos ideas nuevas, ninguna lista para implementar pero con la consistencia de signo mas fuerte medida hasta ahora en esta saga. (1) Factor puro de bucket de spread SMA20/50 (death-cross ancho vs golden-cross, aislado del resto del score, pareado por fecha, 6 universos × 5 horizontes): 0/30 sobrevive Bonferroni, pero **24/30 mismo signo** (test de signos p≈0,0014). (2) Consistencia de tendencia estilo "Frog in the Pan" (Da/Gurun/Warachka 2014): 0/12 sobrevive Bonferroni, pero **11/12 mismo signo** (p≈0,0064), mas fuerte precisamente en `healthcare`. Ninguna se prueba dentro del score real (`TechnicalScoreAnalyzer.php`) por permisos del rol — quedan especificadas para una sesion con permiso de editar `src/`. Momentum ponderado por volumen (Lee &amp; Swaminathan 2000) cerrado con resultado nulo (5/12 signos, contrario a la hipotesis en `largecap60`).
- **`inversor-fundamental`** encontro que ROE alto reduce el drawdown maximo a 60 sesiones (t=3,92, 16/18 fechas) sin mejorar el retorno medio, y pidio auditoria explicita antes de dar credito — **`auditor-estadistico` no lo confirma**: el solapamiento de composicion del grupo "ROE alto" entre fechas consecutivas es de **0,925 (Jaccard)**, con **29/47 tickers (62%) en el mismo grupo ≥95% de las 18 fechas** — no son 18 pruebas independientes, son ~3-5 episodios de mercado repetidos con casi la misma cesta de empresas (megacaps de calidad/tech no financiero vs bancos/ADRs de mercados emergentes). La replica con Deuda/Patrimonio sale invertida (t=-2,51) porque el grupo de "alto apalancamiento" esta dominado de forma permanente por bancos — confusion sectorial, no señal de calidad limpia. No se toca codigo.
- **`gestor-riesgo`** midio que ensanchar el multiplicador de ATR del stop-loss de 2,5x a 3,5x mejora el retorno gestionado de forma monotonica en 4/4 universos — **`auditor-estadistico` repite el test correcto (pareado por fecha, `atr=3,0` vs `atr=2,5` real via `RiskLevelsCalculator`) y no sobrevive**: solo `energy` roza significancia (t=2,39, por debajo del umbral Bonferroni ≈2,5 para 4 universos) y `largecap60` — el universo con mas potencia estadistica (n=1772) — da t≈0 (-0,09), la señal mas fuerte de que no hay efecto real. La "mejora monotonica" original no tenia pareo ni correccion: menos salidas forzadas por stop desplaza la media hacia arriba de forma casi mecanica, no es evidencia por si sola. `RiskLevelsConfig` no se toca. Ademas, `gestor-riesgo` cerro con datos su propia pregunta pendiente del `2026-08-22` (¿domina el tope de riesgo 1,5% o el de peso 20%?): el tope de peso domina en el **73% de los 150 tickers medidos hoy** (mediana ATR%=2,59%), de forma conservadora por diseño — el badge de `RiskLevelsBadge` ya lo comunica posicion a posicion, no hace falta ningun cambio.

Verificado: `ddev exec vendor/bin/phpunit`: **388 tests, 1100 assertions, OK (1 skipped a proposito)** — sube desde 384/1096. `ddev exec vendor/bin/phpstan analyse`: **No errors**. Sin cambios en `config/weights.php` ni en ninguna señal de puntuacion. Todos los scripts de investigacion de los siete agentes (mas el de `auditor-estadistico`) fueron de un solo uso, no comprometidos.

### Propuestas nuevas para "Ideas adicionales sugeridas"

- Bucket de spread SMA20/50 aislado (trader-tendencia, 2026-08-27): p≈0,0014 de consistencia de signo, pendiente de medir dentro del score real con `runCrossSectional()`.
- Consistencia de tendencia "Frog in the Pan" (trader-tendencia, 2026-08-27): p≈0,0064, mas fuerte en `healthcare`, pendiente de medir como campo nuevo en `TechnicalSnapshot`.
...
### Intercambio hecho en `ddev`, Pi todavia pendiente
...
## 2026-09-02 - Segundo bloque: membresia historica del S&P 500, universo de antiguos componentes, y el motor point-in-time del backtest transversal

Estado: puntos 1-4 hechos y verificados en `ddev`; punto 5 con el motor implementado y probado, sin un backtest real de antiguos componentes por un hueco de precio confirmado en vivo (no de codigo). Sin cambios en `config/weights.php` ni sincronizacion con la Pi (fuera de alcance de esta tarea, tal como se pidio).

Objetivo: `roadmap.md`, "Segundo bloque: eliminar el sesgo de supervivencia mientras la suscripcion esta activa" — capturar membresia historica de indices S&P antes de que caduque la suscripcion de EODHD, formar el universo de "antiguos componentes" no cubiertos por los 628 actuales, y preparar el backtest transversal para aceptar un universo que varie por fecha.

### Punto 1: solo GSPC.INDX (S&P 500) tiene membresia historica real bajo este plan — discrepancia real con la documentacion publica de EODHD

Verificado con llamadas de control reales antes de escribir codigo (mismo criterio que `EodhdFiscalPeriodProvider` en `v2.109`): `/api/fundamentals/GSPC.INDX` devuelve `HistoricalTickerComponents` (819 miembros historicos: 504 activos hoy, 315 ya no) y, con `historical=1&from=2012-01-01&to=hoy`, tambien `HistoricalComponents` (326 snapshots point-in-time entre 2012-04-04 y 2026-07-02, confirmando el "cobertura fiable desde abril de 2012" documentado). **`MID.INDX`, `SML.INDX` y `OEX.INDX` SOLO devuelven `Components`** (miembros de HOY, sin fechas), con o sin `historical=1` — respuesta identica byte a byte en ambos casos. La documentacion publica de EODHD (varias paginas, incluida la oficial de la API de fundamentales) afirma que `HistoricalTickerComponents` cubre los cuatro indices; la API real lo desmiente para tres de ellos. Investigado el porque: existe un producto de pago SEPARADO en el marketplace de EODHD ("Indices Historical Constituents Data API", UnicornBay/S&P Global, ~$29,99/mes adicional) que si cubre membresia historica de S&P 400/600/100 — no es parte de "Fundamentals Data Feed", no contratado, no se ha pedido autorizacion para contratarlo.

Los cuatro payloads se archivan igual, sin transformar (`eodhd_raw_index_membership`, migracion 021, mismo patron UPSERT+hash que `eodhd_raw_fundamentals`), con una columna `has_point_in_time` que distingue cual sirve de verdad para reconstruir membresia. `EodhdIndexMembershipProvider` (nuevo) hace la llamada; `bin/capture-index-membership.php` (nuevo) archiva los cuatro y puebla el repositorio normalizado desde GSPC.

### Punto 2: repositorio de membresia normalizado, poblado desde GSPC; `original_symbol` queda vacio (plan de cambios de ticker no contratado)

`index_membership` (migracion 022; `IndexMembershipRepository`): `ticker`, `index_code`, `company_name`, `start_date`, `end_date`, `is_active_now`, `is_delisted`, `original_symbol`, `source`. `EodhdIndexMembershipParser` cruza el `HistoricalTickerComponents` archivado en `list<IndexMembershipRecord>` (DTO nuevo, con `coversDate()` — la logica point-in-time compartida entre el repositorio real y el doble de test). Poblado: 819 filas, UNIQUE por `(ticker, index_code)` (confirmado 0 codigos duplicados en la respuesta real: `HistoricalTickerComponents` modela un unico tramo de membresia por ticker, no reentradas).

Investigado si EODHD ofrece historial de cambios de ticker de EEUU (roadmap.md lo pedia explicitamente): existe, `/api/symbol-change-history` ("US Stock Symbol Rename History API", mapea `old_symbol`->`new_symbol` desde julio de 2022), pero **exige el plan "All-In-One, EOD+Intraday — All World Extended"**, confirmado en vivo con la api_key actual: `403 Forbidden`. No contratado. `original_symbol` queda `NULL` en toda la tabla. Cuando EODHD detecta que un ticker se reutilizo para una empresa NO relacionada, lo refleja con un codigo desambiguado en `HistoricalTickerComponents` (`_old`, `_old1`...; 18/819 casos: `APC_old`, `BBBY_old`, `CA_old`, `CAM_old`, `DELL_old`, `INFO_old1`, `LIFE_old1`, `MRP_old`, `NFX_old`, `NYX_old`, `PCL_old`, `POM_old`, `RAL_old`, `SHLD_old`, `SNDK_old`, `SPLS_old`, `TE_old1`, `VSNT_old`) — ese codigo ya identifica sin ambiguedad al miembro historico, asi que cubre parte del mismo problema sin necesitar el endpoint de pago.

### Punto 3: 310 antiguos componentes identificados, 261/310 (84%) con fundamentales reconstruibles

`IndexMembershipRepository::formerMembersNotIn('GSPC', $tickers628)`: 310 tickers que en algun momento fueron miembro del S&P 500 (`is_active_now = 0`) y no estan en los 628 de `config/universes.php` hoy (`AAL`, `AAP`, `ABMD`, `ADNT`, `ADT`... la lista completa vive en `index_membership`, consultable con esa misma query).

**`bin/archive-legacy-fundamentals.php`** (nuevo) archiva sus fundamentales crudos en la MISMA tabla que ya usan los 628 (`eodhd_raw_fundamentals`, mismo patron reanudable/UPSERT de `v2.111`): **305 archivados en esta ejecucion + 5 ya archivados en una prueba previa = 310/310, 0 errores**, 938 filas totales en la tabla (628 + 310). Los 18 tickers con sufijo `_old`/`_old1` necesitaron una correccion real descubierta en vivo: **el sufijo es SENSIBLE A MAYUSCULAS en la API de EODHD** (`APC_old.US` devuelve 200 con datos reales; `APC_OLD.US` devuelve 401), y `index_membership` guarda el ticker en mayusculas (`APC_OLD`) por consistencia con el resto del proyecto. Se resolvio dando a `EodhdFiscalPeriodProvider::fetchRawJson()` un parametro `$eodhdSymbolOverride` opcional (el ticker de archivado y el simbolo real pedido a la red se separan), con una funcion en el script (`symbolFor()`) que reconstruye el simbolo exacto (`APC_OLD` -> `APC_old.US`) a partir del patron `_OLD(\d*)$`.

**`bin/verify-legacy-fundamentals-coverage.php`** (nuevo, sin red, reconstruye desde el archivo): de los 310, **261 (84%) tienen al menos un `FiscalPeriod` reconstruible**; 49 tienen payload archivado pero sin `Financials.quarterly` utilizable (payloads de 2-5 KB frente a los 300-950 KB tipicos — casos como `DTV` con 1 solo periodo, `MBGL` con 2, o los propios `_old` mas antiguos como `RAL_old`/`VSNT_old`/`TE_old1`, con 2-4 periodos). Es la cifra honesta de "materia prima disponible", no todavia "muestra util" (eso depende tambien del precio, punto 4).

### Punto 4: el plan actual NO da precio historico, ni para los 628 ni para los antiguos componentes; Yahoo se descarta por reciclaje de tickers, con evidencia concreta

Probado en vivo antes de asumir nada: `/api/eod/{ticker}`, `/api/div/{ticker}` y `/api/splits/{ticker}` devuelven **401/403 con la api_key actual PARA CUALQUIER TICKER**, tanto para `AAPL.US` (activo, 628 actual) como para varios antiguos componentes (`AAL.US`, `ABMD.US`, `APC.US`, `BBBY.US`, `ANDV.US`). El plan "Fundamentals Data Feed" no incluye precio historico en absoluto — no es una restriccion especifica de tickers delisted.

Se investigo Yahoo (la fuente de precios ya integrada en el proyecto, `YahooFinanceProvider`) como alternativa, tal como pide el plan ("buscar una fuente compatible" antes de dar el hueco por cerrado), y **se descarta con evidencia concreta, no por sospecha**: Yahoo reutiliza simbolos de empresas delistadas/quebradas para companias completamente NO relacionadas, y sirve sus precios de HOY bajo el ticker antiguo sin ningun aviso.

- `APC` (Anadarko Petroleum, adquirida por Occidental en 2019) hoy es **"ARKO Petroleum Corp."**, una empresa distinta — Yahoo devuelve 200 con precio actual real de ARKO bajo el simbolo `APC`.
- `LEH` (Lehman Brothers, **quebrada en 2008**) devuelve HTTP 200 con datos de otra entidad que reutiliza el simbolo hoy.
- Mismo patron confirmado en `EMC` (Dell EMC, absorbida por Dell en 2016) y `BBBY` (Bed Bath & Beyond, quiebra 2023 — el ticker lo reactivo una empresa distinta, la antigua Overstock.com renombrada, en 2023).
- `ANDV` (Andeavor, adquirida por Marathon Petroleum en 2018) devuelve un stub `MUTUALFUND` sin datos de precio reales (`quote`/`adjclose` vacios).

Pedir precio en bloque a Yahoo para los antiguos componentes contaminaria el backtest con la empresa EQUIVOCADA bajo el ticker correcto — mas peligroso que un 404 limpio, porque no se detecta solo. De los 310 candidatos, 174 estan marcados `is_delisted = 1` por EODHD (sin fuente de precio fiable disponible hoy, ni EODHD ni Yahoo) y 136 no lo estan (siguen cotizando tras salir del indice, p.ej. `AAL`; Yahoo ya los sirve via el pipeline existente sin trabajo adicional, aunque con el mismo riesgo de reciclaje demostrado arriba — conviene verificar nombre de empresa antes de usarlos en una investigacion real, no asumir que el ticker de hoy es la misma entidad de entonces). No se ha conectado ninguna fuente alternativa sin verificar; el hueco queda documentado, no relleno con una suposicion.

### Punto 5: el motor point-in-time existe y esta probado; el backtest real de antiguos componentes sigue bloqueado por el punto 4

`BacktestingService::runCrossSectional()` gana un parametro `?string $indexCode = null`, y el constructor un `?IndexMembershipCheckerInterface $indexMembership = null` (interfaz nueva, no la clase concreta de MySQL directamente, para poder inyectar un doble en memoria en tests sin base de datos — mismo criterio que `MarketDataProviderInterface`). Con los dos conectados, cada muestra tecnica se descarta si `$ticker` no era miembro de `$indexCode` en la fecha de esa muestra; sin ellos (o sin uno de los dos), el comportamiento es identico al de antes de este cambio — mismo criterio de "opcional, no rompe nada existente" que `$fundamentalsHistory` desde `v2.91`. El resultado publica `point_in_time_universe`, `index_code`, `samples_kept` y `samples_dropped_not_member` para que la cobertura real sea visible, no implicita.

Tests nuevos (`BacktestingServicePointInTimeUniverseTest`, con un doble en memoria `ArrayIndexMembershipChecker` que reutiliza `IndexMembershipRecord::coversDate()` en vez de reimplementar la logica point-in-time por segunda vez) fijan **exactamente el criterio de salida que pedia el plan**: un componente que sale del indice ANTES de la fecha D se excluye de esa fecha (`testComponenteQueSalioDelIndiceAntesDeLaFechaDSeExcluye`), uno que entra DESPUES tambien (`testComponenteQueEntroAlIndiceDespuesDeLaFechaDSeExcluye`), y sin `indexCode` nada cambia (`testSinIndexCodeNoActivaElFiltroDeMembresia`). `IndexMembershipRepositoryTest` (integracion, MySQL real) cubre la misma logica expresada en SQL (`isMemberAt()`), y `IndexMembershipRecordTest` la cubre en PHP puro — tres capas de la misma garantia, sin una reimplementacion suelta que pudiera divergir.

**Lo que NO se ha hecho, dicho explicitamente:** un backtest REAL de "S&P 500 desde 2012 con antiguos miembros" (el criterio de salida original del bloque) sigue sin poder ejecutarse de punta a punta, porque el punto 4 confirma que no hay precio fiable para los 174 antiguos componentes genuinamente delistados. El motor esta listo y demostrado correcto con datos sinteticos; conectarlo a `bin/backtest.php --cross-sectional` con `index_membership` real y medir resultados de verdad es la tarea que queda pendiente, condicionada a resolver el hueco de precio (contratar el addon de precios de EODHD, u otra fuente verificada) — no se ha hecho un CLI nuevo para esto porque ejecutarlo hoy solo produciria un universo casi vacio de antiguos componentes (los 136 que siguen cotizando, con el riesgo de reciclaje sin resolver caso a caso).
...
## 2026-09-02 (tercera entrada) - Cuarto bloque: `config/measured_edge.php` actualizado con la medicion point-in-time

Estado: en marcha. El usuario pide seguir con el "Cuarto bloque" del plan de Codex ("hacer util la recomendacion sin exagerar lo medido").

**Punto 2 del bloque** ("mostrar horizonte, fecha/calidad de datos y evidencia historica"): el mecanismo ya existia (`config/measured_edge.php`, `v2.94`-`v2.101`), pero la cifra que mostraba seguia siendo la del `2026-08-15` (34 tickers, whitelist gratuita de FMP) — quedo desactualizada durante toda la investigacion de hoy sin que nadie la tocara. Sustituida por la medicion point-in-time de la entrada anterior (636 tickers, universo real del S&P 500, 121 fechas independientes): **alpha -0,58 pp, t pareado -1,70, sin significancia**. Mismo mensaje de fondo que antes (no hay ventaja demostrada), pero ahora respaldado por una muestra 18 veces mayor y sin la lista de hoy aplicada al pasado. `ddev exec vendor/bin/phpunit`: **477 tests, 1.301 assertions, OK** (sin cambio, ningun test fija los numeros exactos del config). `ddev exec vendor/bin/phpstan analyse`: sin errores.

Lanzados en paralelo el resto de puntos accionables del bloque: separar la semantica de `SELL` segun si el usuario tiene posicion abierta (punto 1, `desarrollador-php`) y el checklist previo a cancelar EODHD (punto 5, `fiabilidad-datos-mercado`: reintentar los 49 antiguos componentes sin datos utilizables, verificar que la reconstruccion funciona sin red, comprobar si el backup de la Pi cubre las tablas nuevas). El punto 3 (modo sombra para una estrategia candidata) se deja sin empezar a proposito: hoy no hay ninguna estrategia candidata que haya superado el rigor de este proyecto (20+ investigaciones nulas), construir la infraestructura de modo sombra ahora seria trabajo sin nada que probar todavia. El punto 4 (Python+DuckDB como laboratorio) no es una tarea, es un permiso ya concedido por el plan sin necesidad de accion — el rigor estadistico de hoy (ridge regression, tests pareados, correccion Bonferroni) ya se ha hecho en PHP puro sin problema.

## 2026-09-02 (cuarta entrada) - Cuarto bloque, punto 1: el badge SELL/STRONG SELL deja de decir lo mismo con y sin posicion abierta

Estado: hecho. Cierra el punto 1 del "Cuarto bloque" lanzado en paralelo en la entrada anterior ("separar `no abrir/evitar` para quien no tiene posicion de `mantener/reducir/salir` para quien ya la tiene").

Objetivo: un `SELL`/`STRONG SELL` es una señal tecnica de entrada, no una orden de liquidar una posicion sin considerar precio de entrada, horizonte, impuestos, concentracion y riesgo — algo que solo puede valorar quien ya tiene la posicion. Antes de hoy el badge mostraba literalmente el mismo texto ("SELL"/"STRONG SELL") en el ranking del Home (donde el usuario normalmente NO tiene posicion) y en "Mi cartera" (donde SI la tiene), confirmado en el codigo en una consulta anterior de esta misma sesion.

Decisiones de arquitectura: cambio deliberadamente de **presentacion/copy contextual**, no un segundo motor de recomendacion. `config/weights.php` sigue siendo la unica fuente de verdad del veredicto BUY/HOLD/SELL/STRONG SELL; `Layout::recommendationClass()` (color/clase del badge, ya distingue SELL de STRONG SELL desde `v2.104`) tampoco se toca. Nuevo `Web\PositionRecommendationNotice` (mismo patron que `MeasuredEdgeNotice`/`EarningsProximityNotice`: clase con un unico `renderInline()` estatico, sin estado) devuelve el texto correcto segun si recibe un `?Holding` o `null`, y solo para SELL/STRONG SELL — BUY/HOLD no llevan aviso porque la ambigüedad "entrar o no entrar" que motiva este texto solo existe para una señal de venta.

Incluye:
- **`StockDetailPage`**: ya recibia `?Holding $position` desde `v2.72` (el panel "Tu posicion"/"Beneficio latente" depende de el). Se conecta a `PositionRecommendationNotice::renderInline($recommendation, $position)` dentro del panel "Por que en la accion X dice Y", justo despues del resumen y el aviso de ventaja medida. Con posicion abierta: *"Tienes una posicion abierta en este valor: esta recomendacion no es una orden automatica de liquidarla. Valora reducirla o vigilarla de cerca segun tu precio de entrada, horizonte, impuestos y el peso que tiene en tu cartera antes de decidir."* Sin posicion: *"No se recomienda abrir posicion en este valor."*
- **`PortfolioPage`** (tabla "Posiciones abiertas"): aqui el usuario SIEMPRE tiene posicion por definicion de la fila, asi que no hace falta ningun `?Holding` adicional. `recommendationBadge()` añade una segunda linea bajo el badge, reutilizando `.cell-sub` (v2.87, "dato secundario de una celda", pensado para justo este caso) en vez de un texto largo que no cabe en una tabla: *"valora reducir o vigilar de cerca"*, solo para SELL/STRONG SELL.
- **`DashboardPage`/ranking y `WatchlistPage`: sin tocar, a proposito.** Investigado antes de decidir, no asumido: `PortfolioService::getPositionFor()` hace una consulta a `transactions` **por ticker** (`findByUserAndTicker()`), y llamarla una vez por cada fila del ranking (60-600 tickers) si seria un N+1 real. Existe una via barata (`PortfolioService::getPortfolio()` ya trae todas las transacciones del usuario en una sola consulta y acumula posiciones en PHP), pero **no existe hoy** un metodo ligero que devuelva solo los tickers con posicion abierta sin las llamadas al proveedor de mercado que `getPortfolio()` hace por cada ticker transaccionado (para calcular precio actual/beneficio, informacion que este aviso no necesita) — construirlo, mas enhebrarlo por `Application::renderDashboard()`/`renderWatchlist()` y los `render()` de las dos paginas, es una pieza de plomeria nueva que el "Cuarto bloque" no pedia y que se sale del alcance deliberadamente pequeño de este cambio (presentacion/copy, no una funcionalidad nueva). Ademas, para el Home el caso por defecto (usuario mirando 60-600 tickers para decidir que comprar, la inmensa mayoria sin posicion) ya es el que "no abrir/evitar" describe bien hoy; la Watchlist es el caso mas discutible (es habitual seguir algo que ya se tiene, para las alertas), pero comparte la misma ausencia de plomeria barata. Queda anotado aqui para una sesion futura si el usuario decide que vale la pena construir ese metodo.

Ficheros nuevos: `src/Web/PositionRecommendationNotice.php`; `tests/Web/PositionRecommendationNoticeTest.php`, `tests/Web/StockDetailPageTest.php` (la ficha de detalle no tenia ningun test hasta ahora). Modificados: `src/Web/StockDetailPage.php`, `src/Web/PortfolioPage.php`, `tests/Web/PortfolioPageTest.php`.

Verificado en ddev con...: **no verificado en esta sesion** — el sandbox de este agente no tiene Docker disponible (`ddev describe` falla con "docker: executable file not found"), asi que no se pudo ejecutar `php -l`, `vendor/bin/phpunit` ni `vendor/bin/phpstan analyse` aqui. Los cuatro ficheros se revisaron a mano con cuidado extra por esta misma razon (firmas de constructor y de `render()` copiadas literal de codigo/tests ya existentes, sin inventar parametros). Pendiente confirmar en un entorno real con `ddev exec vendor/bin/phpunit` y `ddev exec vendor/bin/phpstan analyse` antes de dar esto por cerrado del todo.

Resultado esperado: el badge SELL/STRONG SELL sigue siendo el mismo dato (mismo color, misma clase, mismo veredicto de `config/weights.php`), pero el texto que lo acompaña en la ficha de detalle y en "Mi cartera" ya distingue "no entres" de "no es una orden de vender sin mas, valóralo con tu contexto" — la asimetria que el "Cuarto bloque" pedia cerrar.

**Verificado de forma independiente por Claude** (el agente no tenia Docker en su sandbox): `ddev exec vendor/bin/phpunit`: **489 tests, 1.324 assertions, OK** (+12, sin fallos). `ddev exec vendor/bin/phpstan analyse`: sin errores. Commiteado, subido y desplegado en la Pi (solo codigo, sin tocar datos).

---

## 2026-09-02 (quinta entrada) - Consenso del equipo sobre la revision de Codex del motor, y checklist de EODHD

El usuario pide revisar `REVISION_MOTOR_CODEX_2026-09-02.md` (documento externo, sin comitear en la raiz del repo, dejado por Codex a peticion del usuario) con el equipo completo, para que el motor de analisis sea "lo mas eficiente posible", aprovechando la suscripcion de EODHD mientras dura. Se lanzan en paralelo `auditor-estadistico` (los 5 puntos P0, la parte mas grave del documento) y cuatro agentes de mercado, cada uno sobre la parte de su terreno (P1-P4), mas `fiabilidad-datos-mercado` con el checklist previo a cancelar EODHD ya en marcha desde la entrada anterior. Ninguno toco `src/`, `config/weights.php` ni la Pi (salvo el checklist, ver mas abajo) — es una ronda de auditoria/triage, no de implementacion.

### El hallazgo central de Codex, confirmado por Claude y por el equipo: P0.1 es real

`BacktestingService::sampleHistory()` genera la recomendacion con el cierre de la barra `$current` y mide `forward_return` desde ESE MISMO cierre. El cron real corre a las 23:00, despues del cierre de EEUU: ese precio no es operable, como muy pronto se podria entrar a la apertura del dia siguiente. Confirmado por Claude directamente en el codigo antes de convocar al equipo, y reconfirmado de forma independiente por `auditor-estadistico` y `analista-mercado`.

### Auditoria de `auditor-estadistico` sobre los 5 puntos P0 — veredicto completo, con una correccion de prioridad sobre el propio documento de Codex

- **P0.1** (entrada al cierre de la señal): CONFIRMADO.
- **P0.2** (independencia de fechas: el filtro compara DIAS NATURALES contra un horizonte expresado en SESIONES): **CONFIRMADO, y es el de mayor impacto de los cinco** — corrompe el supuesto de independencia detras de **todos** los t-stats pareados publicados con `runCrossSectional()` desde `v2.70`, no solo el hallazgo de hoy. Con horizonte 20 (el patron estandar de casi toda `versions.md`), 20 sesiones ocupan ~28 dias naturales, asi que fechas separadas 20-27 dias se cuentan como independientes sin serlo del todo.
- **P0.3** (lookback minimo 80 barras, Momentum 12-1 necesita &gt;250): CONFIRMADO literalmente, incluido el "neutral silencioso" que denuncia Codex — sin señal `Signal` visible que avise de que ese valor es relleno, no medicion.
- **P0.4** (retorno total): **matizado, mas favorable de lo que dice Codex**. Verificado EMPIRICAMENTE contra un split real de NVDA (10:1, 2024-06-10) que el `close` de Yahoo YA viene ajustado por splits (el comentario interno del proyecto era correcto, la duda de Codex sobre esto queda resuelta a favor del codigo existente). Solo falta el ajuste por dividendos, de magnitud pequeña frente al ruido de los t-stats actuales.
- **P0.5** (coste vs turnover real): CONFIRMADO el mecanismo, pero con una lectura de riesgo distinta a la implicita en el documento — la metrica principal (`forward_return` de `runCrossSectional()`) no lleva NINGUN coste aplicado, asi que el sesgo actual es conservador (favorece las conclusiones de "no hay alpha" ya publicadas, no las infla). Baja urgencia real hoy.

**Orden de arreglo recomendado por el auditor** (difiere ligeramente del orden del propio documento de Codex): P0.2 primero (el mas barato y el de mayor radio de contaminacion), P0.1 despues, P0.3 en el mismo lote (mismo fichero), P0.4/P0.5 aplazados.

### Veredictos del equipo de mercado sobre P1-P4

- **`gestor-riesgo` (P1.1/P4)**: el diagnostico de "RISK se juzga con la metrica equivocada" es correcto (nunca se ha medido si RISK reduce dispersion/drawdown de la cesta, solo si sube la media — eso ya se cerro en `v2.88`, sin relacion), pero la solucion de tres motores separados es sobredimensionada. Propone primero una medicion barata (añadir dispersion/peor-caso de `$topReturns` a `crossSectionalStatistics()`, repetir el protocolo de `v2.88` comparando esa metrica) antes de tocar arquitectura. **Descarta el simulador de cartera completo de P4** (equal-weight/vol-scaled/beta/sector/SPY/capacidad de broker) como sobre-ingenieria para una herramienta personal — `PortfolioConcentrationCalculator` ya cubre la pregunta real del usuario. La pieza barata de P4 (separar recomendacion sin/con posicion) ya esta hecha (ver entrada anterior).
- **`trader-tendencia` (P1.2/P3.4)**: confirma que mezclar señales de tendencia y de reversion en el mismo bloque `TECHNICAL` es real (verificado linea a linea), y que es una pregunta estructuralmente distinta de la saga ya cerrada del cruce SMA20/50 (no es "la quinta transformacion de lo mismo"). Pero da **prioridad mas alta y mas barata a P3.4** (momentum-only con el lookback corregido a 251 barras): es la señal con mas respaldo academico del proyecto (Jegadeesh-Titman 1993) y el bug de P0.3 la contamina retroactivamente en TODAS las mediciones anteriores, incluida la de hoy. P1.2 queda condicionada a que P0.2 este resuelto para horizontes largos (60/120d). Insiste: no reabrir el cruce SMA20/50 bajo ninguna forma, esa pregunta esta agotada.
- **`inversor-fundamental` (P3)**: P3.1 (puerta de calidad) prematura como pieza aislada sin consumidor, absorberla como paso 0 de P3.3. P3.2 (look-ahead residual en `dividendGrowth5y`) CONFIRMADO en el codigo, arreglo mas barato de lo que parecia (`DividendGrowthCalculator` ya acepta fecha de referencia, solo falta conectarlo en `fundamentalsAt()`), no urgente en solitario (DIVIDEND a peso 0) pero incluir si se hace P3.3. **P3.3 (fuerza relativa fundamental con los 628 tickers) si merece repetirse** — es la primera vez que la muestra cumple el propio umbral `MIN_PEERS=8` del proyecto (47 tickers en `2026-08-15` no lo cumplian, `~4/sector`). No es una investigacion nueva, es ejecutar la condicion que `roadmap.md` linea 133 ya dejo escrita como pendiente. Requiere trabajo previo de `desarrollador-php`: `RelativeFundamentalScorer` no esta conectado al backtest transversal hoy.
- **`analista-mercado` (P2 + vision de conjunto)**: confirma los 4 puntos P0 de forma independiente (incluido el docblock del propio codigo ya admitiendo el look-ahead de dividendos desde `v2.64`). Recuerda el dato que falta en el marco: el score **ya esta invertido** (decil alto peor que decil bajo, t hasta -4,93, 6/6 universos, nunca revertido) — arreglar P0.1 podria cambiar si eso sigue siendo cierto, no solo afinarlo. Sobre P2 (tabla de investigacion congelada + rank-IC + ablacion por familia): parte ya esta hecha con otro nombre (correlacion 0,45-0,85 ya medida en el modelo ridge de `2026-08-25`, deciles/monotonicidad ya medidos en `v2.76`, disciplina de correccion por comparaciones multiples ya es la norma del proyecto desde hace 20+ entradas). Construirla AHORA, antes de arreglar P0, seria enterrar los bugs de P0.2/P0.3 dentro de un dataset que por llamarse "canonico" inspiraria mas confianza de la que merece. **Recomendacion de unico proximo paso: arreglar P0.1+P0.2+P0.3 juntos y repetir solo el baseline actual como nueva referencia — nada mas todavia.**

### Consenso final del equipo: converge en un orden claro

Los cinco agentes, trabajando en paralelo y sin verse entre si, llegan a la misma conclusion de secuencia:

1. **Arreglar P0.1+P0.2+P0.3 juntos** (mismo fichero, `BacktestingService::sampleHistory()`/`runCrossSectional()`, coste bajo, prerrequisito de todo lo demas).
2. **Repetir el baseline actual** (el mismo tipo de medicion que `v2.114`, universo point-in-time real) como nueva referencia valida.
3. **Solo entonces**, con datos de referencia fiables: `P3.4` (momentum-only, prioridad mas alta y mas barata de las investigaciones nuevas) y, en paralelo si hay capacidad, conectar `RelativeFundamentalScorer` al backtest transversal para `P3.3` (aprovechando la suscripcion de EODHD mientras dure).
4. **Aplazado indefinidamente**: P0.4 (dividendos, bajo impacto), P0.5 y P4 (simulador de cartera completo, sobre-ingenieria para esta herramienta), P2 (dataset congelado, prematuro sin P0 resuelto), P1.1 arquitectura de tres motores (condicionada a la medicion barata de dispersion primero).

### Checklist previo a cancelar EODHD (`fiabilidad-datos-mercado`) — con una correccion de Claude tras verificar en vivo

- **49 fallos de fundamentales de antiguos componentes reintentados: 0/49 se resolvieron.** Confirmado que no es rate-limit disfrazado: 20 tickers tienen `Financials.quarterly` genuinamente vacio en la fuente, 29 tienen datos completos pero SIN `filing_date` en ninguna entrada (descartados por diseño, sin fecha de publicacion no hay point-in-time valido). Limitacion real de la fuente para tickers antiguos, no arreglable desde este lado.
- **Reconstruccion sin red: confirmada con evidencia real**, no solo argumentada. Con un `HttpClient` que lanza excepcion ante cualquier peticion HTTP, `EodhdFiscalPeriodProvider::parse()` reconstruyo 889/938 tickers archivados sin tocar la red, cero llamadas — cierra formalmente un criterio de salida del "Segundo bloque" que nunca se habia verificado explicitamente.
- **Backup de la Pi: el mecanismo cubre las tablas nuevas automaticamente** (`stockanalyzer-backup.sh` excluye por lista, no incluye por lista — cualquier tabla nueva se respalda sin tocar el script), confirmado con `CREATE TABLE` de las tres tablas nuevas en el dump de hoy. **Pero: las tres tablas tienen 0 filas en la Pi** — los 938 payloads de fundamentales, los 4 de indices y los 819 registros de membresia archivados hoy **existen solo en `ddev`, no se han sincronizado a la Pi**. Si se cancela EODHD antes de sincronizar esto, la unica copia seria la de `ddev`.
- **Corregido por Claude tras verificar en vivo** (el informe del agente decia "estado limpio, reset-failed"; no lo era): la ejecucion manual del servicio de backup que hizo el agente para esta verificacion **fallo de verdad por timeout** (subida a Google Drive colgada por el limite de cuota `429`, matada por systemd a los 10 minutos, servicio en estado `failed`). La copia LOCAL se completo bien (176 MB, symlink `stockanalyzer-latest.sql.gz` actualizado) y **el temporizador programado sigue intacto** (proxima ejecucion real: hoy 23:30, confirmado con `systemctl list-timers`) — no hay perdida de datos ni de programacion, pero el hallazgo real es que el servicio no falla con gracia ante un rate-limit de Google, se queda colgado hasta que systemd lo mata. No arreglado hoy (fuera de alcance de esta ronda), documentado para una sesion futura de infraestructura. Limpiado tambien un fichero suelto de la propia sesion de Claude (`/tmp/revert_fh.sql`, del intento de revertir `v2.109` que se abandono el `2026-09-01`).
- **Licencia/retencion de EODHD tras cancelar: sigue SIN confirmacion por escrito**, tal y como ya pedia `roadmap.md` desde el "Segundo bloque" original. No dar por hecho que se puede conservar el archivo tras cancelar sin esa confirmacion.
- **Correccion de telemetria de cuota (10 unidades/llamada, no 1) sigue sin implementarse** en `bin/archive-eodhd-fundamentals.php`/`bin/archive-legacy-fundamentals.php` — pendiente, bajo riesgo (hay margen de sobra en 100.000/dia).

### Recomendacion de Claude al usuario, pendiente de decision

1. **Sincronizar `eodhd_raw_fundamentals`/`eodhd_raw_index_membership`/`index_membership` a la Pi** (solo archivo crudo, sin regenerar `fundamentals_history` todavia) — es lo unico realmente urgente por la ventana de la suscripcion, y es barato (mismo patron de dump+scp+carga ya usado hoy, sin necesitar `RENAME TABLE` en produccion).
2. **Implementar P0.1+P0.2+P0.3** en `BacktestingService` (bajo coste, consenso unanime de los 5 agentes) y repetir el baseline de `v2.114` para ver si cambia.
...
## 2026-09-02 (quinta entrada) - P0.1+P0.2+P0.3 implementado en `BacktestingService`, 27 tests rotos arreglados y 4 tests nuevos que fijan el criterio de cada arreglo

Estado: hecho y verificado en `ddev`. La logica de `BacktestingService.php` (P0.1 entrada en la apertura de la sesion siguiente, P0.2 independencia de fechas por sesiones bursatiles reales, P0.3 descarte de muestras sin Momentum 12-1 calculable) la implemento un agente anterior hoy mismo y quedo revisada como solida; esta entrada cierra la tarea con los 27 tests que rompio arreglados (recalibrados, no debilitados) y los tests nuevos que faltaban.

Objetivo: dejar `ddev exec vendor/bin/phpunit`/`vendor/bin/phpstan analyse` en verde tras P0.1+P0.2+P0.3, sin tocar la logica de produccion, y demostrar explicitamente el criterio de correccion de cada punto P0 (no solo "no rompe nada").

### Causa raiz de las 27 roturas: los fixtures de test eran demasiado cortos para Momentum 12-1

Los cinco ficheros de test de `BacktestingService*` construyen historicos sinteticos de 81-91 velas (el minimo que exigia `sampleHistory()` antes de P0). Momentum 12-1 (`TechnicalAnalyzer::momentumSkippingRecent()`) necesita **mas de 250 cierres** para no devolver `null`, y P0.3 descarta por completo cualquier muestra con momentum `null` (antes se rellenaba con un neutral silencioso, 3,5 puntos, que es precisamente el bug que P0.3 corrige). Con fixtures de 81-91 velas, TODAS las muestras se descartaban: de ahi que las 27 roturas fuesen casi todas la misma causa (`0 muestras` donde se esperaba 1+), no 27 fallos independientes.

### La solucion: un prefijo compartido, extraido a un trait

`tests/Services/MomentumPrefixFixture.php` (trait nuevo, usado por los cinco ficheros): antepone `MOMENTUM_PREFIX_LENGTH` (170) velas PLANAS, con fechas ANTERIORES a la primera fecha del patron de cada fixture (nunca insertadas en medio), asi que cada patron ya calibrado conserva exactamente sus fechas y precios de siempre y solo se desplaza en INDICE. El precio del prefijo tiene que coincidir EXACTAMENTE con el primer cierre del patron que sigue: `TechnicalAnalyzer::atr()` usa el cierre del dia anterior para el true range y lo suaviza (Wilder) sobre TODO el historico recibido, asi que un salto de precio en la vela de union arrastraria un pico de ATR14 durante docenas de sesiones (verificado: con un salto, el ATR14 de `BacktestingServiceTest::baselineQuotes()` salia 1,0004658685350087 en vez de 1,0 exacto) y rompia los tests que hardcodean niveles de riesgo exactos.

P0.1 añade una segunda exigencia estructural: la entrada de cada muestra es la apertura de la sesion SIGUIENTE a la señal, asi que cada fixture necesita al menos una vela mas de la que tenia antes (la "vela de entrada"), y en varios casos ademas una vela extra de margen al final para que el horizonte completo siga cabiendo dentro del historico (el bucle exige `$entryIndex + $horizonDays < $count`, una sesion mas de la que exigia antes de P0.1).

### Ficheros de test arreglados y como

- **`tests/Services/BacktestingServiceTest.php`** (12 tests): `baselineQuotes()` usa el prefijo del trait; `historyWithPostEntryPath()` añade la vela de entrada (open=104.0) antes de los dias de horizonte. `mixedRiskLevelsRegimeHistory()` usa el mismo prefijo (sin vela de entrada extra: ese fixture ya tenia margen de sobra). `crashAfterEntryHistory()` necesito un dia mas de caida (11 valores, no 10) y `steadyDeclineHistory()` una vela mas (92, no 91) por el mismo motivo estructural. Recalibrado (valor real medido, no el antiguo forzado): la puntuacion de la señal calibrada sube de 82,12% a **82,42%** (con el prefijo continuo el ATR14 sigue siendo exactamente 1,0, pero Momentum 12-1 deja de ser el neutral silencioso de 0% y pasa a +2,95% real — sigue siendo BUY con holgura de sobra, y ningun test hace `assertSame()` sobre el porcentaje exacto, solo sobre `recommendation === 'BUY'`, asi que no hizo falta tocar ninguna asercion por este cambio, solo el docblock). `testAlphaVsMediaDelUniversoConMezclaDeSenalesBuyYNoBuy` si cambia de verdad: con la entrada desplazada los `forward_return` pasan de +1,92/+0,47/+3,76/-18,10 a **-23,16/-13,70** (P0.1 desplaza que dia exacto entra/sale de la ventana de 5 sesiones) y la segunda señal pasa de HOLD a STRONG SELL (no afecta a la asercion, que solo comprueba que no es BUY).
- **`tests/Services/BacktestingServiceStatisticsTest.php`** (6 tests): `history()` añade el prefijo y, al final, una vela mas que continua la MISMA progresion que la ultima aplicada (no un cierre plano): con un cierre plano, la ultima ventana de 5 dias del caso limite de desviacion tipica cero (`testConDesviacionTipicaCeroNoHayDivisionPorCeroEnElTStat`) rompia la igualdad de retornos que el test necesita por 0,05 pp. Todos los `forward_return`/estadisticos derivados se recalcularon EJECUTANDO el servicio real (no a mano) y se actualizaron los valores esperados con el numero nuevo.
- **`tests/Services/BacktestingServiceCrossSectionalTest.php`** (5 tests): el fichero declaraba explicitamente que TODOS sus `forward_return` debian ser porcentajes exactos (+5,00/+3,00/-5,00/-3,00) para poder calcularse a mano; con P0.1 desplazando la entrada, los deltas diarios calibrados a mano dejaban de dar esos numeros exactos (salian 4,58/2,43/-3,99...). Se sustituyo el mecanismo de "lista de incrementos diarios" por `signalHistory()`+`linearSegment()` (interpolacion lineal hasta un cierre EXACTO = `entrada * (1 + move/100)`), que restaura la propiedad de porcentajes exactos sin tener que recalcular deltas a mano cada vez. Ninguna asercion cambio de valor (todas seguian siendo +5,00/+3,00/-5,00/-3,00), solo la forma de construir el fixture.
- **`tests/Services/BacktestingServicePointInTimeUniverseTest.php`** (3 tests) y **`tests/Integration/BacktestUsesPointInTimeFundamentalsTest.php`** (1 test): mismo patron de prefijo+vela de entrada. En el segundo, ademas, hizo falta mover la fecha de los `recordSnapshot()` de prueba de `2023-12-01` a `2000-01-01` en los dos tests que esperaban cobertura del 100% (ver el bug real, mas abajo): `2023-12-01` caia DENTRO del nuevo prefijo (que ahora empieza sobre `2023-07-15`), asi que parte de las lecturas de fundamentales que hace el prefijo (candidatas luego descartadas por P0.3) quedaban sin snapshot y la cobertura bajaba a 67,57% en vez de 100%.

### Bug real encontrado en `BacktestingService.php` (no corregido, solo documentado: fuera del alcance de esta tarea de tests)

`sampleHistory()` llama a `stockAt()` (y por tanto `fundamentalsAt()`, que incrementa `pointInTimeHits`/`pointInTimeMisses`) **antes** de comprobar si Momentum 12-1 es `null` y descartar la muestra. Antes de P0.3 esto no importaba (nunca se descartaba nada, cada iteracion producia una muestra real). Con P0.3, cada candidato que el bucle visita y luego descarta por momentum nulo **igual cuenta** para `fundamentals_point_in_time_pct`, aunque nunca llegue a aparecer en `samples`/`recent_samples`. El docblock de `pointInTimePercent()` dice explicitamente "que porcentaje de las MUESTRAS uso fundamentales de su propia fecha": con P0.3 activo, el numerador/denominador ya no describen solo las muestras publicadas, sino tambien candidatos descartados invisibles. No cambia ningun resultado incorrecto (la cobertura reportada sigue siendo <=100% y nunca miente en la direccion de "mejor de lo real"; si acaso subestima la cobertura real de las muestras publicadas), asi que no es un bug de seguridad de datos, pero si una imprecision de la metrica que merece su propia entrada si se decide corregir (mover la comprobacion de momentum ANTES de `stockAt()` en `sampleHistory()`). No se toca aqui porque el encargo explicito de esta tarea era "no toques la logica de BacktestingService.php".

### Tests nuevos: `tests/Services/BacktestingServiceP0FixesTest.php`

Cuatro tests que fijan el criterio de correccion de cada punto P0 por separado (no solo que el resto de la suite siga verde):

- `testLaEntradaUsaLaAperturaDeLaSesionSiguienteNoElCierreDeLaSenal` (P0.1): la señal cierra en 104,0 pero la sesion siguiente abre en 200,0 (hueco artificial enorme, deliberado para que no haya ambiguedad posible); `forward_return`/`managed_return` salen calculados desde 200,0 (+0,5%), nunca desde 104,0 (que habria dado +93,27%).
- `testLaUltimaBarraDelHistoricoNoGeneraMuestra` (P0.1): el mismo fixture calibrado, cortado justo en la señal (sin vela de entrada), produce 0 muestras pese a que Momentum 12-1 SI es calculable ahi (251 cierres, justo el umbral); añadiendole una unica vela mas basta para que la misma señal SI produzca una muestra.
- `testDosFechasConMenosSesionesQueElHorizonteSeDescartanAunqueHayaMasDiasNaturales` (P0.2): dos parejas de tickers con historicos identicos salvo que el segundo par arranca 4 DIAS HABILES despues del primero (cruzando un fin de semana): 4 sesiones reales de separacion (<horizonte 5, debe descartarse) pero 6 dias naturales (>=horizonte 5, un filtro por dias naturales lo habria contado como independiente). `dates_evaluated` se queda en 1 y `dates_dropped_overlapping` sube a 1.
- `testUnaMuestraConMenosDe251BarrasSeExcluyeYElContadorLoRefleja` (P0.3): 90 velas (sin el prefijo), tendencia alcista limpia. `run()` produce 0 muestras (no un neutral silencioso) y `runCrossSectional()` publica `samples_dropped_momentum_null = 1`, exactamente la unica candidata que visito el bucle.
...
### Incidente real al desplegar en la Raspberry Pi de produccion, mismo dia

Tras desplegar y aplicar la migracion 025 en la Pi (`938` filas confirmadas en `eodhd_raw_fundamentals` alli tambien, replicadas de antes), se lanzo `bin/backfill-eodhd-fundamental-versions.php` -- **la Pi dejo de responder por completo a la red** (`ping` con 100% de perdida, "Destination Host Unreachable"), no solo SSH lento. El usuario tuvo que apagarla y encenderla fisicamente; `uptime` tras el reinicio confirmo "up 2 min".

**Causa real, no la compresion gzip**: la Pi de produccion tiene solo 906 MiB de RAM y ~199 MiB de swap. El SELECT original (`SELECT ticker, payload_json, payload_hash, fetched_at FROM eodhd_raw_fundamentals ORDER BY ticker`) usa el modo "bufferado" por defecto del driver PDO/MySQL: los ~580 MB de `payload_json` de las 938 filas se cargan TODOS en memoria de PHP antes de que el bucle procese la primera fila, muy por encima de la RAM fisica disponible y sin swap suficiente para absorberlo -- consistente con un cuelgue del sistema entero (no solo un `Fatal error: Allowed memory size exhausted` de PHP, que habria dejado un mensaje claro y no habria tumbado la red). Verificado despues del reinicio: `eodhd_raw_fundamentals` (origen) intacto en 938 filas, `eodhd_raw_fundamental_versions` en 0 filas (crash antes de escribir nada, sin corrupcion), la app respondiendo 200 con normalidad -- el incidente no perdio ni corrompio ningun dato, solo colgo el proceso a mitad.
...
## 2026-09-05 (segunda entrada) - Bloques B2+B3+B7 del plan de Codex: calendario de resultados, tendencias de analistas y SEC Form 4 archivados para los 938 tickers

Estado: implementado y ejecutado contra los 938 tickers reales de `ddev`. Cierra B2 (`calendar/earnings`, 938/938, 0 errores) y B3 (`calendar/trends`, 938/938, 0 errores) del plan de Codex; B7 (SEC Form 4, reemplazo del bloque `InsiderTransactions` heredado) queda con cobertura real de 715/938 tickers, 223 con error -- desglosado abajo, son limites reales del endpoint, no fallos del script. Trabajo hecho ENTERAMENTE contra `ddev`; **ningun SSH ni conexion a la Raspberry Pi de produccion (192.168.1.156)**, misma instruccion del usuario que en la entrada anterior.

### B2+B3: verificacion en vivo ANTES de escribir nada (cambia una premisa del plan)

El plan asumia que agrupar tickers en `symbols=A,B,C` podria ahorrar cuota frente a una llamada por ticker. Verificado en vivo el 2026-09-05 comparando `/api/user` antes/despues de llamadas reales: **el coste es de 1 unidad POR SIMBOLO pedido, no por llamada HTTP** -- una llamada con 3 simbolos cuesta exactamente 3 unidades, igual que 3 llamadas de un simbolo cada una. Agrupar NO ahorra cuota en estos dos endpoints (al contrario que Fundamentals, donde una sola llamada trae todo el historico por un precio fijo). Por eso ambos scripts piden UN ticker por llamada: mismo coste total, pero cada respuesta queda ya perfectamente delimitada a su ticker y se archiva BYTE A BYTE, sin tener que re-cortar un JSON que mezclase varios tickers.

Tambien verificado en vivo, y relevante para cualquier uso futuro de `calendar/earnings`: `symbols` FILTRA la ventana de fechas, no la sustituye -- sin `from`/`to` explicitos la ventana por defecto es "hoy..hoy+7 dias" y casi cualquier ticker real devuelve `earnings: []` aunque tenga historico completo. `bin/archive-eodhd-calendar-earnings.php` exige `--from`/`--to` (por defecto `1970-01-01` y "hoy + 2 años") precisamente por esto.

### Hallazgo critico de B3: `calendar/trends` NO es un archivo historico de snapshots

Verificado en vivo el 2026-09-05, antes de dar por buena la premisa del plan de "demostrar la semantica temporal antes de usarlo en backtest": se pidio `Trend.Quarterly['2024-09-30']` de AAPL por DOS vias el mismo dia -- `Fundamentals.Earnings.Trend` (ya archivado unas horas antes en el Bloque B1) y `/api/calendar/trends` (pedido en el momento) -- y los 22 campos de la fila son **IDENTICOS byte a byte** entre ambas fuentes (mismo `growth`, `earningsEstimateAvg`, `epsTrend7/30/60/90daysAgo`, etc.). Esto demuestra que ambos endpoints leen del MISMO registro en vivo de EODHD, no de un archivo separado con snapshots congelados en el tiempo.

Consecuencia directa para B3: **no hay forma de confirmar, con una sola captura, si un valor mostrado hoy para una fecha antigua (p.ej. 2018) es el que existia realmente en 2018 (congelado desde entonces) o si EODHD lo recalcula/reescribe con datos posteriores** -- no hay ningun campo de "fecha de esta estimacion" en la fila. Se confirmo ademas, cruzando por fecha en AAPL y JPM, que para una MISMA fecha de cierre de periodo pueden coexistir HASTA CUATRO filas con `period` distinto (`+1q`, `0q`, `+1y`, `0y`) y valores DIFERENTES entre si (p.ej. AAPL `2017-09-30`: `+1q` con EPS≈1,81 y 38 analistas, `0q` con EPS≈1,87 y 31 analistas, `0y` con EPS≈9,00 y 34 analistas) -- son snapshots de la estimacion capturados en momentos distintos respecto al reporte real, pero sin marca de fecha propia que lo demuestre desde una unica peticion. **Conclusion, documentada en el codigo (`EodhdCalendarProvider`, `bin/archive-eodhd-calendar-trends.php`) y aqui: NO USAR `calendar/trends` (ni `Fundamentals.Earnings.Trend`, por la misma razon) en un backtest sin resolver antes esta duda** -- repitiendo esta misma captura mas adelante y comparando si los valores de fechas antiguas cambian, o preguntando a soporte de EODHD. Consecuencia colateral: al ser el mismo dato en vivo que `Fundamentals.Earnings.Trend` (Bloque B1), archivar `calendar/trends` hoy NO aporta informacion nueva -- se archiva de todas formas porque lo pedia el plan y porque una futura repeticion serviria precisamente para resolver la duda de arriba (si los valores cambian entre dos capturas, hay revision viva; si no cambian nunca, son snapshots congelados).

### B2: cruce `calendar/earnings` vs `Fundamentals.Earnings.History`, tal como pedia el plan

Verificado con AAPL y JPM: donde ambas fuentes tienen la misma fecha, `actual`/`report_date` (calendario) coincide EXACTAMENTE con `epsActual`/`reportDate` (Fundamentals) en el 100% de los casos comparados (129/129 AAPL, 126/126 JPM, 0 discrepancias). Diferencia real encontrada: `calendar/earnings` con `from=1970-01-01` tiene 3 fechas MENOS que `Earnings.History` en AAPL (1993-12-31 a 1994-06-30, los tres trimestres mas antiguos de su historico) -- no son identicos byte a byte, aunque donde coinciden los valores son fiables.

### B7 (SEC Form 4): dos hallazgos reales sobre la marcha, antes del archivado completo

1. **Paginacion no fiable con el criterio REST habitual.** `page[limit]` (maximo documentado 100) y `page[offset]`, con `meta.total` en la respuesta. Un primer intento de este script paraba de pedir paginas en cuanto una traia MENOS filas que `page[limit]` (el criterio de paginacion mas comun). Probado en vivo: `MSFT.US` con `page[limit]=100` en `offset=0` devolvio 99 filas (una menos del limite) con `meta.total=784` en la MISMA respuesta -- muy por encima de 100. Con el criterio ingenuo, el archivado se habria quedado en 99/784 filings de MSFT (13%) sin avisar de nada. Corregido ANTES de lanzar el lote completo: `fetchAllPagesMerged()` lee `meta.total` de la primera pagina y SIEMPRE pide `ceil(total/limit)` paginas (acotado a `--max-pages`, por defecto 20), nunca decide parar por el tamaño de una pagina intermedia.
2. **El sufijo `_old` (valido en Fundamentals/Calendar) NO es un simbolo valido aqui.** `APC_old.US` -> 422 "The symbol must be a valid ticker symbol.". Los tickers no estadounidenses tampoco: `SAN.MC` -> 404 "Symbol not found" (Form 4 es exclusivo de emisores listados en EEUU). Ambos casos se dejan como error esperado, no como reintento.

Fusionar varias paginas en un unico documento (`{"ticker","total_declared_by_eodhd","total_archived","pages_fetched","page_limit","truncated","data"}`) antes de archivar es una diferencia real frente a `fetchRawJson()`/`fetchRawEarningsJson()`/`fetchRawTrendsJson()`: **NO es el cuerpo original de una unica respuesta HTTP**, es una reconstruccion determinista de varias. Documentado explicitamente en el codigo para que nadie asuma que `sec-form4`/`full` es un archivo bit a bit como el resto de secciones.

### Implementado

- **`src/Providers/EodhdCalendarProvider.php`** (nuevo): `fetchRawEarningsJson(ticker, from, to, ?override)` y `fetchRawTrendsJson(ticker, ?override)`. Clase separada de `EodhdFiscalPeriodProvider` a proposito (familia de endpoints y coste por unidad distintos); duplica ~20 lineas de peticion/validacion en vez de forzar una abstraccion compartida, mismo criterio que ya separa `FmpFiscalPeriodProvider` de `EodhdFiscalPeriodProvider`.
- **`src/Providers/EodhdSecFilingsProvider.php`** (nuevo): `fetchRawForm4Page(ticker, limit, offset, ?override)`, una pagina cruda por llamada; la orquestacion de paginacion/fusion vive en el script, no en el proveedor.
- **`bin/archive-eodhd-calendar-earnings.php`** (nuevo): universo = `EodhdRawFundamentalsRepository::archivedTickers()` (938, no `config/universes.php`), `--from`/`--to` configurables, reanudable via `hasVersion($ticker,'calendar','earnings')`, mismo `$symbolFor()` para los 18 tickers `_old` que ya usaban los scripts de B1. Escribe `api_version='calendar'`, `section='earnings'`.
- **`bin/archive-eodhd-calendar-trends.php`** (nuevo): mismo patron, sin `from`/`to`. Escribe `api_version='calendar'`, `section='trends'`.
- **`bin/archive-eodhd-sec-form4.php`** (nuevo): mismo patron + `fetchAllPagesMerged()` (paginacion por `meta.total`, `--max-pages` configurable, aviso explicito cuando el truncado es por `--max-pages` frente a una discrepancia menor sin relacion con el tope). Escribe `api_version='sec-form4'`, `section='full'`.
- **Tests nuevos**: `tests/Providers/EodhdCalendarProviderTest.php` (7 tests: URL/parametros correctos para earnings y trends, ticker sin punto recibe `.US`, override de simbolo `_old` respetado, error HTTP legible sin filtrar la API key, cuerpo no-JSON rechazado, ticker vacio no gasta llamada) y `tests/Providers/EodhdSecFilingsProviderTest.php` (8 tests: URL con paginacion correcta, `limit` acotado a `MAX_PAGE_LIMIT`, `offset` negativo acotado a 0, ticker no estadounidense y ticker `_old` como errores legibles, cuerpo sin clave `data` rechazado, `data: []` NO es un error, ticker vacio no gasta llamada).

### Resultado real del archivado (ddev, 2026-09-05)

- **B2 (`calendar/earnings`)**: 938/938 tickers con version `calendar`/`earnings` archivada. 933 archivados en el lote completo + 5 ya archivados de una verificacion manual previa (`AAPL`, `MSFT`, `JPM`, `APC_OLD`, `SAN.MC`). **0 errores.**
- **B3 (`calendar/trends`)**: 938/938 tickers con version `calendar`/`trends` archivada. 933 + 5 (mismos 5 de la verificacion previa). **0 errores.**
- **B7 (`sec-form4`)**: **715/938 tickers (76%) con version `sec-form4`/`full` archivada, 223 con error (24%).** Desglose de los 223: 35 son los tickers no estadounidenses (esperado, Form 4 es exclusivo de EEUU -- coincide exactamente con los 35 tickers con sufijo de bolsa de los 938), 18 son los tickers con sufijo `_old`/`_old1` (esperado, ese sufijo no es un simbolo valido aqui), y **170 son tickers estadounidenses "normales" que Fundamentals SI reconoce pero sec-filings/form4 NO** -- prácticamente todos ellos empresas fusionadas/adquiridas/renombradas entre 2016-2024 (`CELG`, `CERN`, `ATVI`, `TWTR`, `MXIM`, `XLNX`, `CTXS`, `ESRX`, `MON`, `WFM`, `RAI`, `PXD`, `CXO`, `NBL`, `FRC`, `SIVB`, `LEH`, y el resto de la lista completa queda en el log de ejecucion): confirma que este endpoint (documentado como "beta") solo reconoce simbolos de emisores VIVOS bajo ese ticker exacto hoy, a diferencia de Fundamentals, que conserva el archivo aunque la empresa ya no exista bajo ese simbolo. No es un fallo de este script.
  - De los 715 archivados, 3 (`AAPL`, `MSFT`, `JPM`) tienen DOS versiones guardadas: una de una verificacion manual anterior al descubrir el bug de paginacion (incompleta) y otra posterior ya corregida (completa) -- `latestFor()` devuelve siempre la mas reciente (la correcta); la version incompleta queda preservada sin mas, que es precisamente el proposito de esta tabla (nunca destruir una captura).
  - Discrepancias menores SIN relacion con `--max-pages` (el limite de 20 paginas nunca se alcanzo: el maximo real fue 9 paginas, `ZION`): un puñado de tickers archivaron entre 1 y 31 filings menos que el `meta.total` declarado por EODHD en la misma respuesta (el caso mas grande, `XOM`, 297/328). Causa no diagnosticada (posible cambio de `meta.total` entre paginas o deduplicacion no perfectamente reversible en el backend de EODHD); documentado con un aviso explicito por ticker en el log, no oculto.
  - Coste: 4.039 paginas pedidas en el lote completo (~40.390 unidades), mas 938+938 unidades de B2/B3 (1 por ticker cada uno). Cuota total consumida HOY (acumulada desde el Bloque B1 de la entrada anterior, que empezo el dia en ~9.380): **54.315 de 100.000**, con margen de sobra para el resto de bloques del plan si se decide continuar.

### Incluye

Nuevo: `src/Providers/EodhdCalendarProvider.php`, `src/Providers/EodhdSecFilingsProvider.php`, `bin/archive-eodhd-calendar-earnings.php`, `bin/archive-eodhd-calendar-trends.php`, `bin/archive-eodhd-sec-form4.php`, `tests/Providers/EodhdCalendarProviderTest.php`, `tests/Providers/EodhdSecFilingsProviderTest.php`. No se toca `eodhd_raw_fundamentals` (tabla legacy) ni ningun consumidor actual.

### Verificado en ddev con

`ddev exec vendor/bin/phpunit`: **573 tests, 1.604 assertions, OK** (1 skip preexistente sin relacion). `ddev exec vendor/bin/phpstan analyse`: sin errores (266 ficheros), limpio. Archivado real de los 938 tickers para los tres endpoints ejecutado y verificado por conteo/cobertura en la base de datos como se detalla arriba (no simulado).
...
### Por que alertas absolutas y no percentil sectorial

El diseño original de Codex para D1 ("Apta/Debil/No evaluable" por percentil dentro del sector, como `RelativeFundamentalScorer` en el backtest) resulto inviable para una ficha individual EN VIVO: `daily_rankings` (poblada por `bin/analyze.php`) no guarda sector ni ratios crudos en su payload JSON (`AnalysisJsonPresenter::ranking()`, verificado), y pedir fundamentales de todo el sector en cada carga de ficha seria lento y caro -- construir ese pipeline no se decidio hacer hoy. Se adopto en su lugar la propia recomendacion de Codex en su documento de resultados ("P2 -- fundamentales utiles sin mezclarlos todavia en el score"): alertas de distress con umbrales absolutos bien establecidos (patrimonio no positivo, caja persistentemente negativa, apalancamiento no interpretable), mas los valores brutos mostrados con transparencia, en vez de sintetizar un veredicto que necesitaria datos que no hay a mano.

### D1 -- `Services\FundamentalHealthAssessor` + `DTO\FundamentalHealthAssessment`

Servicio puro (sin red ni DB, mismo patron que `RiskLevelsCalculator`). Bancos/aseguradoras e inmobiliarias (`Services\FundamentalSectorExclusion`, mismas dos cadenas de sector que `Web\SectorLabel`) quedan excluidos por completo: sus ratios no son comparables a los de una industrial, se etiqueta "no se evalua aqui" en vez de forzar los mismos umbrales. Para el resto, tres alertas INDEPENDIENTES (nunca combinadas en una unica puntuacion): endeudamiento no evaluable (`debtToEquity === null` -- documentado como aproximacion: puede ser por patrimonio no positivo o por simple falta de dato, `PointInTimeFundamentalsBuilder` no distingue los dos casos hoy, asi que el campo se llama `endeudamientoNoEvaluable`, no `patrimonioNoPositivo`, para no afirmar una causa concreta que no esta confirmada), FCF negativo, margen operativo negativo. `datosInsuficientes` distingue "no hay informacion" de "no hay alertas" cuando los cuatro factores (ROIC, margen operativo, deuda/patrimonio, conversion de caja) son nulos -- la leccion de P3.3: ausencia de dato nunca debe leerse como ausencia de alerta.

### D2 -- `Services\FundamentalChangeAssessor` + `DTO\FundamentalChangeAssessment`/`FundamentalChangeFactor` + `Enums\FundamentalChangeVerdict`

Compara el `Fundamentals` actual contra el snapshot de hace ~365 dias via `FundamentalsHistoryRepository::findAsOf()` (el mismo metodo que usa el backtest point-in-time). Cuatro factores candidatos (margen operativo, ROIC, deuda/patrimonio -- aqui mejorar es BAJAR, al reves que el resto --, conversion de caja): un factor se EXCLUYE de la comparacion si falta el dato en cualquiera de las dos fechas, nunca se rellena con cero. Clasificacion (`Mejorando`/`Estable`/`Deteriorando`/`No evaluable`) por MAYORIA DE SIGNO entre los factores disponibles, no por magnitud con un umbral inventado -- pedido explicitamente por `auditor-estadistico` hoy mismo ("un umbral de magnitud añade un parametro libre que invita a p-hacking retroactivo"), con un epsilon de 0,001 solo para descartar ruido de redondeo, nunca para filtrar cambios pequeños de verdad. Con menos de 2 factores disponibles (sin snapshot de hace un año, o solo 1 factor comparable), el veredicto es `No evaluable` -- se expone igualmente la lista de factores comparados (aunque tenga 0 o 1 elemento) para que la vista muestre que se comparo.

### Wiring: `Web\StockDetailPage` + `Services\Application::renderDetail()`

D1 se calcula dentro de `StockDetailPage::render()` (puro, sin nuevo parametro). D2 llega ya calculado como nuevo parametro opcional (`?FundamentalChangeAssessment $fundamentalChange = null`, añadido al FINAL de la firma de `render()` para no romper las llamadas posicionales existentes de los tests) porque necesita `FundamentalsHistoryRepository`, que `Application::renderDetail()` ya tenia instanciado. Best effort: un fallo en D2 (o simplemente no haber snapshot de hace un año) nunca tumba la ficha, cae a `null` y el panel simplemente omite esa seccion.

Nuevo panel "Diagnostico fundamental", situado justo despues de "Sobre la empresa": para sector excluido, una unica nota (D1 y D2 comparten el mismo criterio de exclusion, no se repite el aviso dos veces); si no, "Salud actual" (valores brutos + alertas) y "Cambio interanual" (veredicto + detalle de cada factor comparado). **Redaccion deliberadamente descriptiva, nunca prescriptiva** (pedido por `analista-mercado`): "el margen operativo cae X puntos frente a hace un año", nunca "vigilar la tesis" ni "revisar la salida" -- esa fue precisamente la critica de hoy al Bloque F del plan de Codex, que si implicaba una ventaja de decision con los mismos datos que ya se probaron nulos.

### Incidente durante la implementacion: limite de sesion de la API

El primer intento de implementar esto (un agente `desarrollador-php`) fallo a mitad de camino por un limite de uso de la sesion de Claude ("hit your session limit", HTTP 429). Dejo creados sin terminar: los dos DTO, el enum, `FundamentalSectorExclusion` y los dos servicios -- todos sintacticamente validos y revisados linea a linea antes de continuar. Claude completo el resto directamente (sin relanzar otro agente, para no arriesgar el mismo corte): renombro `patrimonioNoPositivo` a `endeudamientoNoEvaluable` (el nombre original afirmaba una causa concreta -- patrimonio negativo -- que el propio diseño reconocia no poder confirmar), escribio los tests de ambos servicios (18 tests, cubriendo sector excluido, cada alerta de D1 por separado, datos insuficientes, D2 sin historico, D2 con un unico factor, D2 con mayoria mejorando/empeorando/empate, y que deuda/patrimonio mejora al BAJAR), y realizo el wiring completo en `StockDetailPage`/`Application`.

### Verificado con

`ddev exec vendor/bin/phpunit`: **599 tests, 1.656 assertions, OK** (1 skip preexistente sin relacion). `ddev exec vendor/bin/phpstan analyse`: sin errores (278 ficheros). Verificacion visual con Chrome headless real: ficha de WFC (Financial Services) muestra solo la nota de exclusion de sector; ficha de AAPL muestra "Salud actual" (ROIC y conversion de caja en "-", sin inventar dato; margen operativo 32,62%; deuda/patrimonio 0,78; "Sin alertas de distress") y "Cambio interanual: Mejorando" (margen operativo 32,62% ahora vs 31,87% hace un año, deuda/patrimonio 0,78 ahora vs 1,54 hace un año -- ROIC y conversion de caja excluidos de la comparacion por falta de dato en una de las dos fechas). Confirmado que `ScoreCalculator`, `Score` y `config/weights.php` no se tocaron en ningun momento.
...
**Correccion del 2026-09-06 (`v2.114`, mas abajo en este fichero): la afirmacion de "lista" de esta entrada era PREMATURA.** Una auditoria independiente de Codex encontro que `store()` seguia usando `INSERT IGNORE` contra una clave que incluye `payload_hash`, asi que una recaptura con contenido identico se descartaba en silencio (indistinguible de "nunca se recaptura") y una secuencia A->B->A perdia la tercera captura, dejando que `latestFor()` devolviera un estado obsoleto. `allPayloadsFor()`/`latestFor()`/`store()` y el propio `bin/compare-eodhd-calendar-versions.php` descritos aqui abajo se reescribieron en `v2.114` para corregirlo. El detalle completo (causa raiz, diseno de la tabla de observaciones nueva, tres tests de aceptacion A->A/A->B/A->B->A) esta en esa entrada, no se repite aqui.
...
## 2026-09-05 (octava entrada) - Cierre definitivo de la via "transformar `sma20-sma50` dentro del score": cuarta repeticion, mismo veredicto nulo

Estado: medido (`trader-tendencia`), auditado (`auditor-estadistico`), cero cambios en `src/`.

El usuario pide continuar con backtesting propio del motor. `roadmap.md` (linea 197) todavia describia una escala continua sobre el spread SMA20-SMA50 como "la via mas prometedora, sin medir todavia" -- una nota que quedo desactualizada tras cerrarse exactamente esa idea el `2026-08-22` (ver entradas "tercera"/"cuarta" de ese dia, mas abajo en este fichero) y que nadie corrigio entonces: un bug de documentacion real, no de codigo, que hizo que se volviera a pedir la misma medicion.

**Cuarta repeticion (`trader-tendencia`, hoy):** rampa lineal continua sobre `(sma20-sma50)/precio` declarada ANTES de medir (saturacion en ±8%, calibrada sobre la distribucion del propio spread -- percentiles p10/p90 de 449.208 observaciones reales -- no sobre los retornos que motivaron la hipotesis), parche temporal a `TechnicalScoreAnalyzer.php:104-117` revertido con `git checkout` al terminar. Medida con `runCrossSectional()` pareado por fecha en los 6 universos ya usados en esta saga (`largecap60`, `ibex35`, `healthcare`, `industrials`, `energy`, `consumer`) y 4 horizontes (5/20/40/60 sesiones) = 24 combinaciones nuevas. **0/24 cruza el umbral Bonferroni de esta ronda (|t|>=3,08); 1/24 cruza el umbral nominal sin corregir** (`largecap60` h=60, t=-2,09) **en direccion adversa** (empeora, no mejora) y sin sostenerse en el split de mitades (t=-1,65/-1,40, mismo signo, ninguna mitad significativa). En 23/24 combinaciones menos del 50% de las fechas individuales mejoran (35,1%-47,4%) -- el mismo patron de "media arrastrada por fechas extremas" que este proyecto trata como alarma de falso positivo desde el `2026-08-15`, no como confirmacion.

**Auditoria agregada (`auditor-estadistico`):** contando solo la familia de tests con la metrica estandar del proyecto (t pareado por fecha via `runCrossSectional()`) sobre esta UNA variable subyacente a lo largo de cuatro rondas -- invertir el flag (`2026-08-21`, 18 combinaciones), rampa continua v1 (`2026-08-22`, 20), piloto solo-`healthcare` (`2026-08-22`, 10: dos variantes x 5 horizontes, cifra corregida de 8 a 10 respecto al resumen inicial) y rampa continua v2 (hoy, 24) -- se acumulan **72 comparaciones**. De esas 72, exactamente **1** cruza |t|>=1,96, y en sentido contrario al buscado. Ningun umbral razonable (Bonferroni acumulado ≈3,4-3,5, o Benjamini-Hochberg sobre las 72) rescata ese unico resultado.

**Correccion (`Codex`, revision independiente, `2026-09-06`):** las 72 comparaciones NO son independientes entre si -- comparten tickers, fechas y horizontes solapados entre las cuatro rondas -- asi que citar "menos falsos positivos que los 3,6 esperados por puro azar" (formulacion que aparecia aqui hasta hoy) es una lectura invalida del binomio de comparaciones independientes sobre un conjunto que no lo es. No rescata la señal: el veredicto de cierre se mantiene, pero la conclusion defendible es mas modesta -- "las variantes probadas no aportaron una mejora reproducible", no un argumento cuantitativo sobre frecuencia de falsos positivos. Ademas, la ronda de hoy (rampa continua v2) no dejo script/configuracion/JSON congelados tras revertir el parche temporal, asi que sus cifras son auditables solo desde esta narracion, no reproducibles de forma independiente -- limitacion real, anotada para que futuras rondas de medicion conserven el artefacto (aunque sea en `storage/scratch/`, no comprometido) hasta que la auditoria lo revise.

**La quinta transformacion pendiente (bucket discreto "death-cross ancho vs golden-cross", anotado el `2026-08-27`, p≈0,0014 de consistencia de signo en 30 combinaciones) NO se prueba, y se retira de "Ideas adicionales sugeridas" sin medirla dentro del score real.** Dos motivos: (1) su significancia viene de cambiar de metrica (test de signos binomial en vez del t pareado que fallo cuatro veces seguidas) DESPUES de que la metrica estandar no diera nada -- sospechoso de seleccion de metrica a posteriori, exactamente lo que este proyecto evita; (2) las 30 combinaciones detras de ese p-valor comparten fechas correladas entre horizontes solapados y tickers muy repetidos entre universos sectoriales, violando la independencia que el test binomial asume -- el mismo problema de no-independencia que ya se corrigio para el squeeze de Bollinger el `2026-08-22`, por lo que ese p≈0,0014 probablemente esta inflado.

**Se cierra la via completa: ninguna transformacion de `sma20-sma50` (binaria, invertida, continua, discreta) se implementa, y no se reabre sin una hipotesis genuinamente distinta o un cambio material de universo/regimen -- no con una quinta variante de la misma señal.** `config/weights.php` no se toca. `TechnicalScoreAnalyzer.php` sin cambios (`git status`/`git diff --stat` confirmados limpios tras revertir el parche); `ddev exec vendor/bin/phpunit --filter TechnicalScoreAnalyzer` en verde (7 tests, 21 assertions) tras el revert. Corregidas a la vez las tres notas que citaban esta idea como pendiente (para que no se repita una quinta vez): `roadmap.md` (linea 197), `.claude/agents/trader-tendencia.md` (su "investigacion pendiente mas obvia") y la entrada del bucket discreto en "Ideas adicionales sugeridas" de este mismo fichero.

---

## v2.113 - La version mostrada en el Home llevaba desde `v2.104` sin sincronizar

Estado: implementado.

El usuario nota que el numero de version de la esquina superior derecha del Home (`DashboardPage::APP_VERSION`) no parece la actual. Confirmado: llevaba en `v2.104` (`2026-08-25`) pese a que la ultima version numerada real es `v2.112` (`2026-09-01`) y, ademas, desde entonces (`2026-09-02` en adelante) el propio `versions.md` dejo de asignar numeros de version a las entradas nuevas -- paso a fechar cada entrada del plan de EODHD/Codex (`2026-09-02` a `2026-09-05`, once entradas) sin que nadie lo decidiera explicitamente ni sincronizara la constante, exactamente el riesgo que el propio comentario de la clase ya advertia ("se ha quedado desactualizada mas de una vez").

En ese hueco sin numerar se implementaron cambios reales de produccion, no solo investigacion: el archivo versionado de EODHD (Bloque A), el archivo crudo de Fundamentals v1.1/Calendar/SEC Form 4/listas de simbolos (Bloque B1-B3-B6-B7), la seccion "Diagnostico fundamental" nueva en la ficha de detalle (Bloque D1/D2), `earnings_events` normalizado (Bloque C) y varias correcciones de UI (acentos/ñ en toda la aplicacion, BUY/HOLD/SELL traducidos, buscador del Home con fallback a Yahoo, pan vertical del grafico de velas, badge SELL/STRONG SELL, historial de operaciones paginado). Retomar la numeracion con `v2.113` para esta entrada deja la constante sincronizada con el estado real; el detalle completo de cada pieza sigue en sus propias entradas fechadas (`2026-09-02` a `2026-09-05`), no se duplica aqui.

`DashboardPage::APP_VERSION` pasa de `'v2.104'` a `'v2.113'`. Sin cambios de comportamiento, solo el literal mostrado.

## v2.114 - El archivo versionado de EODHD ya no confunde "recapturado identico" con "nunca recapturado", y A->B->A no pierde el estado real

Estado: implementado y probado.

Objetivo: corregir un bug real de correccion, no de estilo, senalado por una auditoria independiente de Codex sobre el commit `d608747` (herramientas de comparacion temporal para `calendar/earnings`/`calendar/trends`, `2026-09-05`; ver `RESULTADOS_OPTIMIZACION_MOTOR_CODEX_2026-09-05.md`, seccion P0 punto 2). Ese commit, y la entrada de este mismo fichero fechada `2026-09-05 (septima entrada)`, afirmaban que la herramienta de comparacion "estaba lista" -- **esa afirmacion era prematura**, corregida aqui.

Causa raiz encontrada: `eodhd_raw_fundamental_versions` (migracion 025) deduplica BLOBS por `payload_hash` con una clave unica `(ticker, api_version, section, payload_hash)`, y `EodhdRawFundamentalVersionsRepository::store()` usaba `INSERT IGNORE` contra esa misma clave. Dos consecuencias reales:

1. **"Se recapturo y era identico" era indistinguible de "nunca se ha vuelto a capturar".** Si la misma captura (contenido byte a byte identico) se repetia, la segunda insercion se ignoraba en silencio y solo quedaba una fila -- exactamente la misma fila que si nunca se hubiera vuelto a pedir el dato a EODHD. `bin/compare-eodhd-calendar-versions.php` (el comparador creado en `d608747`) contaba versiones via `allPayloadsFor()`, que leia esa misma tabla deduplicada: su contador "identico" era matematicamente inalcanzable con el guardado normal, y el test nuevo de esa entrega solo probaba dos payloads DISTINTOS, por lo que nunca ejercito este caso.
2. **Una secuencia A->B->A perdia la tercera captura y corrompia `latestFor()`.** Si el valor cambiaba de A a B y despues volvia a A (p.ej. una revision de EODHD que se deshace), la tercera captura (A) colisionaba por hash con la primera fila ya existente y `INSERT IGNORE` la descartaba. `latestFor()` resolvia por la fila de blob mas reciente (`ORDER BY fetched_at DESC`), asi que devolvia B como "el mas reciente" pese a que el ultimo estado REAL observado fuera A. Cualquier consumidor futuro de `latestFor()` (normalizacion de `earnings_events`, el propio E2/E3 que este comparador queria desbloquear) habria quedado alimentado con un dato obsoleto sin ningun aviso.

Decisiones de arquitectura:

- Los BLOBS siguen deduplicados por `payload_hash` en `eodhd_raw_fundamental_versions` (sin cambios de forma en esa tabla): no tiene sentido volver a comprimir y guardar el mismo JSON dos veces.
- Tabla nueva `eodhd_raw_fundamental_version_observations` (migracion 027, `ON DELETE CASCADE` hacia la version): registra **cada peticion exitosa** con `version_id`, `ticker`/`api_version`/`section` (denormalizados para no necesitar JOIN en la consulta mas frecuente), `observed_at_utc`, `http_status`, `source_symbol` y `request_from`/`request_to` (la ventana de fechas pedida a EODHD, cuando el endpoint la admite -- hoy solo `calendar/earnings`). Esta tabla NUNCA deduplica: dos observaciones identicas dejan dos filas, con su propio `observed_at_utc` cada una. La migracion incluye el script de datos que convierte cada fila YA archivada en su primera observacion, usando el `fetched_at` existente como `observed_at_utc`.
- `EodhdRawFundamentalVersionsRepository::store()` sustituye `INSERT IGNORE` por `INSERT ... ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID(id)` (una unica sentencia atomica: inserta el blob si es nuevo, o dirige `LAST_INSERT_ID()` al id del blob ya existente sin perderlo) seguido, en la misma transaccion, de un `INSERT` incondicional en la tabla de observaciones. `rowCount()` de la primera sentencia distingue de forma fiable blob nuevo (1 fila afectada) de contenido duplicado (0, MySQL documenta que una fila "actualizada" a sus mismos valores no cuenta como afectada). El metodo ya no devuelve `void`: devuelve `DTO\EodhdFundamentalVersionStoreResult` (`versionId`, `observationId`, `Enums\EodhdFundamentalVersionStoreOutcome::NEW_VERSION|DUPLICATE_CONTENT`), siguiendo el patron de DTOs ya establecido en el proyecto -- ası un llamador ya no tiene que adivinar con un `count()` antes/despues (`bin/backfill-eodhd-fundamental-versions.php` hacia exactamente eso; se simplifica para usar `hasVersion()` como guardia de idempotencia, igual que el resto de archivadores de este directorio).
- `latestFor()` resuelve ahora por `MAX(observed_at_utc)` de la tabla de observaciones (JOIN contra el blob referenciado), no por la fila de blob mas reciente: en A->B->A, la ultima OBSERVACION apunta de vuelta al blob de A aunque ese blob se insertara antes que el de B.
- `allPayloadsFor()` devuelve ahora una fila POR OBSERVACION (nunca deduplicadas), no una fila por blob distinto -- es el cambio que arregla de raiz el bug del comparador: con esta forma, `count($observaciones) < 2` significa de verdad "nunca recapturado", y el conjunto completo de hashes observados (no solo los dos extremos) permite distinguir "siempre identico" de "cambio en algun punto de la secuencia", incluido un A->B->A que vuelve al valor inicial.
- `bin/compare-eodhd-calendar-versions.php` (Bloque E2/E3) se reescribe sobre esta nueva forma: cuenta observaciones (no blobs) para "nunca recapturado"; compara el conjunto COMPLETO de hashes de la secuencia (no solo el primero contra el ultimo) para "identico" vs "con al menos un cambio"; y recorre cada TRANSICION CONSECUTIVA de la secuencia (no solo el extremo mas antiguo contra el mas reciente) para poder reconstruir un A->B->A completo con sus dos transiciones reales.
- El diff de `earnings` del mismo script recorria solo los periodos fiscales de la captura MAS RECIENTE (`foreach ($newEvents as ...)`), por lo que un periodo que desaparece entre dos capturas nunca se habria detectado. Se corrige para recorrer la UNION de periodos fiscales de ambas capturas e informar anhadidos/eliminados/cambiados por separado (`EodhdEarningsEventsNormalizer::parse()` en si mismo ya era correcto -- deduplica por periodo fiscal DENTRO de un unico payload, que es un problema distinto; no necesito cambios).

Incluye:

- `database/migrations/027_create_eodhd_raw_fundamental_version_observations.sql` (tabla nueva + migracion de datos de las filas ya archivadas).
- `src/Enums/EodhdFundamentalVersionStoreOutcome.php`, `src/DTO/EodhdFundamentalVersionStoreResult.php` (nuevos).
- `src/Repository/EodhdRawFundamentalVersionsRepository.php`: `store()` transaccional con resultado explicito, `latestFor()`/`allPayloadsFor()` reescritos sobre observaciones.
- `bin/compare-eodhd-calendar-versions.php`: contadores corregidos, diff por UNION de periodos fiscales, recorrido de transiciones consecutivas.
- `bin/backfill-eodhd-fundamental-versions.php`: guardia `hasVersion()` en vez de `count()` antes/despues (ademas de mas simple, evita que una repeticion futura de este backfill duplique observaciones -- ya no hay `INSERT IGNORE` que lo hiciera gratis).
- `bin/archive-eodhd-calendar-earnings.php`: pasa `request_from`/`request_to` a `store()`.
- `tests/Integration/EodhdRawFundamentalVersionsRepositoryTest.php`: 4 tests nuevos, incluidos los tres criterios de aceptacion LITERALES que pidio Codex -- `testSecuenciaAaConservaUnBlobPeroDejaDosObservaciones` (A->A: un blob, dos observaciones), `testSecuenciaAbConservaDosBlobsYDosObservaciones` (A->B: dos blobs, dos observaciones, `latestFor()` devuelve B) y `testSecuenciaAbaDejaTresObservacionesYLatestForDevuelveElUltimoEstadoReal` (A->B->A: solo dos blobs distintos pero tres observaciones, `latestFor()` devuelve A -- el ultimo estado real, no B).
- `tests/Integration/IntegrationTestCase.php`: `eodhd_raw_fundamental_version_observations` anhadida a la lista de tablas que se vacian antes de cada test.
- Corregidas las afirmaciones de "lista" en `versions.md` (entrada `2026-09-05`, septima) y `roadmap.md` (Bloque E2/E3) con una nota que apunta a esta entrada -- no se borra el texto anterior, se anota como incompleto en su momento.

Verificado en ddev con `vendor/bin/phpunit tests/Integration/EodhdRawFundamentalVersionsRepositoryTest.php` (18/18 tests, 79 assertions, OK -- incluye los 3 tests de aceptacion A->A/A->B/A->B->A) y `vendor/bin/phpstan analyse` (0 errores nuevos; el unico error del arbol en el momento de esta verificacion es preexistente y ajeno en `src/Services/BacktestingService.php`, de una investigacion de RSI(2) en curso en paralelo, no tocada aqui). No se ha ejecutado la suite completa de PHPUnit a proposito (aviso explicito de que hay un parche temporal sin revertir de esa investigacion paralela en `TechnicalAnalyzer`/`TechnicalSnapshot`/`BacktestingService`); la suite completa la corre quien cierre esa investigacion.

Resultado esperado: cuando dentro de unas semanas se repita `bin/archive-eodhd-calendar-earnings.php --force`/`bin/archive-eodhd-calendar-trends.php --force` (ver `roadmap.md`), cada captura real -- cambie o no el contenido -- deja su propia observacion, `latestFor()` siempre refleja el ultimo estado observado de verdad (nunca un blob "mas nuevo" que en realidad es una version antigua que volvio a aparecer), y `bin/compare-eodhd-calendar-versions.php` puede por fin contestar de forma correcta la pregunta que bloquea E2/E3 (¿el consenso que hoy parece historico es un vintage congelado, o EODHD lo reescribe?), incluida la posibilidad de que se reescriba y luego se revierta. No cambia nada del universo/score/pesos de produccion.

---

## 2026-09-06 - Correccion de Codex sobre el diagnostico fundamental D1/D2 (revision de solo lectura de `7f91aef`): dos bugs funcionales y seis precisiones

Estado: implementado. Verificacion parcial (ver "Verificado con" para el detalle honesto de que se pudo y que no se pudo ejecutar en este entorno).

Objetivo: cerrar la revision de solo lectura que Codex hizo sobre el commit `7f91aef` (D1 "Salud fundamental" + D2 "Cambio interanual", ver la entrada `2026-09-05` cuarta de este mismo fichero), documentada en `RESULTADOS_OPTIMIZACION_MOTOR_CODEX_2026-09-05.md` seccion "P2 -- diagnostico fundamental": dos bugs funcionales reales mas seis correcciones de precision, todas explicitamente sin conectar D1/D2 a `ScoreCalculator`/`Score`/`config/weights.php` ni implementar percentil sectorial -- ambas cosas siguen fuera de alcance, decision ya tomada y confirmada por el equipo el 2026-09-05.

Causa raiz encontrada:

- **Bug 1 (el mas importante).** `FundamentalHealthAssessor::assess()` calculaba `datosInsuficientes` mirando solo cuatro de los cinco factores de D1 (`roic`, `operatingMargin`, `debtToEquity`, `cashConversion`), excluyendo `freeCashFlow`. Cuando el FCF era el UNICO dato conocido (negativo, el resto null), `datosInsuficientes=true` Y `fcfNegativo=true` a la vez, y `StockDetailPage::renderFundamentalHealthSection()` retornaba en cuanto veia `datosInsuficientes`, sin llegar a leer `fcfNegativo`: el caso mas informativo (FCF negativo conocido) quedaba oculto detras del menos informativo (datos insuficientes). El test existente (`testFcfNegativoSeSeñala`) no lo detectaba porque anadia tambien `roic`, lo que evitaba `datosInsuficientes=true` y enmascaraba el bug.
- **Bug 2.** `FundamentalChangeAssessor::assess()` compara el `Fundamentals` "actual" (que llega del proveedor de mercado ACTIVO -- Yahoo hoy, FMP si se cambia la configuracion) contra `fundamentals_history`, reconstruido ENTERAMENTE desde EODHD. El veredicto `Mejorando/Deteriorando` podia estar midiendo una diferencia de proveedor y de formula de calculo, no (solo) un cambio real de la empresa, sin ningun aviso visible al respecto.

Decisiones de arquitectura:

- **`datosInsuficientes` ahora exige que los CINCO factores sean null**, incluido `freeCashFlow`. Un FCF negativo conocido ya cuenta por si solo como "dato suficiente" para disparar su propia alerta, sin depender de que el resto de factores tambien tengan dato -- no se anadio ningun flag ni contador nuevo, solo se amplio el criterio de suficiencia ya existente al quinto factor que faltaba.
- **Sector enriquecido, no el sector base.** `StockDetailPage::render()` ahora calcula D1 (`FundamentalHealthAssessor::assess()`) sobre `$companyProfile ?? $company` en vez de `$company` a secas, y `Application::renderDetail()` pasa `$companyProfile` (ya calculado unas lineas antes via `YahooCorporateProfileProvider::fetchCached()`) a `FundamentalChangeAssessor::assess()` para D2, en vez de `$analysis->getStock()->getCompany()`. Motivo real, no hipotetico: `FmpParser` nunca rellena `sector` (siempre `''`), y `YahooParser` solo lo rellena si el modulo `assetProfile` respondio; un sector vacio nunca coincide con `FundamentalSectorExclusion::EXCLUDED_SECTORS`, asi que una entidad financiera o inmobiliaria dejaria de excluirse por simple ausencia de dato del proveedor activo, no porque sus ratios fueran realmente comparables. `$companyProfile` ya es el `Company` "enriquecido" que el resto de la ficha usa para "Sobre la empresa" (sector con fallback al de `$company` si Yahoo tampoco lo tenia, ver `YahooCorporateProfileProvider::fetch()`), asi que D1/D2 pasan a usar exactamente la misma fuente de sector que ya usaba el resto de la pagina.
- **`ESTABLE` y `MIXTO` son veredictos distintos.** `Enums\FundamentalChangeVerdict` gana el caso `MIXTO` ('Mixto'). `FundamentalChangeAssessor::classify()` distinguia antes por `default => ESTABLE` tanto "ningun factor cambio" como "empate real entre mejora y deterioro"; ahora `ESTABLE` exige `$improved === 0 && $worsened === 0` (ningun factor con cambio real, incluido ruido de redondeo via `FundamentalChangeFactor::NOISE_EPSILON`) y cualquier otro empate (`$improved === $worsened > 0`) cae en `MIXTO`.
- **Fecha real del snapshot anterior, con limite de antiguedad.** `FundamentalsHistoryRepository` gana `findAsOfWithDate()` (devuelve `{payload, snapshotDate}`), y `findAsOf()` pasa a ser un metodo fino sobre el (`$this->findAsOfWithDate(...)['payload'] ?? null`) para no duplicar la consulta SQL ni cambiar el contrato del que ya dependen `BacktestingService`/`FundamentalDeteriorationFlagger` -- ningun consumidor existente de `findAsOf()` se toca. `FundamentalChangeAssessment` gana `?DateTimeImmutable $previousSnapshotDate`, poblado por `FundamentalChangeAssessor::assess()` con la fecha REAL devuelta por `findAsOfWithDate()`, nunca asumida. Se anade `MAX_SNAPSHOT_AGE_DAYS = 730` (2 anhos, criterio propio documentado en el codigo, no una cifra calibrada estadisticamente): si el snapshot mas cercano disponible supera esa antiguedad respecto a `$asOf`, se trata como "no evaluable" pero conservando la fecha real descartada en el DTO (`noEvaluableResult($previousSnapshotDate)`), para que la vista pueda explicar el motivo exacto en vez de decir "sin historico" cuando si existe uno, solo que demasiado viejo.
- **D2 nunca omite en silencio.** `StockDetailPage::renderFundamentalDiagnostics()` mostraba antes `''` para D2 si `$fundamentalChange === null` (best-effort de `Application::renderDetail()` al fallar el calculo). Ahora siempre renderiza "Cambio interanual no disponible." en ese caso.
- **Aviso de comparabilidad de proveedor visible.** `StockDetailPage::renderFundamentalChangeSection()` anade una nota fija junto a cualquier veredicto real (no en los casos "no evaluable" sin factores, donde no hay nada que interpretar): "Compara datos del proveedor de mercado activo con un historico reconstruido desde otro proveedor (EODHD); el veredicto puede reflejar diferencias de formula o fuente, no solo un cambio real de la empresa." Documentado tambien en el docblock de `FundamentalChangeAssessor`.
- **Umbrales de D1 documentados como heuristicas de distress, no formulas de rentabilidad.** Nuevo parrafo en el docblock de `FundamentalHealthAssessor` aclarando explicitamente que FCF negativo/margen operativo negativo/endeudamiento no evaluable son señales de alerta descriptivas, nunca una formula validada de retorno esperado ni un criterio de compra/venta.
- El percentil sectorial sigue sin implementarse (aplazado explicitamente, sin cambios en esta tarea) y `EodhdRawFundamentalVersionsRepository`/migraciones/`bin/compare-eodhd-calendar-versions.php` no se tocan (trabajo disjunto de otro agente en paralelo, confirmado por los cambios reales encontrados en este mismo fichero durante la tarea).

Incluye:

- Modificados: `src/Services/FundamentalHealthAssessor.php`, `src/DTO/FundamentalHealthAssessment.php`, `src/Repository/FundamentalsHistoryRepository.php`, `src/DTO/FundamentalChangeAssessment.php`, `src/Enums/FundamentalChangeVerdict.php`, `src/Services/FundamentalChangeAssessor.php`, `src/Web/StockDetailPage.php`, `src/Services/Application.php`.
- Tests modificados: `tests/Services/InMemoryFundamentalsHistoryRepository.php` (nuevo metodo `findAsOfWithDate()`), `tests/Services/FundamentalHealthAssessorTest.php` (nuevo test del caso exacto del Bug 1: FCF negativo como UNICO dato conocido), `tests/Services/FundamentalChangeAssessorTest.php` (el antiguo `testEmpateDaVeredictoEstable` se divide en un test de `MIXTO` real y uno de `ESTABLE` real; nuevos tests de fecha real expuesta y de snapshot demasiado antiguo).
- Test nuevo: `tests/Web/StockDetailPageFundamentalDiagnosticsTest.php` (integracion de pagina completa: FCF negativo como unico dato, sector enriquecido excluido con sector base vacio, ausencia de historico, los cinco veredictos posibles, aviso de comparabilidad de proveedor, fecha real mostrada, y escapado HTML de una etiqueta de factor maliciosa).

Verificado con: **no fue posible ejecutar `ddev`/PHPUnit/PHPStan en este entorno de trabajo** -- el sandbox de esta tarea no tenia Docker ni un interprete PHP instalados (`docker`/`php` ausentes del `PATH`), a diferencia de tareas anteriores de este mismo proyecto que si corrian dentro de `ddev`. Se hizo en su lugar una revision manual linea a linea de cada fichero tocado (tipos, nombres de parametro con argumentos nombrados, orden de constructores, rutas de codigo alcanzables, y un rastreo manual de cada asercion nueva contra la logica exacta de `FundamentalHealthAssessor`/`FundamentalChangeAssessor`/`StockDetailPage`). **Pendiente de confirmar en el entorno real**: `ddev exec vendor/bin/phpunit tests/Services/FundamentalHealthAssessorTest.php tests/Services/FundamentalChangeAssessorTest.php tests/Web/StockDetailPageFundamentalDiagnosticsTest.php tests/Web/StockDetailPageTest.php` y `ddev exec vendor/bin/phpstan analyse`.

Resultado esperado: la ficha de detalle deja de ocultar la alerta de FCF negativo cuando es el unico dato conocido, excluye correctamente entidades financieras/inmobiliarias aunque el proveedor de mercado activo no traiga sector, distingue "sin cambio real" de "empate entre mejora y deterioro", muestra siempre la fecha real del snapshot comparado (o explica por que se descarto por antiguo), nunca omite D2 en silencio si falla, y deja explicito en la propia ficha que la comparacion interanual mezcla dos proveedores de datos distintos. Sigue siendo puramente informativo: `ScoreCalculator`/`Score`/`config/weights.php` no se han tocado.

**Confirmado por Claude en el entorno real (`ddev`) el mismo dia, junto con la correccion de P0** (ver entrada `v2.114` mas arriba): `vendor/bin/phpunit` completo -- **650 tests, 1.800 assertions, OK** (1 skip preexistente, sube desde 631) -- y `vendor/bin/phpstan analyse` -- **sin errores** (291 ficheros). Antes de confiar en el resultado se detecto y corrigio un problema aparte: un proceso concurrente de investigacion (RSI(2), ver entrada siguiente) habia dejado tres ficheros de produccion (`TechnicalAnalyzer.php`, `TechnicalSnapshot.php`, `BacktestingService.php`) con finales de linea CRLF en vez de LF pese a que su contenido era identico a `HEAD` -- mismo bug de inyeccion CRLF ya documentado para ediciones de `versions.md`/`roadmap.md` (`v2.87` y otras), esta vez en codigo de produccion. Corregido con `git checkout` antes de dar la suite por buena.

---

## 2026-09-06 (segunda entrada) - RSI(2) de Connors: unico indicio de esta sesion que no es el falso positivo habitual, ni tampoco una confirmacion

Estado: medido (`analista-mercado`) y auditado (`auditor-estadistico`), cero cambios en `src/`. Ultima candidata sin medir del backlog de "Ideas adicionales sugeridas" (2026-08-27).

Diseño predeclarado: RSI(2) (reversion de muy corto plazo, opuesto al momentum 12-1 ya existente) medido como factor aislado top-10 continuo, sin neutralizar por sector (RSI ya esta acotado 0-100 de forma identica en cualquier sector). 6 universos estandar x horizontes 5/10 sesiones x direccion oversold/overbought = 24 combinaciones, metrica unica t pareado por fecha via `runCrossSectional()`, umbral Bonferroni |t|>=3,078 (calculado con la inversa de la normal). Resultado: **1 de 24 cruza el umbral** (`consumer`/horizonte 5/oversold, t=3,14, alpha +0,21pp), con **mismo signo en ambas mitades** del periodo (t=2,66 y t=1,66) -- a diferencia de todo lo demas medido esta sesion (SMA20/50, fundamentales P3.3/P3.4), aqui NO hay cambio de signo entre eras.

**Auditoria (`auditor-estadistico`):** ni el falso positivo tipico de este proyecto (no cambia de signo) ni una confirmacion robusta (la caida de t=2,66 a t=1,66 entre mitades es la regresion a la media esperable al elegir el maximo de 24 pruebas, no evidencia fuera de muestra; dos mitades con el mismo signo por puro azar ya tiene ~50% de probabilidad bajo la nula). Rechaza explicitamente recalcular el umbral asumiendo pares oversold/overbought (~12, umbral 2,87) *porque* eso haria cruzar dos combinaciones mas -- decidir el denominador de la correccion despues de ver que resultados quedan al borde es el mismo tipo de maniobra que este rol existe para vetar; en cualquier caso no cambia que solo `consumer` sobresale. Confirma un bug real heredado de otro sitio: `sampleHistory()` descarta muestras por `momentum12m1` nulo para cualquier modo salvo `'fundamental'` (mismo defecto que motivo la correccion "Prioridad cero-ter" del 2026-09-04), sesgando la muestra hacia tickers con mas historico aunque RSI(2) no necesite esa señal -- hay que corregirlo antes de medir mas. Señala ademas una rotura de disciplina propia: esta es la unica investigacion de la sesion que no dejo ningun script/log reproducible en `bin/`/`storage/scratch/`.

**Veredicto: no se adopta ni se cierra.** Se autoriza UN test adicional, estrecho y predeclarado -- no una nueva ronda exploratoria -- restringido a `consumer`/horizonte 5 dentro de `mode='full'` (el score real, no factor aislado), con el fix de `momentum12m1` aplicado primero, misma metrica (t pareado, `welchStdErr()`), y contado explicitamente como la prueba numero 25 de esta misma idea (no una serie nueva desde cero). El diseño de dos grupos evento/universo que sugirio `analista-mercado` como posible mejora queda descartado para este test: es una hipotesis nueva y separada si se quiere probar mas adelante, no una mejora gratuita de la prueba ya predeclarada.

---

## 2026-09-06 (tercera entrada) - P1 de Codex (separar evidencia de retorno del indicador de riesgo): decision del usuario, no se implementa

Codex proponia en `RESULTADOS_OPTIMIZACION_MOTOR_CODEX_2026-09-05.md` (seccion "P1") dejar de mostrar `COMPRAR`/`MANTENER`/`VENDER` como si tuvieran ventaja de retorno demostrada -- nunca validada en ningun backtest de este proyecto -- y sustituirlo por un estado de evidencia (`return_evidence_status`, hoy siempre `INSUFICIENTE`) separado de un indicador de riesgo bajista basado directamente en volatilidad20/ATR14 (el unico hallazgo con valor real que dejo P4). Antes de decidir, se construyo un mockup visual (Artifact, datos reales de AAPL, mismos tokens de color y clases de componente que la app real) comparando la cabecera actual contra la propuesta.

**Decision explicita del usuario, tras ver el mockup: mantener `Comprar`/`Mantener`/`Vender` tal como estan.** Las etiquetas NO desaparecen. El foco sigue en mejorar el motor de analisis, no en cambiar como se presenta lo que ya mide. El aviso "estas recomendaciones no tienen ventaja demostrada" se queda donde esta hoy (nota al pie de la seccion "Por que..."), no se promueve a mensaje principal. No se toca `StockDetailPage`, `RecommendationLabel` ni `Score::recommendationFor()`. No se reabre esta idea sin una peticion explicita nueva del usuario.

---

## 2026-09-06 (cuarta entrada) - RSI(2) de Connors: prueba 25 y ultima, dentro del score real, veredicto nulo -- via cerrada

Estado: medido (`analista-mercado`), cero cambios en `src/`. Cierra el backlog de "Ideas adicionales sugeridas" del 2026-08-27.

Test unico autorizado por `auditor-estadistico` en la entrada anterior: RSI(2) integrado DENTRO del bloque `MOMENTUM` real de `TechnicalScoreAnalyzer` (puntos por tramo, oversold premiado, overbought sin premiar -- unica direccion con indicio en la ronda de 24 pruebas anteriores), midiendo el score COMPLETO (`mode='full'`) en `consumer`/horizonte 5 con y sin el cambio. Presupuesto de `MOMENTUM` subido temporalmente de 10 a 12 puntos brutos (7+3+2) reescalado por el mecanismo `scale()` ya existente, para no tocar `config/weights.php` y mantener el total del score identico entre ambas corridas.

**Resultado: nulo, con claridad.** El top-10 diario cambio en 434 de 452 fechas (96%) -- el parche si reordena el ranking de verdad, no es un no-op -- pero la alpha pareada por fecha se queda en +0,01pp tanto sin como con RSI(2), y el t pasa de 0,09 a 0,16: muy lejos de 1,96, mucho mas lejos de cualquier correccion por comparaciones multiples. La exigencia de `momentum12m1` en `sampleHistory()` SI aplica correctamente a este caso (a diferencia de la ronda anterior): `mode='full'` si consume esa señal dentro del propio bloque `MOMENTUM`, confirmado tanto en el codigo ("Prioridad cero-ter" mantiene la exigencia para `full`/`technical`/`momentum`) como empiricamente (`samples_dropped_momentum_null=1054` identico en ambas corridas).

**Contexto acumulado: 25 pruebas sobre la misma idea, 1 sola por encima de 1,96 (y esa sin confirmarse robusta por el auditor), 0 dentro del score real.** Se cierra la via: no se implementa, y no se repite con otro universo/horizonte/formula de RSI(2) sin una hipotesis genuinamente distinta (p.ej. sustituir en vez de sumar dentro del mismo hueco de puntos que hoy ocupa RSI14, señalado por el propio `analista-mercado` como observacion de diseño, no como propuesta lista para medir). `config/weights.php` no se toca. Artefactos de la corrida (script de parche + JSON de control/tratamiento) en `storage/scratch/`, no comprometidos.

De las tres candidatas del `2026-08-27` ("Ideas adicionales sugeridas"), quedan dos cerradas con veredicto nulo (bucket SMA20/50, cerrado hoy dentro de la via completa de esa señal; RSI(2), cerrado aqui). **"Consistencia de tendencia Frog in the Pan" sigue sin medir** -- no se ha tocado esta sesion, sigue pendiente en el backlog tal cual quedo anotada el `2026-08-27` (0/12 sobrevive Bonferroni pero 11/12 mismo signo, p≈0,0064, mas fuerte en `healthcare`; requeriria un campo nuevo en `TechnicalSnapshot`).

---

## 2026-09-06 (quinta entrada) - Frog in the Pan medido con la metrica estandar: veredicto nulo, cierra el backlog del 2026-08-27 completo

Estado: medido (`trader-tendencia`), cero cambios en `src/`. Con esta entrada se cierran las TRES candidatas anotadas el `2026-08-27` en "Ideas adicionales sugeridas" (bucket SMA20/50, RSI(2), Frog in the Pan) -- ninguna sobrevive a medirse con la metrica estandar del proyecto (t pareado por fecha via `runCrossSectional()`).

Formula declarada ANTES de medir: `trend_consistency` = % de sesiones con retorno diario positivo dentro de la MISMA ventana que `momentum12m1` (250 sesiones menos las ultimas 21) -- la variante mas directa de las dos que sugeria el backlog original, elegida por ser mas defendible sin ajustar a los numeros ya vistos. Ranking top-10 sin neutralizar por sector (porcentaje ya acotado 0-100). 6 universos estandar x horizontes 20/60 = 12 combinaciones, umbral Bonferroni exacto |t|>=2,8653.

**Resultado: 0/12 cruza el umbral.** Maximo |t|=1,86 (`consumer`/60 sesiones), lejos incluso del umbral nominal sin corregir. Split de mitades calculado igualmente para las 12 (no obligatorio por protocolo al no cruzar nada): inestable donde mas alto sale (`consumer`/60: t=0,76 primera mitad, t=2,10 segunda), no una confirmacion.

**Matiz honesto, distinto de las otras dos vias cerradas hoy:** 11 de 12 t-stats salen con signo positivo (solo `ibex35`/60 es negativo) -- es la PRIMERA vez esta sesion que la metrica estandar REPRODUCE el patron de consistencia de signo que motivo la idea original (2026-08-27, test de signos con p≈0,0064), en vez de contradecirlo (como paso con el bucket SMA20/50) o no mostrar ninguna direccion (como RSI(2)). La direccion se sostiene; la magnitud no llega a ser distinguible del ruido. Bajo la regla de decision predeclarada (cruzar Bonferroni con la metrica estandar es la unica puerta a medir dentro del score real), este matiz NO autoriza un segundo test -- se registra como nota historica, no como excepcion.
...
## 2026-09-06 (septima entrada) - Bug real: demasiados indicadores ausentes se convertian en una recomendacion de venta

Estado: implementado y verificado. Cierra P1 de `MEJORAS_MOTOR_ASTRA_2026-09-06.md` (P0 en la entrada anterior).

**El bug, confirmado:** `TechnicalScoreAnalyzer` rellena cada indicador tecnico ausente con un valor "neutro" (la mitad de sus puntos) para no romper la suma de TECHNICAL+MOMENTUM+RISK -- correcto para UN hueco aislado (una OPV sin 250 sesiones de historico para Momentum 12-1, por ejemplo), pero con TODOS los indicadores ausentes el resultado aterriza EXACTAMENTE en el 50% del maximo (15+5+5 de 30+10+10 = 25 de 50), que `Score::recommendationFor()` clasifica como `SELL`. "No tengo datos" se convertia en una orden de venta.

**Correccion:** nuevo `TechnicalSnapshot::availableIndicatorCount()`/`hasSufficientTechnicalData()` (cuenta cuantos de los 10 indicadores independientes que puntua `TechnicalScoreAnalyzer` tienen dato real; el cruce de medias y las Bandas de Bollinger cuentan como uno cada uno, no dos, porque necesitan el par completo). Umbral: al menos 5 de 10 (la mitad) -- corte simple y defendible que evita el caso extremo (y cualquiera con mayoria de huecos) sin descartar un ticker por faltarle uno o dos indicadores aislados, que es normal.

Nuevo `StockAnalysis::getRecommendation()`: unico punto de entrada para la recomendacion de un analisis EN VIVO. Devuelve `'DATOS_INSUFICIENTES'` si `hasSufficientTechnicalData()` es falso; si no, delega en `Score::getRecommendation()` sin cambios. Deliberadamente NO se toca `Score::recommendationFor()` (formula pura sobre un porcentaje, reutilizada tal cual por `BacktestingService` sobre percentiles historicos que no tienen este concepto de cobertura en vivo) ni `TechnicalScoreAnalyzer` (sigue rellenando neutro para un hueco aislado, que es correcto).

Todos los consumidores del ranking/ficha/cartera/watchlist/API que decidian la recomendacion via `analysis->getScore()->getRecommendation()` pasan a `analysis->getRecommendation()`: `Application.php` (alertas de watchlist y cartera, filtro del ranking), `DashboardPage.php` (badge, contador de candidatas compra, listas de recomendados), `StockDetailPage.php`, `WatchlistPage.php`, `AnalysisJsonPresenter.php` (API). `RecommendationExplainer` mantiene su texto separado para `DATOS_INSUFICIENTES` (nunca inventa señales negativas). `RecommendationLabel::translate()` y `Layout::recommendationClass()` (nueva clase CSS `.insufficient-data`, deliberadamente NI verde/ambar/rojo -- no es un punto en la escala compra-venta) tratan el nuevo valor explicitamente en vez de caer en su `default`, que antes lo pintaba como `SELL` (rojo).

`AlertService::checkRecommendationChange()` ignora `'DATOS_INSUFICIENTES'` por completo: ni genera alerta de "cambio de recomendacion" ni lo guarda como ultimo estado, para que al recuperarse el dato la comparacion siga siendo contra la ultima recomendacion REAL conocida, no contra "sin datos". `checkStopLossBreach()` (P0, entrada anterior) ya era independiente de esto -- solo necesita precio/ATR, no el score completo.

**Fuera de alcance, documentado como limitacion conocida, no una tarea de hoy:** `score_history` (tabla usada para una futura "tendencia del score", todavia sin ningun consumidor que la lea, ver `roadmap.md`) no guarda ninguna recomendacion ni el estado de cobertura de aquel dia -- solo porcentaje/desglose por categoria. Si en el futuro se construye esa funcionalidad, reconstruir retroactivamente si un dia concreto tuvo "datos insuficientes" no sera posible sin guardar tambien la cobertura en ese momento.
...
## 2026-09-06 (decima entrada) - Universo nuevo: MSCI World (~1.251 tickers reales, "solo cron")

Estado: implementado y verificado. El usuario pide un universo mas amplio para meter tickers nuevos y acumular historico con el que investigar mas adelante (`score_history`/`fundamentals_history`, hoy solo EEUU + IBEX35).

**Fuente real, no una lista fabricada de memoria** (verificado por `fiabilidad-datos-mercado` antes de escribir nada): MSCI no publica gratis la composicion completa del indice, EODHD devuelve `Components: {}` vacio para `MSCIWORLD.INDX`/`990100.INDX`/`ACWI.INDX` (200 OK, pero sin datos -- distinto del 401/403 ya visto en otros endpoints), y Yahoo solo expone el top-10 de cualquier ETF via `topHoldings`. La via viable: el CSV oficial de holdings de `URTH` (iShares MSCI World, replica fisica, ID 239696), publicado sin autenticacion en `ishares.com` y descargado el mismo dia -- 1253 filas `Asset Class=Equity` de 1276 totales, 23 paises desarrollados.

**Verificacion de calidad de datos en dos rondas** antes de comprometer nada: 7 bolsas principales primero (Japon, RU, Alemania, Suiza, Francia, Canada, Australia), despues las 16 restantes (Suecia, Paises Bajos, Italia, Hong Kong, Singapur, Israel, Dinamarca, Finlandia, Noruega, Belgica, Portugal, Nueva Zelanda, Irlanda, Austria) -- las 23 dieron cotizacion coherente, ~5 años de historico diario sin huecos anomalos y fundamentales completos contra Yahoo real. Hallazgos concretos que evitaron errores reales: el mapeo tiene que hacerse por columna `Exchange`, no `Location` (ArcelorMittal domiciliada en Francia cotiza en Amsterdam como `MT.AS`; Teva domiciliada en Israel cotiza como ADR en NYSE sin sufijo); los puntos finales de Londres (`BP.`, `RR.`) y los puntos internos de clases de accion (`TECK.B`→`TECK-B.TO`) necesitan conversion; `ROG.SW` (el ticker que se "recordaba" para Roche) da 404, el literal del CSV `ROP.SW` es el real -- confirma otra vez que no hay que adivinar ningun ticker sin probarlo. `BRKB` (asi, sin separador, como lo escribe iShares para Berkshire Hathaway Clase B) se corrigio a mano a `BRK-B` tras confirmar que es el UNICO ticker de EEUU de todo el CSV donde el proveedor omite el separador de clase que Yahoo si exige -- revisado el resto de "CLASS A/B" de EEUU del CSV uno a uno, ninguno mas lo necesitaba.

**Bug real encontrado de camino, corregido antes de anhadir el universo**: `Application::resolveTickerRequest()` pasaba los tickers de CUALQUIER universo por `TickerNormalizer::normalize()`, que trunca a 60 -- el mismo bug que `Utils\UniverseTickerResolver` ya corrigio el 2026-08-18 para el camino CLI, pero nunca se corrigio en el camino WEB porque ningun universo individual lo habia superado hasta hoy. `msci_world` (1251 tickers) lo hace evidente. Corregido: los tickers de un universo de config ya vienen limpios/validados y NO necesitan pasar por el normalizador (pensado para el buscador de texto libre).

**Diseño "solo cron"**: 1251 tickers analizados en vivo en una sola peticion web arriesgarian timeout/rate-limit de Yahoo (nunca probado con exito en este proyecto a esa escala). Nuevo campo `selectable` en `Config\UniverseConfig::all()` (`true` por defecto, ausente en el resto de universos): `msci_world` se marca `selectable => false`, lo que lo excluye del desplegable del Home (`DashboardPage::renderUniverseOptions()`) y hace que `?universe=msci_world` en la URL caiga en `largecap60` (`Application::isValidUniverseKey()`), verificado por HTTP real (~1,2s de respuesta, nada que ver con analizar 1251 tickers). `bin/analyze.php --universe=msci_world`/`--all-universes` SI lo procesa completo, porque usa `Utils\UniverseTickerResolver`, que no pasa por `isValidUniverseKey()`. Como los despliegues a la Pi siguen pausados, la siembra real de `score_history`/`fundamentals_history` para estos tickers no empieza hasta que se reanude el cron alli.

**Cautelas documentadas, no bloqueantes**: Reino Unido cotiza en GBp (peniques) e Israel en agorot -- inocuo para indicadores tecnicos e incluso para los ratios que Yahoo ya calcula, pero riesgo real de bug de 100x si algo recalculara ratios por-accion mezclando precio de Yahoo con fundamentales de otra fuente para estos tickers (`PointInTimeFundamentalsBuilder.php:171`, sin uso activo hoy fuera de EEUU/`.MC`). Cotizaciones muy recientes (Verisure en Suecia, Kioxia en Japon) tienen historico corto -- dato real, no fallo de mapeo. `HOLX` (Hologic) y un residuo interno de Constellation Software quedaron fuera por no tener ticker real utilizable en su fila del CSV (Constellation Software SI esta en la lista con su ticker real, `CSU.TO`).

**Nota del usuario a considerar mas adelante, no resuelta hoy**: preguntó si EODHD podría usarse para actualizar/completar el histórico de estos tickers nuevos. Ya se confirmó en esta misma sesión (bloque "Segundo bloque" de `roadmap.md`) que `/api/eod`/`/api/div`/`/api/splits` de EODHD devuelven 401/403 con el plan actual para CUALQUIER ticker, asi que no serviria para precio historico. Queda sin comprobar si el plan actual de EODHD cubre **fundamentales** (no precio) para tickers fuera de EEUU/`.MC` -- pendiente, no una tarea de hoy.
...
## 2026-09-07 - Hallazgo real: `filing_date` de EODHD es un placeholder (= fecha de cierre) para Ibex35 completo y varios ADR -- nuevo chequeo en `FundamentalsQualityAuditor`, no afecta al veredicto ya cerrado de P3.3/`v2.114`

Estado: investigado, verificado de forma independiente (no solo por un agente), y con un chequeo permanente nuevo añadido al auditor de calidad existente. Cero cambios en `PointInTimeFundamentalsBuilder`, cero cambios en `config/weights.php`.

**Como aparecio.** Buscando un universo genuinamente independiente para repetir la investigacion del "score fundamental" (condicion pendiente desde el `2026-08-21`, ver `roadmap.md`), `china_adr`/`asia_pacific_adr`/`latam_adr` (66 tickers en `config/universes.php`) parecian la opcion mas barata: ya curados, ya verificados en vivo el `2026-07-31`, y con `fundamentals_history` ya poblado 2016-2026 (confirmado por SQL directo, sin backfill nuevo necesario). Antes de medir nada, `fiabilidad-datos-mercado` investigo si la cadencia trimestral que asume `PointInTimeFundamentalsBuilder` es correcta para estos emisores extranjeros (muchos presentan 20-F, a veces semestral). **La cadencia SI es trimestral real para los 10 tickers muestreados** (nada de bug tipo `v2.110`), pero el agente encontro un problema DISTINTO: `filing_date` coincide con `date` (cierre del trimestre) en una fraccion sustancial de casos.

**Verificacion propia, independiente del agente**, contra `eodhd_raw_fundamentals` (JSON archivado, sin red): confirmado con una consulta directa. Alcance exacto medido sobre los 890 tickers archivados, trimestres desde 2020-01-01:

- **35/35 tickers `.MC` (Ibex, 100% del universo español) tienen filing_date==date en el 96-100% de sus trimestres desde 2020.** Ejemplos literales: `BBVA.MC`/`REP.MC`/`IBE.MC`/`ITX.MC` al 100%; `SAN.MC` al 96% (25/26).
- 22 tickers NO-`.MC` de 809 tambien >=75%, sobre todo ADR latinoamericanos/asiaticos: `AMX`, `ASR`, `BBD`, `CX`, `FMX`, `IBN`, `ITUB`, `KT`, `WIT`, `XPEV`... (mas un puñado de casos raros: bancos quebrados en 2023 `FRC`/`SBNY`, tickers legacy con poca muestra).
- Sobre el universo de 66 ADR concreto que se queria usar: solo **13/66 (20%) limpios de verdad (&lt;10%)**; **31/66 (47%) contaminados al &gt;=50%**, incluidos casi todos los nombres mas grandes y liquidos (`BABA` 38%, `TM` 58%, `ITUB` 77%, `PBR` 50%, `NU` 73%). Excluir solo los peores dejaria una muestra demasiado pequeña para el t pareado estandar del proyecto.
- Tickers core de EEUU (`AAPL`, `MSFT`) estan **0% afectados desde 2020** (el ~20% que tienen en total esta concentrado antes de 2010).
- Agregado global desde 2020 sobre los 890: 2.227/20.674 trimestres-ticker (10,8%), pero muy concentrado por geografia: 719/890 tickers (80,8%) tienen &lt;25% (ruido disperso, casos legitimos de publicacion rapida), el resto se concentra en Ibex/Latam/parte de Asia.

**Por que importa:** `PointInTimeFundamentalsBuilder::orderedFiledBefore()` filtra estrictamente por `filingDate &lt;= D` (la regla de no-look-ahead de todo el sistema point-in-time). Cuando `filing_date` es un placeholder igual al cierre, el builder cree que el trimestre era publico el mismo dia que cerro, cuando en la practica tardo semanas o meses en conocerse: es look-ahead bias en la VENTANA de visibilidad, no un error de magnitud como el bug de `v2.110` (las cifras -- TTM, ratios -- siguen siendo correctas).

**¿Afecta a las investigaciones ya cerradas de "score fundamental" (P3.3 del 2026-09-03, corregida en `v2.114` del 2026-09-04)?** Consultado `auditor-estadistico` con las cifras exactas de arriba, y verificado por mi de forma independiente (no solo aceptado el argumento): el universo real de 636 tickers de esas dos mediciones NO se construye desde `config/universes.php` en bloque, sino desde `storage/scratch/build_universe.php` -- union de miembros historicos confirmados del S&P 500 (`index_membership WHERE index_code='GSPC'`) mas "dropped pero cotizando" verificados. Inspeccionado el `point_in_time_universe.txt` real generado ese dia (636 lineas): **cero tickers `.MC`, cero coincidencias con los 22 ADR afectados** -- estructuralmente imposible que se colaran, IBEX35 nunca ha sido constituyente del S&P 500. **El veredicto nulo de P3.3/`v2.114` queda intacto, sin necesidad de remedir nada.** (`ibex35` si se usa en backtests TECNICOS -- SMA, RSI, inversion de señal -- pero esos no dependen de `filing_date` en absoluto.)

**¿Y la busqueda del universo independiente?** Sigue sin resolverse: `china_adr`/`asia_pacific_adr`/`latam_adr` no es viable tal cual por este mismo motivo (47% del universo contaminado), y arreglarlo necesitaria o bien excluir la mitad de la muestra (demasiado pequeña despues) o un lag estimado conservador para los tickers afectados (no implementado hoy, decision pendiente). Las otras dos vias ya conocidas (small/mid-cap EEUU: necesita curacion + backfill nuevo de EODHD; S&P500/Nasdaq100 completos: mismo regimen de mercado, ya descartado por `analista-mercado` el `2026-08-15`) siguen igual de abiertas que antes.

**Nuevo chequeo permanente, no solo una investigacion puntual**: añadido `filing_date_placeholder` a `Services\FundamentalsQualityAuditor::auditRawPayload()` (severidad `warning`, igual que `currency_unit_jump`/`extreme_ratio`: un caso aislado puede ser una publicacion legitimamente rapida, la señal real esta en la frecuencia agregada). Solo audita `Income_Statement` (la unica seccion cuyo `filing_date` usa realmente `EodhdFiscalPeriodProvider::parse()`; repetirlo en `Balance_Sheet`/`Cash_Flow` triplicaria el mismo hallazgo). `bin/audit-fundamentals-quality.php` lo recoge automaticamente sin cambios (el informe agrega por `$issue->type` de forma generica). Verificado en vivo: `SAN.MC` 156 hallazgos, `AAPL` 34 (historicos, no recientes), `ITUB` 96, `MELI` 9.

**Tests**: 3 nuevos en `FundamentalsQualityAuditorTest.php` -- detecta `filing_date==date` (caso `SAN.MC`), no marca un `filing_date` posterior real como placeholder, y no repite el hallazgo si la coincidencia esta solo en `Balance_Sheet`/`Cash_Flow` y no en `Income_Statement`.

Verificado: `ddev exec vendor/bin/phpunit` -- **656 tests, 1.780 assertions, OK** (1 skip preexistente, sube desde 653), `ddev exec vendor/bin/phpstan analyse` -- **sin errores**. `config/weights.php` no se toca, `PointInTimeFundamentalsBuilder` no se toca (el chequeo es diagnostico puro, no corrige nada).

---

## 2026-09-07 (segunda entrada) - Universo nuevo: S&P MidCap 400 (`sp400`, 400 tickers reales), para el universo independiente pendiente desde el 2026-08-21

Estado: implementado y verificado. El usuario elige explicitamente esta via (frente a "aplicar un lag estimado" o "dejar los fundamentales de lado por ahora") tras el hallazgo de `filing_date` de relleno en los universos ADR (entrada anterior, mismo dia).

**Fuente real, mismo patron que `msci_world`** (verificado por `fiabilidad-datos-mercado` antes de escribir nada en `config/`): el CSV publico de holdings de iShares Core S&P Mid-Cap ETF (`IJH`, fondo de replica fisica/muestreo representativo, no sintetico), descargado el mismo dia. 407 filas `Asset Class=Equity`, de las cuales 7 son `Type=SWAP` que repiten el ticker de una tenencia `EQUITY` ya contada -- 400 tickers unicos, verificado por mi de forma independiente contando directamente el CSV crudo (no solo aceptando la cifra del agente). Se descarto mezclar con `IJR` (S&P SmallCap 600, tambien descargado y guardado en `storage/scratch/ijr_holdings.csv` por si se quiere una segunda ronda): un unico regimen homogeneo es mas simple de interpretar estadisticamente que mezclar dos tamaños de empresa distintos en la misma prueba, y 400 tickers ya es un tamaño comparable a `sp500` (503) sin acercarse a la escala "solo cron" de `msci_world`.

**Verificacion 400/400 contra Yahoo real** (`chart/{ticker}`, no de memoria): confirmado que todos cotizan hoy como `EQUITY` en bolsa de EEUU. Un solo caso de formato, mismo patron que `BRKB`->`BRK-B` en `msci_world`: el CSV escribe `MOGA` (Moog Inc. Clase A) sin separador de clase; corregido a mano a `MOG-A` tras confirmar en Yahoo (`longName="Moog Inc."`, NYSE) -- verificado tambien por mi mismo con una consulta directa al endpoint de Yahoo, no solo con el reporte del agente. 17 tickers son OPV/spin-offs de 2021-2025 (`BROS`, `TOST`, `KD`, `ESAB`, `CRBG`, `NXT`, `CR`, `TLN`, `KNF`, `CAVA`, `SN`, `CART`, `BTSG`, `AHR`, `ULS`, `SARO`, `SOLS`) con menos de ~5 años de historico -- dato real, no fallo de mapeo, igual que las cotizaciones recientes ya documentadas en `msci_world`. 36/400 ya estaban en otro universo del fichero (`financials`/`healthcare`/`industrials`/`msci_world`/`asia_pacific_adr`); los **364 restantes son genuinamente nuevos**, nunca usados en ningun backtest de este proyecto.

**`selectable` por defecto (a diferencia de `msci_world`)**: 400 tickers es del mismo orden que `sp500` (503, ya seleccionable desde el Home sin problema documentado), asi que `sp400` no necesita el tratamiento "solo cron" -- aparece en el desplegable del Home igual que cualquier otro universo curado. Verificado por HTTP que el Home lista "S&P MidCap 400" en el desplegable; `?universe=sp400` resuelve sin truncar (confirmado con `UniverseTickerResolverTest`, ver mas abajo). Una primera visita en vivo con datos de Yahoo sin cachear tarda varios minutos con 400 tickers -- mismo comportamiento esperable que tendria `sp500` la primera vez que alguien lo mira, no un problema especifico de este universo.

**Proposito distinto de `sp500`/`nasdaq100`**: esos dos llevan escrito explicitamente "no apto como universo independiente, mismo regimen que `largecap60`" (`v2.105`). `sp400` es justo lo contrario: se añade PARA satisfacer esa condicion, un regimen de mercado genuinamente distinto (mid-cap, no large-cap EEUU) para repetir la investigacion del "score fundamental" (P3.3/`v2.114`, veredicto nulo) con un universo que no sea otro recorte de lo mismo ya medido.
...
## 2026-09-08 (segunda entrada) - P0 de la auditoria de backtesting de Astra: la vela de entrada se ignoraba por completo en la simulacion de stop-loss/objetivo

Estado: implementado y verificado. Astra (Codex) dejo `AUDITORIA_BACKTESTING_ASTRA_2026-09-08.md` con 8 hallazgos (P0 a P2) sobre `BacktestingService`. Antes de tocar nada se verificaron los dos P0 leyendo el codigo real (no solo aceptando el documento): **ambos confirmados, exactos**.

**Bug 1: `simulateManagedExit()` empezaba a comprobar en `$offset = 1` (`$history[$entryIndex + 1]`), dejando la vela de ENTRADA completa fuera de la simulacion.** La posicion se compra en la apertura de esa vela (`entryPrice = $entry->getOpen()`), pero su alto/bajo nunca se comprobaban contra el stop/objetivo -- una caida o subida brusca justo el dia de la compra nunca se detectaba, aunque el riesgo ya empieza en la apertura, no al dia siguiente.

**Bug 2: dentro de cada vela, se comprobaba siempre el low contra el stop ANTES que el high contra el objetivo, sin mirar la apertura.** Un hueco alcista que ya abre por encima del objetivo (venta resuelta en apertura, sin exposicion al resto de la sesion) podia perder frente a un minimo posterior que tambien perforase el stop, invirtiendo el orden real de los hechos: la apertura sucede ANTES que cualquier minimo/maximo intradia posterior.

**Alcance verificado por mi mismo (no solo citado del documento)**: `forward_return` se calcula aparte (`(($future->getClose() / $entryPrice) - 1) * 100`, linea ~1806), sin pasar nunca por `simulateManagedExit()`. **Los dos bugs afectan solo a `managed_return`/`exit_reason`/`exit_day` y a las tasas `stop_loss_rate`/`target_rate`/`horizon_rate` -- ninguno de los dos toca `forward_return`, que es lo unico que alimenta la alpha transversal de `runCrossSectional()`. Los veredictos ya cerrados de P3.3/`v2.114`/`sp400`/`sp600` (entradas de ayer y hoy) no cambian.**

**Diseño de la correccion**: `simulateManagedExit()` ahora recorre `$offset = 0..horizonDays` (antes `1..horizonDays`), añadiendo la vela de entrada como "dia 0" sin cambiar cuantas sesiones representa un horizonte (mismo criterio que ya usa `forward_return`: `$index` es el dia de referencia, no el primero de los `horizonDays`). La logica de cada vela se extrajo a `resolveDayExit()`, que primero comprueba si la propia apertura YA resuelve la salida sin ambigüedad (hueco alcista -> objetivo, hueco bajista -> stop, cada uno a su precio real de apertura, mismo criterio de huecos que `v2.73`) y solo cae al criterio conservador de low/high (stop gana el empate) cuando la apertura cae dentro de la banda `[stop, objetivo]` -- ahi si el orden intradia es real mente desconocido.

**Verificacion de que no rompe nada existente, hecha ANTES de tocar codigo**: se rastrearon a mano los valores de cada test existente de `BacktestingServiceTest.php`/`BacktestingServiceP0FixesTest.php` contra la logica nueva -- la vela de entrada hardcodeada en `historyWithPostEntryPath()` (alto 104,5/bajo 103,5) cae siempre DENTRO de la banda de stop/objetivo de estos tests (101,5/109,0), asi que nunca dispara nada en el dia 0 y ninguna aserción existente cambiaba. La UNICA excepcion, exactamente la que señalaba Astra: `BacktestingServiceP0FixesTest.php::testLaEntradaUsaLaAperturaDeLaSesionSiguienteNoElCierreDeLaSenal()` usaba una vela de entrada con alto/bajo (205,0/195,0) que SI cruzaba ambos niveles por accidente (el test prueba el PRECIO de entrada, no el mecanismo de stop/objetivo) -- corregido el fixture a un rango seguro (200,5/199,5) sin tocar la aserción que el test realmente comprueba.

**Tests nuevos** (`BacktestingServiceTest.php`): la vela de entrada dispara el stop-loss (dia 0), la vela de entrada dispara el objetivo (dia 0), la vela de entrada cruza ambos (gana stop, mismo criterio conservador), y el caso central del bug 2 -- un hueco alcista que abre ya por encima del objetivo pero cuyo low tambien perfora el stop el mismo dia, que debe resolverse como `target` (la apertura ya vendio) y no como `stop_loss` (lo que habria devuelto el codigo antiguo).
...
## 2026-09-08 (cuarta entrada) - P1 de la auditoria de Astra: guarda de seguridad contra huecos de calendario por ticker, con medicion real antes de decidir la magnitud de la correccion

Estado: implementado y verificado. Decision del usuario: "tu decides" con qué seguir, con el mandato de que cualquier cambio (arquitectonico o no) se haga solo si mejora de verdad el motor.

**El hallazgo de Astra es real, verificado leyendo `sampleHistory()`/`runCrossSectional()`**: cada ticker se muestrea por INDICE LOCAL (cada `$step` velas desde el 80 de SU PROPIO array), no desde un calendario compartido construido antes. Un ticker con un hueco (una fecha en la que otros cotizaron y el no) desplaza todas sus fechas muestreadas posteriores frente al resto, sin que `runCrossSectional()` lo note: sus muestras siguen entrando en `$samplesByDate`, solo que en fechas ligeramente distintas a las que "deberian".

**Antes de decidir si hacia falta el rediseño completo (calendario construido ANTES de muestrear cada ticker, arquitectura genuinamente distinta y de riesgo alto por tocar el camino comun de los cuatro modos), se midio la magnitud real**: con los precios de `sp400`+`sp600` ya cacheados de hoy (1.002 tickers, sin ninguna llamada nueva), se comparo el historico propio de cada ticker contra el calendario union dentro de su rango activo (primera a ultima cotizacion propia). **Resultado: 0/1.002 tickers con algun hueco.** Los datos de Yahoo para este universo son completamente consistentes de calendario -- el riesgo es real en el papel (reproducido de forma sintetica por Astra quitando una vela), pero hoy no esta corrompiendo ninguna medicion.

**Decision proporcionada al riesgo medido**: en vez del rediseño completo (alto riesgo de introducir bugs nuevos en los cuatro modos para un problema con impacto medido de CERO hoy), se añade una guarda de seguridad de bajo riesgo: `runCrossSectional()` pasa a dos pasadas -- la primera recoge fechas propias de cada ticker (sin duplicar en memoria el historico completo: solo las fechas, no las velas) y construye el calendario compartido; la segunda, YA con el calendario completo, comprueba cada ticker con `hasCalendarGap()` (nuevo metodo privado) y **excluye entero** al que tenga un hueco interno, en vez de sumar sus muestras desalineadas. Nuevos campos publicados, mismo criterio de "no ocultar la merma" que el resto de contadores: `samples_dropped_calendar_gap`, `tickers_dropped_calendar_gap`.

**Alcance deliberadamente mas estrecho que "cualquier diferencia de longitud de historico"**: un ticker que EMPEZO MAS TARDE (OPV reciente, ya cubierto por un test existente, `testFechasSinSuficienteAmplitudNoSeEvaluan`) no cuenta como hueco -- solo se compara DENTRO de su propio rango activo. Es una distincion real: `sp400`/`sp600` tienen 17+54 tickers de OPV reciente (dato legitimo, ya se autoexcluyen de fechas donde son minoria via `dates_dropped_low_breadth`), frente a 0 tickers con huecos internos genuinos.

**Test nuevo** (`BacktestingServiceCrossSectionalTest::testUnTickerConUnHuecoInternoSeExcluyeEnteroEnVezDeDesalinear()`): quita una vela intermedia del prefijo plano de un ticker (fecha muy anterior a cualquier señal real, para que sea inequivocamente un hueco y no un arranque tardio) y comprueba que el resultado con ese ticker (excluido entero) coincide EXACTO con el universo de los tres tickers limpios -- ni rastro del ticker con hueco. Encontrado y corregido un error real de CONSTRUCCION del propio test mientras se escribia (no del motor): `$clean + ['DDD' => $dddWithGap]` usa el operador `+` de PHP, que en colision de claves conserva el valor del array IZQUIERDO -- `$clean['DDD']` (el original, sin hueco) ganaba silenciosamente sobre el reemplazo. Corregido a `array_merge()`, que si sobrescribe. Verificado con un script de depuracion aparte antes de confiar en el test.

**Verificado sobre datos reales, no solo sinteticos**: se repitio la medicion completa de `sp400` (`run_sp400_fundamental_backtest.php`) tras el cambio -- resultado identico al ya publicado (t=-1,52/-1,80), como se esperaba con 0 huecos reales detectados. El cambio es una guarda para el futuro (una recaptura con un hueco de proveedor, un universo nuevo con datos menos limpios), no una correccion de un problema ya observado en ninguna medicion ya cerrada.

**Sigue pendiente, documentado como limitacion conocida, no urgente dado el impacto medido**: la version "completa" de este hallazgo -- tickers con longitudes de historico DISTINTAS (no solo huecos) tambien desalinean su muestreo por indice frente a pares mas antiguos -- ya esta parcialmente mitigada por el filtro de amplitud existente (`dates_dropped_low_breadth`, confirmado por un test ya existente), y un rediseño completo a calendario-primero solo se justificaria si una medicion futura mostrase un efecto real, no hoy.
