...
final class BacktestingServiceStatisticsTest extends TestCase
{
    ...
    private function history(string $startDate, array $increments, float $startClose = 100.0, float $baselineStep = 0.05): array
    {
        ...
        // P0.3: prefijo plano (MomentumPrefixFixture) para que Momentum 12-1
        // deje de ser null en el indice 80 -sin el, sampleHistory()
        // descartaba TODAS las muestras de este fichero-. $startClose y no
        // otro valor: la primera vela del patron que sigue (i=0, sin
        // incremento aplicado todavia) cierra exactamente en $startClose,
        // asi que el prefijo tiene que terminar ahi para no crear un salto
        // de precio que distorsione el ATR14 (ver el docblock del trait).
        ...
        // que antes ($index + 1 + horizonDays < count, no $index +
        // horizonDays < count). Sin esta vela final, el ultimo punto que
        // este fixture pretendia muestrear se quedaba fuera del bucle.
        // $baselineStep y no un cierre plano: en el tramo alcista
        // (`$risingBaseline`), `testConDesviacionTipicaCeroNoHayDivisionPorCeroEnElTStat`
        // necesita que las 5 ventanas de 5 dias sigan dando el MISMO
        // forward_return (0,24) para seguir demostrando el caso limite de
        // desviacion tipica cero -con un cierre plano en vez de continuar
        // la progresion aritmetica, la ultima ventana rompia esa igualdad
        // por 0,05 pp-. Fuera del tramo alcista (los fixtures bajistas de
        // este fichero) se mantiene el cierre plano: nada depende de que
        // esa ultima vela iguale a las demas.
        $paddingClose = $risingBaseline ? $close + $baselineStep : $close;
        $quotes[] = new HistoricalQuote($date, $paddingClose, $paddingClose + 0.5, $paddingClose - 0.5, $paddingClose, 1_000_000);

        return $quotes;
    }

    /**
     * 111 velas propias (mas el prefijo de MOMENTUM_PREFIX_LENGTH velas
     * planas que añade `history()`, ver `MomentumPrefixFixture`): tras el
     * tramo alcista de arranque, 15 dias de subidas de ritmo variable y
     * despues un desplome sostenido. Con horizonte=5/paso=5 produce 6
     * muestras, 4 de ellas BUY con `forward_return` distintos entre si
     * (+1,63 / +1,13 / -0,75 / -17,84 con P0.1: la entrada es la apertura
     * de la sesion siguiente a la señal, no su propio cierre) y 2 no-BUY,
    ...
    /**
     * 106 velas propias (mas el prefijo de MOMENTUM_PREFIX_LENGTH, ver
     * `MomentumPrefixFixture`) con el MISMO incremento constante de +0,05
     * durante todo el recorrido, incluida la vela de P0.1 que añade
     * `history()` -por eso sigue una progresion aritmetica pura hasta el
     * final-: las 5 muestras tienen exactamente el mismo `forward_return`
     * (+0,24%), aunque solo 4 sean BUY (la 5a queda en HOLD por puntuacion,
     * no por precio), asi que la desviacion tipica sigue siendo exactamente
     * 0. Fixture del caso limite de division por cero.
     *
     * @return list<HistoricalQuote>
     */
    ...
    /**
     * 91 velas propias (mas el prefijo de MOMENTUM_PREFIX_LENGTH, ver
     * `MomentumPrefixFixture`): tramo alcista de arranque y despues un
     * desplome inmediato. Produce 2 muestras, UNA sola BUY: fixture del
     * caso n < 2, en el que no hay dispersion calculable.
     *
     * @return list<HistoricalQuote>
     */
     ...
     * `riseThenCrashHistory()` (6 muestras, 4 BUY: +1,63 / +1,13 / -0,75 /
     * -17,84, media de todos los dias -7,11) y `BBB` con
     * `steadyRiseHistory()` (5 muestras, 4 BUY de +0,24 cada una -el
     * "buy_signals baja de 9 a 8" de mas abajo-, media de todos los dias
     * +0,24). Ambos historicos empiezan el mismo dia, asi que sus muestras
     * BUY caen en los mismos dos meses (2024-03 y 2024-04): justo el solape
     * entre tickers que motiva la vista por episodios.
     *
     * Cifras recalculadas a mano con los retornos de P0.1 (entrada en la
     * apertura de la sesion siguiente, no en el cierre de la señal):
     *
     * - muestras: 6 + 5 = 11; señales BUY: 4 + 4 = 8 (de 2 tickers)
     * - retorno medio BUY: (-15,83 + 0,96)/8 = -1,86
     * - media de todos los dias, ponderada por muestras de cada ticker:
     *   (6*-7,11 + 5*0,24)/11 = -3,77
     * - alpha: -1,86 - (-3,77) = +1,91
     * - win rate BUY: 6 de 8 positivas = 75%
     * - 2024-03: (1,63+1,13-0,75+0,24+0,24+0,24)/6 = +0,46
     *   2024-04: (-17,84+0,24)/2 = -8,80
     *   media de medias mensuales: (0,46 - 8,80)/2 = -4,17, muy por debajo
     *   del -1,86 por muestra: el resultado esta dominado por un unico mes
    ...
    public function testAgregadoDeUniversoConDosTickersQueComparteMeses(): void
    {
        ...
        self::assertSame(-1.86, $aggregate['avg_buy_forward_return']);
        self::assertSame(-3.77, $aggregate['avg_all_days_forward_return']);
        self::assertSame(1.91, $aggregate['buy_alpha_vs_all_days']);
        self::assertSame(75.0, $aggregate['win_rate_buy']);
        self::assertSame(2, $aggregate['distinct_buy_tickers']);
        self::assertSame(2, $aggregate['distinct_buy_months']);
        self::assertSame(-4.17, $aggregate['avg_of_monthly_avgs']);
        self::assertSame(['month' => '2024-04', 'avg_forward_return' => -8.8], $aggregate['worst_month']);
    }
    ...
    public function testBuySamplesContieneSoloLasMuestrasDeCompraConSuFecha(): void
    {
        ...
        self::assertSame([
            ['date' => '2024-03-21', 'forward_return' => 1.63],
            ['date' => '2024-03-26', 'forward_return' => 1.13],
            ['date' => '2024-03-31', 'forward_return' => -0.75],
            ['date' => '2024-04-05', 'forward_return' => -17.84],
        ], $ticker['buy_samples']);
    }
