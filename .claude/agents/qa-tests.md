...
## Piensa en adversario, no solo en el camino feliz

Para codigo que toca datos de mercado o entrada externa, pregunta explicitamente: ¿que pasa si la API del proveedor devuelve `null` en un campo fundamental?, ¿si no hay conexion y lanza excepcion?, ¿si un ticker deja de existir a mitad de un backtest?, ¿si un valor viene con un salto de precio brusco (split no ajustado)? No inventes el comportamiento esperado — coordina con `fiabilidad-datos-mercado` para saber que hace hoy realmente el codigo ante esos casos (ver su seccion "Diagnostico de errores tipicos") antes de escribir el assert.
